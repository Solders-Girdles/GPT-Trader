...
class AlertingSystem:
    ...
    def check_metric(self, metric_name: str, value: float, context: Optional[Dict] = None):
    ...
    def trigger_alert(self, level: AlertLevel, category: str, message: str, 
    ...
    def _setup_default_handlers(self):
        ...
        def console_handler(alert: Alert):
            icon = "ℹ️" if alert.level == AlertLevel.INFO else "⚠️" if alert.level == AlertLevel.WARNING else "❌"
            print(f"{icon} [{alert.level.name}] {alert.category}: {alert.message}")
            ...
            if alert.level in [AlertLevel.CRITICAL, AlertLevel.ERROR]:
                try:
                    with open("critical_alerts.log", "a") as f:
                        f.write(f"{alert.timestamp.isoformat()} [{alert.level.name}] {alert.category}: {alert.message}\n")
                except Exception as e:
    ...
    def _setup_default_rules(self):
        ...
        self.register_rule(AlertRule(
            name="high_cpu",
            condition=lambda ctx: ctx.get('metric_name') == 'cpu_percent' and ctx.get('value', 0) > 80,
        ...
        self.register_rule(AlertRule(
            name="high_memory",
            condition=lambda ctx: ctx.get('metric_name') == 'memory_percent' and ctx.get('value', 0) > 85,
            level=AlertLevel.WARNING,
            category="resource",
        ...
        self.register_rule(AlertRule(
            ...
            condition=lambda ctx: ctx.get('metric_name') == 'memory_percent' and ctx.get('value', 0) > 95,
        ...
        self.register_rule(AlertRule(
            name="trading_error",
            condition=lambda ctx: ctx.get('category') == 'trading' and ctx.get('error_count', 0) > 5,
            level=AlertLevel.CRITICAL,
            category="trading",
        ...
        self.register_rule(AlertRule(
            name="data_stale",
            condition=lambda ctx: ctx.get('metric_name') == 'data_age_minutes' and ctx.get('value', 0) > 30,
            level=AlertLevel.WARNING,
            category="data",
    ...
    def clear_alerts_by_category(self, category: str):
    ...
    def get_alert_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                'critical_alerts': [a for a in self.alerts.values() if a.level == AlertLevel.CRITICAL],
                'recent_alerts': self.alert_history[-10:] if self.alert_history else []
            }
    
    def export_alert_history(self, filename: str = None) -> str:
