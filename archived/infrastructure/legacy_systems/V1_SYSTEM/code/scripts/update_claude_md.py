...
class ClaudeMDUpdater:
    ...
    def update_date(self) -> None:
        ...
        self.content = re.sub(
            r"\*\*Date\*\*: \d{4}-\d{2}-\d{2}", f"**Date**: {self.today}", self.content
        )
...
"""
        ...
        section = (
            "High Priority Issues" if severity in ["Critical", "High"] else "Medium Priority Issues"
        )
...
"""
        ...
        if section_marker in self.content:
            ...
            if first_decision != -1:
                ...
                self.content = (
                    self.content[:insert_after_header]
                    + decision_template
                    + self.content[insert_after_header:]
                )
