...
from .walk_forward_validator import WalkForwardResults
...
class ValidationReporter:
            ...
            "sharpe_ratio": (
                {
                    ...
                    "min": min(
                        (f.sharpe_ratio for f in results.fold_results if f.sharpe_ratio),
                        default=None,
                    ),
                    "max": max(
                        (f.sharpe_ratio for f in results.fold_results if f.sharpe_ratio),
    ...
    def _analyze_degradation(self, results: WalkForwardResults) -> dict:
        ...
        degradation_analysis = {
            ...
            "degradation_rate": (
                len(results.degradation_folds) / results.n_folds if results.n_folds > 0 else 0
            ),
        }
        ...
        if results.degradation_folds:
            ...
            degradation_analysis["degraded_performance"] = {
                ...
                "accuracy_drop": (
                    np.mean(normal_accuracies) - np.mean(degraded_accuracies)
                    if normal_accuracies and degraded_accuracies
                    else None
                ),
            }
            ...
            for fold in results.fold_results:
                if fold.is_degraded and fold.performance_change:
                    if fold.performance_change < -0.1:
                        triggers.append(
                            f"Fold {fold.fold_number}: Sudden drop ({fold.performance_change:.3f})"
                        )
                    elif fold.performance_change < -0.05:
                        triggers.append(
                            f"Fold {fold.fold_number}: Gradual decline ({fold.performance_change:.3f})"
                        )
    ...
    def _analyze_features(self, results: WalkForwardResults) -> dict:
        ...
        sorted_features = sorted(
            results.feature_importance_stability.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def _summarize_backtesting(self, results: WalkForwardResults) -> dict | None:
        ...
        return {
            "sharpe_ratio": {
                ...
                "positive_rate": (
                    sum(1 for s in sharpe_ratios if s > 0) / len(sharpe_ratios)
                    if sharpe_ratios
                    else 0
                ),
            },
            ...
            "risk_adjusted_return": (
                results.mean_sharpe * np.sqrt(252) if results.mean_sharpe else None
            ),  # Annualized
        }
    ...
    def _generate_recommendations(self, results: WalkForwardResults) -> list[str]:
        ...
        if results.mean_test_accuracy < 0.55:
            recommendations.append(
                "Model accuracy below acceptable threshold. Consider feature engineering improvements."
            )
        ...
        if results.degradation_detected:
            ...
            if degradation_rate > 0.3:
                recommendations.append(
                    f"High degradation rate ({degradation_rate:.1%}). Implement regular retraining."
                )
            else:
                recommendations.append(
                    f"Degradation detected in {len(results.degradation_folds)} folds. Monitor closely."
                )

        # Stability recommendations
        if results.stability_score < 0.7:
            recommendations.append(
                "Low stability score. Consider ensemble methods or regularization."
            )

        # Backtesting recommendations
        if results.mean_sharpe and results.mean_sharpe < 0.5:
            recommendations.append(
                "Low Sharpe ratio. Review risk management and signal generation."
            )
        ...
        if results.feature_importance_stability:
            ...
            if low_stability > len(results.feature_importance_stability) * 0.3:
                recommendations.append(
                    "Many unstable features. Consider feature selection refinement."
                )
    ...
    def _generate_and_save_plots(
        self, results: WalkForwardResults, model_name: str, timestamp: str
    ) -> dict[str, str]:
        ...
        if "total_return" in df.columns and df["total_return"].notna().any():
            ...
            ax.axvline(
                x=returns.mean(), color="r", linestyle="--", label=f"Mean: {returns.mean():.1f}%"
            )
        ...
        if results.feature_importance_stability:
            ...
            features, stabilities = zip(*sorted_features, strict=False)
    ...
    def _generate_html_report(
        self, report: dict, results: WalkForwardResults, model_name: str, timestamp: str
    ) -> Path:
...
if __name__ == "__main__":
    ...
    from .walk_forward_validator import WalkForwardConfig, create_walk_forward_validator
    ...
    config = WalkForwardConfig(
        train_window=252, test_window=63, step_size=21, backtest_each_fold=True
    )
