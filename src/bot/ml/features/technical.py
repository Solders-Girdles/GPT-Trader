...
class TechnicalFeatureEngineer(FeatureEngineer):
    ...
    def __init__(self, config: Optional[ComponentConfig] = None):
        ...
        if config is None:
            config = ComponentConfig(
                component_id='technical_features',
    ...
    def _generate_momentum_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        features['macd_cross_up'] = ((macd > signal) & (pd.Series(macd).shift(1) <= pd.Series(signal).shift(1))).astype(int)
        features['macd_cross_down'] = ((macd < signal) & (pd.Series(macd).shift(1) >= pd.Series(signal).shift(1))).astype(int)
    ...
    def _generate_pattern_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if pattern_cols:
            ...
            features['pattern_strength'] = features['bullish_patterns'] - features['bearish_patterns']
