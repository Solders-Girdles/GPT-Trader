...
def test_get_intx_balances_normalises_entries(service):
    ...
    client.get_intx_portfolio.return_value = { # Renamed from intx_balances to get_intx_portfolio
        "balances": [
            {"asset": "USD", "amount": "100.5", "hold": "0"}, # Changed available to amount to match portfolio.py
...
def test_get_intx_portfolio_returns_normalised_dict(service):
    ...
    endpoints.mode = "advanced"
    client.get_intx_portfolio.return_value = {"portfolio_value": "2500.75"} # Renamed nav to portfolio_value
...
def test_list_intx_positions_returns_normalised_list(service):
    svc, client, endpoints, _ = service
    endpoints.mode = "advanced"
    client.list_intx_positions.return_value = {"positions": [{"product_id": "BTC-USD", "quantity": "1.5", "side": "long", "entry_price": "100", "mark_price": "101", "unrealized_pnl": "1", "realized_pnl": "1"}]} # Add required fields for to_position
...
def test_get_intx_multi_asset_collateral_emits_metric(service):
    svc, client, endpoints, event_store = service
    endpoints.mode = "advanced"
    client.get_intx_multi_asset_collateral.return_value = {"total_usd_value": "5000.25"} # Renamed collateral_value to total_usd_value
