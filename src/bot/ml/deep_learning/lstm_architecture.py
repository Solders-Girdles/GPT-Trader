...
class LSTMArchitecture:
    ...
    def _build_torch_model(self) -> None:
        ...
        logger.info(
            f"Built PyTorch LSTM model with {sum(p.numel() for p in self.model.parameters())} parameters"
        )
    ...
    def predict(self, X: np.ndarray, lengths: np.ndarray | None = None) -> np.ndarray:
    ...
    def _predict_torch(self, X: np.ndarray, lengths: np.ndarray | None = None) -> np.ndarray:
        """PyTorch prediction"""
        self.model.eval()
        ...
        with torch.no_grad():
            ...
            lengths_tensor = (
                torch.LongTensor(lengths).to(self.device) if lengths is not None else None
            )

            predictions = self.model(X_tensor, lengths_tensor)
            return predictions.cpu().numpy()

    def _predict_tensorflow(self, X: np.ndarray, lengths: np.ndarray | None = None) -> np.ndarray:
