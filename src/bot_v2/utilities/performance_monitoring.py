...
from .logging_patterns import get_logger
...
class PerformanceTimer:
    ...
    def stop(self) -> float:
        ...
        metric = PerformanceMetric(
            name=self.operation_name, value=duration, unit="s", tags=self.tags
        )
    ...
    def generate_report(self) -> str:
        ...
        if profile_data:
            for func_name, data in sorted(
                profile_data.items(), key=lambda x: x[1]["total_time"], reverse=True
            ):
                lines.append(
