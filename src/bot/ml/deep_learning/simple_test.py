...
def test_data_pipeline():
    ...
    try:
        ...
        # Create sequences
        X_seq, y_seq, lengths, timestamps = pipeline.create_sequences(data, "target", feature_cols)
...
def test_training_framework():
    ...
    try:
        ...
        logger.info(
            f"✓ Training Framework test passed - {results.total_epochs} epochs, {training_time:.1f}s"
        )
...
def test_attention_mechanisms():
    ...
    try:
        ...
        config = AttentionConfig(
            attention_type=AttentionType.SCALED_DOT_PRODUCT, d_model=64, num_heads=4, dropout=0.1
        )
...
def test_performance_targets():
    ...
    try:
        ...
        config = LSTMConfig(input_size=50, hidden_size=128, num_layers=2, sequence_length=30)
