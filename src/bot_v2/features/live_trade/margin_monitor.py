...
class MarginWindowPolicy:
    ...
    def __init__(self):
        ...
        self._next_window_change: Optional[datetime] = None
        
    def determine_current_window(self, current_time: Optional[datetime] = None) -> MarginWindow:
        """
        ...
        for funding_time in self._funding_times:
            ...
            elif (funding_hour == 0 and current_hour == 23 and 
                  current_minute >= 30):
                return MarginWindow.PRE_FUNDING
    ...
    def calculate_next_window_change(self, current_time: Optional[datetime] = None) -> datetime:
...
class MarginStateMonitor:
    ...
    def add_alert_callback(self, callback: Callable[[MarginSnapshot, str], None]):
    ...
    async def compute_margin_state(
        ...
        total_equity: Decimal,
        cash_balance: Decimal,
        positions: Dict[str, Dict]  # symbol -> {quantity, mark_price, ...}
    ) -> MarginSnapshot:
        """
        ...
        is_liquidation_risk = (total_equity - maintenance_margin_req) <= (maintenance_margin_req * self.liquidation_buffer)
    ...
    async def check_window_transition(self) -> Optional[Dict]:
        """
        ...
        if current_window != future_window:
            ...
            return {
                ...
                'leverage_reduction_factor': float(future_req.max_leverage / current_req.max_leverage) if current_req.max_leverage > 0 else 1.0
            }
    ...
    async def get_max_position_size(
        ...
        symbol: str,
        price: Decimal,
        available_equity: Optional[Decimal] = None
    ) -> Decimal:
        """
    ...
    def get_current_state(self) -> Optional[MarginSnapshot]:
        """Get current margin snapshot."""
        return self._current_snapshot
    
    def get_margin_history(self, hours_back: int = 24) -> List[Dict]:
    ...
    async def _check_alerts(self, snapshot: MarginSnapshot):
    ...
    def _store_snapshot(self, snapshot: MarginSnapshot):
...
async def create_margin_monitor(client=None, **kwargs) -> MarginStateMonitor:
