...
class LSTMTrainingFramework:
    ...
    def __init__(self, config: TrainingConfig):
        ...
        logger.info(f"Initialized LSTM training framework")
    ...
    def train(
        ...
        val_lengths: Optional[np.ndarray] = None
    ) -> TrainingResults:
        """
        ...
        if model.backend == "torch":
            results = self._train_torch(X_train, y_train, X_val, y_val, 
                                      train_lengths, val_lengths)
        elif model.backend == "tensorflow":
            results = self._train_tensorflow(X_train, y_train, X_val, y_val,
                                           train_lengths, val_lengths)
        else:
        ...
        if total_time > self.config.max_training_time_minutes * 60:
            logger.warning(f"Training time ({total_time:.1f}s) exceeded target "
    ...
    def _train_torch(
        ...
        X_val: np.ndarray,
        y_val: np.ndarray,
        train_lengths: Optional[np.ndarray],
        ...
        if self.config.scheduler_type == SchedulerType.STEP:
            ...
            scheduler = optim.lr_scheduler.ReduceLROnPlateau(
                ...
                factor=self.config.lr_factor, verbose=True
            )
        ...
        for epoch in range(self.config.epochs):
            ...
            for batch_idx, (X_batch, y_batch, lengths_batch) in enumerate(
                ...
                if hasattr(model.config, 'gradient_clipping'):
                    torch.nn.utils.clip_grad_norm_(model.parameters(), model.config.gradient_clipping)
            ...
            # Validation phase
            val_loss, val_accuracy = self._validate_torch(model, criterion, X_val, y_val, val_lengths)
            ...
            if self.tb_writer:
                ...
                self.tb_writer.add_scalar('Epoch/Learning_Rate', optimizer.param_groups[0]['lr'], epoch)
    ...
    def _save_checkpoint(self, model, optimizer, epoch: int, val_loss: float, is_best: bool = False):
