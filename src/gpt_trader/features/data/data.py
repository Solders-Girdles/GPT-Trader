...
# Console helpers
def console_success(*args): pass
def console_warning(*args): pass
def console_error(*args): pass
def console_cache(*args): pass
def console_storage(*args): pass
def console_data(*args): pass

def store_data(symbol: str, data: pd.DataFrame, **kwargs) -> bool:
    if _storage:
        success = _storage.store(symbol=symbol, data=data, data_type=kwargs.get('data_type'), source=kwargs.get('source'))
        ...
        class StatsObj:
             def __init__(self, **kwargs):
                 for k, v in kwargs.items():
                     setattr(self, k, v)
             @property
             def cache_entries(self): return 0
