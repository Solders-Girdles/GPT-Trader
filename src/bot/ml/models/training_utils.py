...
class ModelTrainer:
    ...
    def train_regime_detector(
        self, data: pd.DataFrame, symbol: str = None, n_regimes: int = 5, save_model: bool = True
    ) -> tuple[MarketRegimeDetector, dict[str, float]]:
        ...
        detector = MarketRegimeDetector(db_manager=self.db_manager, n_components=n_regimes)
    ...
    def prepare_training_data(
        self, symbol: str, start_date: str, end_date: str, source: str = "yfinance"
    ) -> pd.DataFrame:
    ...
    def cross_validate_regime_detector(
        self, data: pd.DataFrame, n_splits: int = 5, n_regimes_range: list[int] = [3, 4, 5, 6]
    ) -> dict[int, dict]:
        ...
        for n_regimes in n_regimes_range:
            ...
            for i in range(n_splits):
                ...
                train_data = pd.concat([features.iloc[:test_start], features.iloc[test_end:]])
    ...
    def _set_active_model(self, model_type: str, model_id: str):
        ...
        try:
            ...
            # Activate this model
            self.db_manager.execute(
                "UPDATE ml_models SET is_active = 1 WHERE model_id = ?", (model_id,)
            )
...
class RegimeAnalyzer:
    ...
    def analyze_regime_transitions(
        self, data: pd.DataFrame, lookback_days: int = 250
    ) -> dict[str, Any]:
    ...
    def _calculate_regime_returns(self, data: pd.DataFrame, regimes: np.ndarray) -> dict[str, dict]:
        ...
        for regime in np.unique(regimes):
            ...
            if len(regime_rets) > 0:
                regime_returns[regime] = {
                    ...
                    "sharpe_ratio": float(
                        np.mean(regime_rets) / (np.std(regime_rets) + 1e-10) * np.sqrt(252)
                    ),
    ...
    def get_regime_trading_signals(self, current_regime: str, confidence: float) -> dict[str, Any]:
