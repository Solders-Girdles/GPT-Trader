...
def calculate_indicators(data: pd.DataFrame) -> Dict[str, float]:
    ...
    indicators['price_change_1d'] = (close_prices[-1] / close_prices[-2] - 1) if len(close_prices) > 1 else 0
    ...
    # Support/Resistance
    if len(close_prices) >= 20:
        indicators['distance_to_high20'] = (np.max(high_prices[-20:]) - close_prices[-1]) / close_prices[-1] * 100
        indicators['distance_to_low20'] = (close_prices[-1] - np.min(low_prices[-20:])) / close_prices[-1] * 100
...
def calculate_high_low_ratio(high_prices: np.ndarray, low_prices: np.ndarray, window: int = 20) -> float:
...
def engineer_regime_features(base_features: RegimeFeatures) -> np.ndarray:
    ...
    stress_indicator = base_features.realized_vol_30d / 30 + (1 - base_features.correlation_market) / 2
