...
class TestSchedulerDriftDetectorIntegration:
    ...
    def test_combined_adaptation(self):
        ...
        scheduler_config = SchedulerConfig(
            scheduler_type=SchedulerType.ADAPTIVE, initial_lr=0.01, performance_window=20
        )
        scheduler = LearningRateScheduler(scheduler_config)

        drift_config = DriftDetectorConfig(warmup_period=20, performance_threshold=0.15)
        ...
        for i in range(50):
            ...
            # Check for drift
            drift = drift_detector.add_sample(
                features, target=target, performance_metric=performance
            )
