...
class MetricSeries:
    ...
    def add_point(self, value: float, tags: Optional[Dict] = None):
...
class MetricsCollector:
    def __init__(self):
        ...
        self._agent_counts = self._load_agent_counts()

    def _load_agent_counts(self) -> Dict[str, int]:
        """Load agent counts from .knowledge/STATE.json with sane fallbacks."""
        defaults = {
            'available_count': 45,
            'custom_count': 21,
            'builtin_count': 24,
        ...
        try:
            state_path = self._find_state_json()
            if not state_path:
                return defaults
            ...
            return {
                'available_count': int(workforce.get('total_agents', defaults['available_count'])),
                'custom_count': int(workforce.get('custom_agents', defaults['custom_count'])),
                'builtin_count': int(workforce.get('built_in_agents', defaults['builtin_count'])),
            }
        except Exception:
            return defaults

    def _find_state_json(self) -> Optional[Path]:
        """Search upwards from this file for .knowledge/STATE.json."""
        try:
            start = Path(__file__).resolve().parent
            for base in [start] + list(start.parents):
                candidate = base / '.knowledge' / 'STATE.json'
                if candidate.exists():
                    return candidate
                # Also check one level up (repo root when this file is in src/..)
                candidate2 = base.parent / '.knowledge' / 'STATE.json'
    ...
    def collect_system_metrics(self):
        ...
        # Agent workflow metrics (loaded from knowledge state)
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            total_errors = sum(v for k, v in self.counters.items() if 'error' in k or 'failure' in k)
