...
from .types import MarketRegime, VolatilityRegime, TrendRegime, RegimeFeatures
...
class HMMRegimeDetector:
        ...
        for t in range(n_obs-2, -1, -1):
            for i in range(self.n_states):
                beta[t, i] = np.sum(self.transition_matrix[i] * 
                                   self._emission_prob(returns[t+1]) * beta[t+1])
    ...
    def _emission_prob(self, observation: float) -> np.ndarray:
        ...
        for i in range(self.n_states):
            ...
            probs[i] = np.exp(-0.5 * (observation - self.emission_means[i])**2 / var) / \
                       np.sqrt(2 * np.pi * var)
        return probs
    
    def _update_parameters(self, returns: np.ndarray, alpha: np.ndarray, beta: np.ndarray):
        ...
        for i in range(self.n_states):
            ...
            if weight_sum > 0:
                ...
                self.emission_vars[i] = np.sum(weights * (returns - self.emission_means[i])**2) / weight_sum
...
class RegimeEnsemble:
    ...
    def fit(self, returns: np.ndarray, prices: np.ndarray):
    ...
    def _combine_predictions(
        ...
        hmm_signal = hmm_regimes.get(hmm_state, 'sideways')
