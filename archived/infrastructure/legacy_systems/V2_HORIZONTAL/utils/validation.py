...
class DataFrameValidator:
    ...
    def validate_daily_bars(df: pd.DataFrame, symbol: str) -> None:
        ...
        # Check for required OHLC columns (case-insensitive)
        df_cols_lower = [col.lower() for col in df.columns]
        required_cols = ["open", "high", "low", "close"]
        
        # Check if we have the lowercase versions
        has_lowercase = all(col in df_cols_lower for col in required_cols)
        
        # Check if we have the uppercase versions  
        ...
        if not (has_lowercase or has_uppercase):
            ...
            if has_lowercase:
                present = [c for c in required_cols if c in df_cols_lower]
            else:
                present = [c for c in ["Open", "High", "Low", "Close"] if c in df.columns]
            
            if not present:
                raise ValueError(f"{symbol}: missing all price columns (need Open, High, Low, Close)")
            else:
                missing = set(["Open", "High", "Low", "Close"]) - set(df.columns)
                raise ValueError(f"{symbol}: missing columns {missing}")
        
        # Use the columns that are present
