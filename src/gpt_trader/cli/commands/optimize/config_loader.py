...
def resolve_optimize_preset_inheritance(raw_config: dict[str, Any]) -> dict[str, Any]:
    ...
    selection = OptimizePresetSelection(
        profile=_normalize_override_name(
            preset_container.get("profile") or raw_config.get("profile"), "profile"
        ),
        scenario=_normalize_override_name(
            preset_container.get("scenario") or raw_config.get("scenario"), "scenario"
        ),
    )
...
def parse_config(raw_config: dict[str, Any]) -> OptimizeCliConfig:
    ...
    missing_parameter_space = object()
    param_space_raw = raw_config.get("parameter_space", missing_parameter_space)
    if param_space_raw is missing_parameter_space:
        param_space_raw = {}
    elif not isinstance(param_space_raw, dict):
