...
class PortfolioValuationService:
    ...
    def __init__(
        ...
        logger.info(
            f"PortfolioValuationService initialized - mark staleness: {mark_staleness_seconds}s"
        )

    def update_account_data(self, balances: list[Balance], positions: list[Position]) -> None:
        """Update cached account data."""
        self._cached_balances = {b.asset: b for b in balances}
    ...
    def update_mark_prices(self, mark_prices: dict[str, Decimal]) -> None:
    ...
    def update_trade(
        ...
        logger.debug(
            f"Trade updated - {symbol} {side} {quantity} @ {price}, realized PnL: {pnl_result.get('realized_pnl', 0)}"
        )
    ...
    def get_equity_curve(self, hours_back: int = 24) -> list[dict[str, object]]:
    ...
    def get_daily_metrics(self) -> dict[str, float | int | str]:
        ...
        return cast(
            ...
            self.pnl_tracker.generate_daily_metrics(latest.total_equity_usd),
        )
...
async def create_portfolio_valuation_service(
    pnl_tracker: PnLTracker | None = None, **kwargs: Any
