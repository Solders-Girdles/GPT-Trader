...
def test_feature_selection_methods():
    ...
    for method in methods:
        ...
        results[method.value] = {
            ...
            'top_features': pipeline.selected_feature_names[:5] if pipeline.selected_feature_names else []
        }
...
def test_model_validation():
    ...
    for model_name, model in models.items():
        ...
        performance = pipeline.train_and_validate(
            type(model), X_train, y_train, X_test, y_test,
...
def test_pipeline_persistence():
    ...
    # Verify
    n_features_loaded = len(new_pipeline.selected_feature_names) if new_pipeline.selected_feature_names else 0
    
    logger.info(f"\nOriginal features: {n_features_original}")
    logger.info(f"Loaded features: {n_features_loaded}")
    logger.info(f"Pipeline persistence: {'✅' if n_features_original == n_features_loaded else '❌'}")
