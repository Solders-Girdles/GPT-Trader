...
def _validate_position_request(
    request: PositionSizeRequest, field_name: str
) -> PositionSizeRequest:
...
def _validate_kelly_safety(win_rate: float, avg_win: float, avg_loss: float) -> None:
    ...
    if win_rate < 0.01 or win_rate > 0.99:
        raise ValidationError(
            f"Win rate {win_rate} outside reasonable bounds [0.01, 0.99]", field="win_rate"
        )
    ...
    if expected_value <= 0:
        raise ValidationError(
            f"Strategy has negative expected value: {expected_value:.6f}", field="expected_value"
        )


@validate_inputs(request=_validate_position_request)
...
def _calculate_intelligent_size(request: PositionSizeRequest) -> PositionSizeResponse:
        ...
        try:
            ...
            if request.confidence >= request.risk_params.confidence_threshold:
                ...
                adjusted_size, conf_explanation = confidence_adjusted_size(
                    base_size, request.confidence, adj_params
                )
                ...
                base_size = 0.0
                warnings.append(
                    f"Confidence {request.confidence:.2f} below threshold {request.risk_params.confidence_threshold:.2f}"
                )
    ...
    if request.market_regime:
        try:
            ...
            adjusted_size, regime_explanation = regime_adjusted_size(
                base_size, request.market_regime, multipliers
            )
    ...
    if base_size < 0:
        raise ValidationError(
            "Position size cannot be negative", field="base_size", value=base_size
        )
    ...
    try:
        position_value, share_count = kelly_position_value(
            request.portfolio_value, base_size, request.current_price, request.risk_params
        )
    except Exception as e:
...
def calculate_portfolio_allocation(requests: list[PositionSizeRequest]) -> PositionSizingResult:
    ...
    requests[0].portfolio_value  # Assume same portfolio
    ...
    sum(resp.risk_pct for resp in individual_responses)
    ...
    if total_position_pct > max_portfolio_allocation:
        ...
        for resp in individual_responses:
            ...
            scaled_shares = int(
                scaled_value / (resp.recommended_value / max(1, resp.recommended_shares))
            )
    ...
    return PositionSizingResult(
        primary=primary, alternatives=individual_responses, portfolio_impact=portfolio_impact
    )
...
def _estimate_portfolio_risk(
    position_size_pct: float, avg_loss: float | None, volatility: float | None
) -> float:
