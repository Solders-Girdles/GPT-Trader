...
def build_bot(
    ...
    svc_registry = build_service_registry(config, registry)
    bot = PerpsBot(config, registry=svc_registry)
    # PerpsBot may have enriched the registry with lazily constructed services
    return bot, bot.registry


def bot_from_profile(profile: str, **overrides) -> tuple[PerpsBot, ServiceRegistry]:
