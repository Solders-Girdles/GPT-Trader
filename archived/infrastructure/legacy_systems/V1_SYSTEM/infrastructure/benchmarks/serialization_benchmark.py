...
# import pickle  # Excluded due to security vulnerabilities (CVE-2022-48560)
import joblib
import numpy as np
import pandas as pd
import psutil
...
class SerializationBenchmark:
    ...
    def generate_test_data(self, n_rows: int = 10000, n_cols: int = 20) -> pd.DataFrame:
    ...
    def measure_performance(self, func, *args, **kwargs) -> tuple[float, float]:
    ...
    def benchmark_csv(self, df: pd.DataFrame, filepath: Path) -> dict[str, float]:
        ...
        # Write benchmark
        write_time, write_mem, _ = self.measure_performance(df.to_csv, filepath)
    ...
    def benchmark_parquet(self, df: pd.DataFrame, filepath: Path) -> dict[str, float]:
        ...
        # Read benchmark
        read_time, read_mem, _ = self.measure_performance(pd.read_parquet, filepath)
        results["read_time"] = read_time
        results["read_memory"] = read_mem
    ...
    def benchmark_feather(self, df: pd.DataFrame, filepath: Path) -> dict[str, float]:
        ...
        # Write benchmark
        write_time, write_mem, _ = self.measure_performance(df_reset.to_feather, filepath)
        results["write_time"] = write_time
        results["write_memory"] = write_mem

        # Read benchmark
        read_time, read_mem, df_read = self.measure_performance(pd.read_feather, filepath)
        results["read_time"] = read_time
        results["read_memory"] = read_mem
    ...
    def benchmark_hdf5(self, df: pd.DataFrame, filepath: Path) -> dict[str, float]:
        ...
        # Read benchmark
        read_time, read_mem, _ = self.measure_performance(pd.read_hdf, filepath, key="data")
        results["read_time"] = read_time
        results["read_memory"] = read_mem
    ...
    def benchmark_pickle(self, df: pd.DataFrame, filepath: Path) -> dict[str, float]:
        """Pickle format benchmark - DISABLED for security reasons.

        Pickle has been removed due to security vulnerabilities:
        - CVE-2022-48560: Arbitrary code execution during deserialization
        - Use joblib for sklearn models instead
        - Use parquet/feather for DataFrames instead
        """
        return {
            "write_time": 0.0,
            "write_memory": 0.0,
    ...
    def benchmark_joblib(self, df: pd.DataFrame, filepath: Path) -> dict[str, float]:
        ...
        # Write benchmark
        write_time, write_mem, _ = self.measure_performance(joblib.dump, df, filepath, compress=3)
        results["write_time"] = write_time
        results["write_memory"] = write_mem

        # Read benchmark
        read_time, read_mem, _ = self.measure_performance(joblib.load, filepath)
        results["read_time"] = read_time
        results["read_memory"] = read_mem
    ...
    def benchmark_json(self, df: pd.DataFrame, filepath: Path) -> dict[str, float]:
        ...
        # Read benchmark
        read_time, read_mem, _ = self.measure_performance(pd.read_json, filepath, orient="split")
        results["read_time"] = read_time
        results["read_memory"] = read_mem
    ...
    def run_benchmark(self, data_sizes: list = None, formats: list = None) -> pd.DataFrame:
        ...
        for n_rows, n_cols in data_sizes:
            ...
            for format_name in formats:
                ...
                try:
                    ...
                    print(
                        f"✓ (Total: {metrics['total_time']:.3f}s, Size: {metrics['file_size_mb']:.1f}MB)"
                    )
