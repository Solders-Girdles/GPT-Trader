...
class ModelCalibrator:
    ...
    def calibrate(
        ...
        self.calibration_metrics = self._evaluate_calibration(y_val, base_probs, calibrated_probs)

        # Optimize threshold if requested
        if self.config.optimize_threshold:
            self.threshold_result = self._optimize_threshold(y_val, calibrated_probs)
            self.optimal_threshold = self.threshold_result.optimal_threshold

        # Estimate confidence intervals
        self.confidence_intervals = self._estimate_confidence_intervals(X_val, y_val)

        logger.info(
            f"Calibration complete. ECE: {self.calibration_metrics.ece:.4f}, "
    ...
    def _isotonic_calibration(
        ...
        calibrated = CalibratedClassifierCV(model, method="isotonic", cv=self.config.cv_folds)
        calibrated.fit(X, y)
        return calibrated
    ...
    def _sigmoid_calibration(
        ...
        calibrated = CalibratedClassifierCV(model, method="sigmoid", cv=self.config.cv_folds)
        calibrated.fit(X, y)
        return calibrated
    ...
    def _evaluate_calibration(
        self, y_true: np.ndarray, base_probs: np.ndarray, calibrated_probs: np.ndarray
    ) -> CalibrationMetrics:
        """Evaluate calibration quality"""
    ...
    def _optimize_threshold(
        ...
        for threshold in thresholds:
            ...
            profit = np.where(y_pred == 1, np.where(y_true == 1, 0.02, -0.01), 0).sum()
    ...
    def calculate_position_size(self, probability: float, current_capital: float = 100000) -> float:
        """
        ...
        position_fraction = min(max(safe_kelly, 0), self.config.max_position_size)
...
"""
        ...
        if self.confidence_intervals:
            ...
            errors = [
                (
                    centers[i] - self.confidence_intervals[m][0],
                    self.confidence_intervals[m][1] - centers[i],
                )
                for i, m in enumerate(metrics)
            ]

            errors = list(zip(*errors, strict=False))
    ...
    def _plot_reliability_diagram(self, y_true, y_prob_uncal, y_prob_cal, ax):
        ...
        if rel_uncal["bin_centers"]:
            ax.plot(
                rel_uncal["bin_confidences"],
                rel_uncal["bin_accuracies"],
                "o-",
                color="blue",
                label="Uncalibrated",
                markersize=8,
        ...
        if rel_cal["bin_centers"]:
            ax.plot(
                rel_cal["bin_confidences"],
                rel_cal["bin_accuracies"],
                "s-",
                color="green",
                label="Calibrated",
                markersize=8,
    ...
    def save_calibration_state(self, filepath: str):
        ...
        state = {
            ...
            "optimal_threshold": self.optimal_threshold,
            "calibration_metrics": (
                self.calibration_metrics.to_dict() if self.calibration_metrics else None
            ),
            "confidence_intervals": self.confidence_intervals,
            "threshold_result": (
                self.threshold_result.to_dict() if hasattr(self, "threshold_result") else None
            ),
...
def create_model_calibrator(config: CalibrationConfig | None = None) -> ModelCalibrator:
...
if __name__ == "__main__":
    ...
    # Plot diagnostics
    fig = calibrator.plot_calibration_diagnostic(y_test.values, y_prob_uncal, y_prob_cal)
    plt.show()
