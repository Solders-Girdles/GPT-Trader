...
from bot_v2.features.brokerages.core.interfaces import (
    IBrokerage, Product, MarketType, Quote, Order, OrderSide, OrderType, TimeInForce, OrderStatus, Balance, Position
)
...
class SimpleBroker(IBrokerage):
    ...
    def get_product(self, symbol: str) -> Product:
        return Product(symbol=symbol, base_asset='BTC', quote_asset='USD', market_type=MarketType.PERPETUAL, min_size=Decimal('0.001'), step_size=Decimal('0.001'), min_notional=Decimal('10'), price_increment=Decimal('0.01'))

    def get_quote(self, symbol: str) -> Quote:
        return Quote(symbol=symbol, bid=Decimal('100'), ask=Decimal('100.10'), last=Decimal('100.05'), ts=datetime.utcnow())

    def place_order(self, symbol: str, side: OrderSide, order_type: OrderType, qty: Decimal, price: Optional[Decimal] = None, stop_price: Optional[Decimal] = None, tif: TimeInForce = TimeInForce.GTC, client_id: Optional[str] = None, reduce_only: Optional[bool] = None, leverage: Optional[int] = None) -> Order:
        now = datetime.utcnow()
        oid = client_id or f"ord_{len(self._orders)+1}"
        o = Order(id=oid, client_id=oid, symbol=symbol, side=side, type=order_type, qty=qty, price=price, stop_price=stop_price, tif=tif, status=OrderStatus.FILLED, filled_qty=qty, avg_fill_price=price or Decimal('100.10') if side == OrderSide.BUY else Decimal('100'), submitted_at=now, updated_at=now)
        self._orders[oid] = o
        return o
    ...
    def list_orders(self, status: Optional[OrderStatus] = None, symbol: Optional[str] = None):
        return list(self._orders.values())

    def list_products(self, market: Optional[MarketType] = None):  # pragma: no cover
        return []
...
def test_slippage_multiplier_path_and_cancel_all_orders():
    broker = SimpleBroker()
    risk_cfg = RiskConfig(slippage_guard_bps=10000, max_position_pct_per_symbol=0.9, max_exposure_pct=0.9)  # disable slippage guard
    risk = LiveRiskManager(config=risk_cfg)
    engine = LiveExecutionEngine(broker, risk_manager=risk, event_store=None, bot_id='test', slippage_multipliers={'BTC-PERP': 0.001})

    oid = engine.place_order(symbol='BTC-PERP', side=OrderSide.BUY, order_type=OrderType.MARKET, qty=Decimal('0.2'))
