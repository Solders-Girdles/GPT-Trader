...
class AlertDeduplicator:
    ...
    def is_duplicate(self, alert: Alert) -> Tuple[bool, Optional[Alert]]:
        """
...
class AlertFatiguePrevention:
    ...
    def _exceeds_rate_limit(self, alert: Alert) -> bool:
        ...
        self.alert_rate[category_key] = [
            t for t in self.alert_rate[category_key]
            if t > hour_ago
        ]
    ...
    def _calculate_noise_level(self) -> float:
        ...
        # Calculate metrics
        low_priority_ratio = sum(
            1 for a in recent_alerts if a.priority_score < 30
        ) / len(recent_alerts)
        
        duplicate_ratio = sum(
            1 for a in recent_alerts if a.occurrence_count > 1
        ) / len(recent_alerts)
        
        info_ratio = sum(
            1 for a in recent_alerts if a.severity == AlertSeverity.INFO
        ) / len(recent_alerts)
        
        # Combine metrics
        noise_level = (low_priority_ratio * 0.4 +
                      duplicate_ratio * 0.3 +
                      info_ratio * 0.3)
...
class IntelligentAlertSystem:
    ...
    def get_summary(self) -> str:
        ...
        summary.append(f"Suppressed: {stats['suppressed_alerts']} "
                      f"({stats.get('suppression_rate', 0):.1%})")
        summary.append(f"Deduplicated: {stats['deduplicated_alerts']} "
                      f"({stats.get('deduplication_rate', 0):.1%})")
        
        summary.append("\nTop Active Alerts:")
        for alert in active_alerts[:5]:
            summary.append(f"  - [{alert.severity.value}] {alert.title} "
