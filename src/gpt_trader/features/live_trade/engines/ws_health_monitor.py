"""WebSocket health watchdog for the live trading engine."""
...
async def monitor_ws_health(engine: Any) -> None:
    """Monitor WebSocket health and trigger degradation on staleness.

    Periodically polls WS health metrics from the broker. If messages or
    heartbeats are stale beyond configured thresholds, triggers reduce-only mode,
    symbol pause, and notification alerts. On reconnect, pauses briefly to allow
