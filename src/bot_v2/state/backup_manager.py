...
class StorageTier(Enum):
    ...
    LOCAL = "local"      # Fast local disk
    NETWORK = "network"  # Network attached storage
    CLOUD = "cloud"      # S3 or similar
    ARCHIVE = "archive"  # Glacier or tape
...
class BackupMetadata:
    ...
    backup_duration_seconds: float = 0
...
class BackupConfig:
    ...
    network_storage_path: Optional[str] = None
    s3_bucket: Optional[str] = "bot-v2-backups"
...
class BackupManager:
    ...
    def __init__(self, state_manager: Any, config: Optional[BackupConfig] = None):
        ...
        self._last_differential_backup: Optional[datetime] = None
        ...
        logger.info(f"BackupManager initialized with {len(self._backup_history)} backups in history")
    ...
    async def start_scheduled_backups(self):
        ...
        self._scheduled_tasks.append(
            asyncio.create_task(self._run_full_backup_schedule())
        )
        
        # Schedule differential backups
        self._scheduled_tasks.append(
            asyncio.create_task(self._run_differential_backup_schedule())
        )
        
        # Schedule incremental backups
        self._scheduled_tasks.append(
            asyncio.create_task(self._run_incremental_backup_schedule())
        )
        
        # Schedule cleanup
        self._scheduled_tasks.append(
            asyncio.create_task(self._run_cleanup_schedule())
        )
        
        # Schedule verification
        self._scheduled_tasks.append(
            asyncio.create_task(self._run_verification_schedule())
        )
    ...
    async def _run_full_backup_schedule(self):
    ...
    async def _run_differential_backup_schedule(self):
    ...
    async def _run_incremental_backup_schedule(self):
    ...
    async def create_backup(self, backup_type: BackupType = BackupType.FULL) -> Optional[BackupMetadata]:
        ...
        with self._backup_lock:
            ...
            try:
                ...
                # Serialize data
                serialized = json.dumps(backup_data, default=str)
                ...
                # Store backup
                storage_path = await self._store_backup(
                    backup_id, data_to_store, storage_tier
                )
                ...
                metadata = BackupMetadata(
                    ...
                    backup_duration_seconds=(datetime.utcnow() - start_time).total_seconds(),
                    data_sources=list(backup_data.keys())
                )
                
                # Verify if enabled
                if self.config.verify_after_backup:
                    if await self._verify_backup(metadata, data_to_store):
                ...
                logger.info(f"Backup {backup_id} completed in "
    ...
    async def restore_from_backup(self, backup_id: str) -> bool:
        ...
        try:
            ...
            # Parse backup data
            restored_data = json.loads(backup_data.decode() if isinstance(backup_data, bytes) else backup_data)
    ...
    async def restore_latest_backup(self, backup_type: Optional[BackupType] = None) -> bool:
        """
        ...
        """
        # Find latest backup
        backups = [b for b in self._backup_history 
    ...
    async def _store_backup(self, backup_id: str, data: bytes, 
                          storage_tier: StorageTier) -> str:
    ...
    async def _retrieve_backup(self, metadata: BackupMetadata) -> Optional[bytes]:
        """Retrieve backup data"""
        if metadata.storage_tier == StorageTier.LOCAL:
            ...
            if self.s3_client:
                try:
                    prefix = "archive" if metadata.storage_tier == StorageTier.ARCHIVE else "backups"
    ...
    async def _run_verification_schedule(self):
    ...
    def _get_last_backup_time(self) -> datetime:
        ...
        successful = [b for b in self._backup_history 
                     if b.status in [BackupStatus.COMPLETED, BackupStatus.VERIFIED]]
    ...
    def get_backup_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_full_backup': self._last_full_backup.isoformat() if self._last_full_backup else None,
...
async def create_backup(state_manager: Any, backup_type: BackupType = BackupType.FULL) -> Optional[BackupMetadata]:
    """Create a backup"""
