...
class RiskConfig:
    ...
    daytime_start_utc: Optional[str] = None  # e.g., "09:00"
    daytime_end_utc: Optional[str] = None    # e.g., "17:00"
    ...
    max_total_exposure_pct: Optional[float] = None  # alias for max_exposure_pct
    max_position_usd: Optional[Decimal] = None      # deprecated, unused
    max_daily_loss_pct: Optional[float] = None      # deprecated, unused
    ...
    def from_json(cls, path: str) -> RiskConfig:
        ...
        if 'max_intraday_volatility_threshold' in raw:
            data['max_intraday_volatility_threshold'] = float(raw['max_intraday_volatility_threshold'])
        ...
        if 'volatility_reduce_only_threshold' in raw:
            data['volatility_reduce_only_threshold'] = float(raw['volatility_reduce_only_threshold'])
        if 'volatility_kill_switch_threshold' in raw:
            data['volatility_kill_switch_threshold'] = float(raw['volatility_kill_switch_threshold'])

        # Mappings
        if 'leverage_max_per_symbol' in raw and isinstance(raw['leverage_max_per_symbol'], dict):
            data['leverage_max_per_symbol'] = {str(k): int(v) for k, v in raw['leverage_max_per_symbol'].items()}
        if 'max_notional_per_symbol' in raw and isinstance(raw['max_notional_per_symbol'], dict):
            data['max_notional_per_symbol'] = {str(k): Decimal(str(v)) for k, v in raw['max_notional_per_symbol'].items()}
        # Optional day/night schedule
        if 'daytime_start_utc' in raw:
            data['daytime_start_utc'] = str(raw['daytime_start_utc']) if raw['daytime_start_utc'] else None
        if 'daytime_end_utc' in raw:
            data['daytime_end_utc'] = str(raw['daytime_end_utc']) if raw['daytime_end_utc'] else None
        if 'day_leverage_max_per_symbol' in raw and isinstance(raw['day_leverage_max_per_symbol'], dict):
            data['day_leverage_max_per_symbol'] = {str(k): int(v) for k, v in raw['day_leverage_max_per_symbol'].items()}
        if 'night_leverage_max_per_symbol' in raw and isinstance(raw['night_leverage_max_per_symbol'], dict):
            data['night_leverage_max_per_symbol'] = {str(k): int(v) for k, v in raw['night_leverage_max_per_symbol'].items()}
        if 'day_mmr_per_symbol' in raw and isinstance(raw['day_mmr_per_symbol'], dict):
            data['day_mmr_per_symbol'] = {str(k): float(v) for k, v in raw['day_mmr_per_symbol'].items()}
        if 'night_mmr_per_symbol' in raw and isinstance(raw['night_mmr_per_symbol'], dict):
            data['night_mmr_per_symbol'] = {str(k): float(v) for k, v in raw['night_mmr_per_symbol'].items()}
