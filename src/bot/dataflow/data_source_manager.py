...
class DataSourceManager:
    ...
    async def fetch_market_data(self, symbol: str, 
                               timeout: Optional[float] = None) -> Optional[MarketData]:
        """
    ...
    async def _fetch_from_source(self, source: DataSource, 
                                symbol: str,
                                timeout: Optional[float] = None) -> Optional[MarketData]:
    ...
    async def _fetch_alpaca_quote(self, symbol: str, 
                                 timeout: Optional[float] = None) -> Optional[MarketData]:
    ...
    async def _fetch_polygon_quote(self, symbol: str,
                                  timeout: Optional[float] = None) -> Optional[MarketData]:
    ...
    async def _fetch_iex_quote(self, symbol: str,
                              timeout: Optional[float] = None) -> Optional[MarketData]:
    ...
    async def _fetch_yahoo_historical(self, symbol: str,
                                     start_date: datetime,
                                     end_date: datetime) -> Optional[pd.DataFrame]:
    ...
    async def _fetch_alpaca_historical(self, symbol: str,
                                      start_date: datetime,
                                      end_date: datetime) -> Optional[pd.DataFrame]:
    ...
    async def _fetch_polygon_historical(self, symbol: str,
                                       start_date: datetime,
                                       end_date: datetime) -> Optional[pd.DataFrame]:
    ...
    def _update_source_error(self, source: DataSource, error: str):
        ...
        if status.consecutive_errors >= self.config.failover_threshold:
            if status.is_active:
                logger.warning(f"Deactivating {source.value} after {status.consecutive_errors} errors")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'source_quality': {
                source.value: status.quality.value
