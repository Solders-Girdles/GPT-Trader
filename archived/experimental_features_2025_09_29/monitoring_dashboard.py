...
class MonitoringDashboard:
    ...
    def __init__(self, history_size: int = 1000) -> None:
        ...
        self.slice_status: dict[str, dict[str, Any]] = {}
    ...
    def update_slice_status(
        self, slice_name: str, status: str, details: dict[str, Any] | None = None
    ) -> None:
    ...
    def get_slice_metrics(self, slice_name: str, minutes: int = 15) -> list[dict[str, Any]]:
        """Get metrics for a specific slice over time period."""
        cutoff = datetime.now() - timedelta(minutes=minutes)
        slice_metrics = []

        for snapshot in self.metrics_history:
            if snapshot.timestamp > cutoff and snapshot.tags.get("slice") == slice_name:
        ...
        def monitor_resources() -> None:
            while self._resource_monitor_active:
                try:
                    ...
                    self.record_metric(
                        "system.memory_percent", memory.percent, {"type": "resource"}
                    )
    ...
    def _calculate_slice_health(self) -> dict[str, dict[str, Any]]:
        ...
        for slice_name, status in self.slice_status.items():
            score = (
                ...
                else 0.5 if status["status"] == "degraded" else 0.0
            )
    ...
    def _get_active_workflows(self) -> list[dict[str, Any]]:
        ...
        for wf_id, status in self.workflow_status.items():
            if status["updated_at"] > cutoff and status["status"] not in [
                "completed",
                "failed",
                "cancelled",
            ]:
    ...
    def generate_summary(self) -> str:
        ...
        for workflow in data["active_workflows"]:
            summary.append(
                f"  - {workflow['id']}: {workflow['status']} ({workflow['progress']:.0%})"
            )
        ...
        for slice_name, health in data["slice_health"].items():
            status_icon = (
                "✅" if health["score"] == 1.0 else "⚠️" if health["score"] == 0.5 else "❌"
            )
            summary.append(
                f"  {status_icon} {slice_name}: {health['status']} ({health['score']:.0%})"
            )
...
def update_slice_status(
    slice_name: str, status: str, details: dict[str, Any] | None = None
) -> None:
    """Convenience function to update slice status."""
    get_dashboard().update_slice_status(slice_name, status, details)


def track_workflow(workflow_id: str, status: str, progress: float = 0.0) -> None:
