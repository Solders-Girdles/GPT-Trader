...
class TestBacktestIntegration:
    ...
    def test_data_pipeline_integration(self, backtest_engine):
        ...
        with patch('src.bot.dataflow.sources.yfinance_source.YFinanceSource.fetch_data') as mock_fetch:
            # Setup mock data
            ...
            for symbol in backtest_engine.universe:
                ...
                assert all(col in data[symbol].columns for col in ["Open", "High", "Low", "Close", "Volume"])
    ...
    def test_portfolio_allocation_integration(self, mock_market_data):
        ...
        # Allocate capital
        allocations = allocator.signal_weighted_allocation(
        ...
        assert all(
            alloc >= allocator.min_position_size or alloc == 0
    ...
    def test_large_universe_backtest(self, backtest_config):
        ...
        for symbol in large_universe:
            ...
            large_data[symbol] = pd.DataFrame(
                {"Close": prices, "Volume": np.random.uniform(1e5, 1e7, len(dates))},
                index=dates
            )
