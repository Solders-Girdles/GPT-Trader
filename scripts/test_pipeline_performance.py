...
class PipelinePerformanceTester:
    ...
    async def test_data_source_latency(self):
        ...
        for source in sources:
            ...
            if latencies:
                ...
                logger.warning(f"    No successful requests")
    ...
    async def test_failover_speed(self):
        ...
        if data:
            ...
            logger.error(f"  ❌ Failover failed")
            failover_time = None
    ...
    async def test_memory_usage(self):
        ...
        from src.bot.dataflow.realtime_feed import RealtimeDataFeed, DataFeedConfig
    ...
    async def run_performance_tests(self):
        ...
        if all_results.get('source_latency'):
            ...
            if primary_latency and primary_latency < 100:
                requirements.append(f"✅ Primary source P95 latency: {primary_latency:.2f}ms (< 100ms)")
        ...
        if all_results.get('failover_time_ms'):
            if all_results['failover_time_ms'] < 1000:
                requirements.append(f"✅ Failover time: {all_results['failover_time_ms']:.2f}ms (< 1000ms)")
            else:
                requirements.append(f"❌ Failover time: {all_results['failover_time_ms']:.2f}ms (>= 1000ms)")
                meets_requirements = False
        ...
        if all_results.get('validation') and all_results['validation'].get('throughput'):
            ...
            if val_throughput >= 10000:
                requirements.append(f"✅ Validation throughput: {val_throughput:.0f} msg/s (>= 10000)")
            else:
                requirements.append(f"⚠️ Validation throughput: {val_throughput:.0f} msg/s (< 10000)")
