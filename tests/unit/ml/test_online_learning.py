...
from src.bot.ml.online_learning import (
    OnlineLearningPipeline, OnlineLearningConfig, LearningMode,
    UpdateStrategy, MemorySample, create_online_learning_pipeline,
    CONSERVATIVE_CONFIG, AGGRESSIVE_CONFIG, DRIFT_ADAPTIVE_CONFIG
)
...
class TestOnlineLearningPipeline:
    ...
    def test_model_info(self):
        ...
        required_keys = [
            'is_initialized', 'sample_count', 'memory_buffer_size',
            'current_learning_rate', 'scheduler_stats', 'drift_detector_stats'
        ]
    ...
    def test_state_persistence(self):
        ...
        try:
            ...
            # Cleanup
            for ext in ['.json', '.model.joblib', '.backup.joblib', '.scaler.joblib', '.encoder.joblib']:
                try:
...
class TestIntegration:
    ...
    def test_full_pipeline_integration(self):
        ...
        performance_before_drift = np.mean(list(pipeline.performance_history)[-10:]) if pipeline.performance_history else 0.5
