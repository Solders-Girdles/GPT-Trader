...
class IsolationForestDetector:
    ...
    def fit(self, data: pd.DataFrame, feature_columns: Optional[List[str]] = None):
        """
        ...
        if len(data) < self.config.min_samples:
            logger.warning(f"Insufficient data for training: {len(data)} < {self.config.min_samples}")
            return
...
class StatisticalAnomalyDetector:
    ...
    def detect_ewma(self, metric_name: str, value: float) -> Optional[Anomaly]:
        """
    ...
    def detect_cusum(self, metric_name: str, value: float) -> Optional[Anomaly]:
        """
    ...
    def detect_mad(self, metric_name: str, value: float) -> Optional[Anomaly]:
        """
...
class MarketMicrostructureAnalyzer:
    ...
    def analyze_spread(self, bid: float, ask: float) -> Optional[Anomaly]:
        """
    ...
    def analyze_volume(self, volume: float, avg_volume: float) -> Optional[Anomaly]:
        """
    ...
    def analyze_order_imbalance(self, buy_volume: float, sell_volume: float) -> Optional[Anomaly]:
        """
...
class AnomalyDetectionSystem:
    ...
    def detect(self,
              ...
              market_data: Optional[Dict[str, float]] = None) -> List[Anomaly]:
        """
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        method_counts = {}
        for anomaly in self.anomaly_history:
            method_counts[anomaly.detection_method.value] = method_counts.get(anomaly.detection_method.value, 0) + 1
    ...
    def save_model(self, filepath: str):
        ...
        model_data = {
            'isolation_forest': self.isolation_forest.model if self.isolation_forest.is_fitted else None,
            'scaler': self.isolation_forest.scaler if self.isolation_forest.is_fitted else None,
            'config': self.config
        }
    ...
    def load_model(self, filepath: str):
        ...
        if model_data.get('isolation_forest'):
            self.isolation_forest.model = model_data['isolation_forest']
            self.isolation_forest.scaler = model_data['scaler']
