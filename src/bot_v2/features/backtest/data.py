...
from ...validation import validate_inputs, SymbolValidator, DateValidator
...
def get_data_quality_report(data: pd.DataFrame, symbol: str) -> dict:
    ...
    try:
        ...
        if data.isnull().any().any():
            report['data_quality_issues'].append('null_values_present')
        
        if len(data) > 1:
            # Check for duplicate dates
            if data.index.duplicated().any():
                report['data_quality_issues'].append('duplicate_dates')
            
            # Check for extreme returns
            returns = data['close'].pct_change().abs()
            if (returns > 0.5).any():
                report['data_quality_issues'].append('extreme_returns')
