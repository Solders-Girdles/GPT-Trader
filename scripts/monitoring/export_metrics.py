...
def _extract_limit_remaining(limits: Dict[str, Any]) -> float:
    if not isinstance(limits, dict):
        return 0.0
    for key in ('remaining', 'remaining_balance', 'available', 'max_order', 'available_trading_balance'):
...
def render_prometheus(metrics: Dict[str, Any], events_path: Path) -> str:
    ...
    lines.append('# HELP gpt_trader_order_preview_failures_total Total number of order preview failures observed')
...
def main():
    ...
    parser.add_argument('--metrics-file', default='data/perps_bot/prod/metrics.json', help='Path to metrics.json')
    parser.add_argument('--events-file', default='results/managed/events.jsonl', help='Path to events log')
