...
class ABTestConfig:
    ...
    # Segmentation
    segment_by: Optional[List[str]] = None  # Features to segment by
...
class ABTestingFramework:
    ...
    def create_test(self, config: ABTestConfig) -> str:
        ...
        logger.info(f"Created A/B test {config.test_id}: {config.model_a_id} vs {config.model_b_id}")
    ...
    def assign_variant(self, test_id: str, features: Optional[pd.DataFrame] = None) -> str:
        """
    ...
    def get_current_results(self, test_id: str) -> Optional[ABTestResult]:
        """
        ...
        statistic, p_value = stats.ttest_ind(
            test['metrics_a'][config.metric],
            test['metrics_b'][config.metric]
        )
        ...
        effect_size = abs(difference) / np.sqrt(
            (np.var(test['metrics_a'][config.metric]) + 
             np.var(test['metrics_b'][config.metric])) / 2
        )
    ...
    def _check_early_stopping(self, test_id: str) -> bool:
        ...
        if config.sequential_testing:
            ...
            if var_a > 0 and var_b > 0:
                ...
                llr = sum(
                    np.log(stats.norm.pdf(x, mean_b, np.sqrt(var_b)) / 
    ...
    def get_test_summary(self, test_id: str) -> Dict[str, Any]:
        ...
        if test_id in self.active_tests:
            ...
            summary = {
                ...
                'models': {
                    'A': test['config'].model_a_id,
                    'B': test['config'].model_b_id
                },
                'samples': {
...
def create_ab_test(model_a_id: str,
                   model_b_id: str,
                   metric: str = "accuracy",
                   **kwargs) -> Tuple[ABTestingFramework, str]:
    """
