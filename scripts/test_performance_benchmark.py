...
from src.bot.ml.performance_benchmark import (
    create_benchmark, BenchmarkConfig
)
...
def test_ml_vs_baseline():
    ...
    all_models = {
        'XGBoost': ml_model,
        'EnsembleBaseline': baseline_model
    }
    ...
    benchmark = create_benchmark(config)
    results = benchmark.run_comprehensive_benchmark(
        all_models, X[:2000], y[:2000], prices[:2000]
    )
