...
class LiveCommand(BaseCommand):
    ...
    def add_parser(cls, subparsers: argparse._SubParsersAction) -> None:
        parser = subparsers.add_parser(
            ...
            description='⚠️  WARNING: Live trading uses real money!'
        )
    ...
    def execute(self) -> int:
        ...
        try:
            ...
            monitor = TradingMonitor(
                refresh_interval=self.args.refresh,
    ...
    def execute(self) -> int:
        ...
        try:
            ...
            cmd = [
                ...
                str(dashboard_path),
                '--server.port', str(self.args.port),
                '--server.address', self.args.host,
            ]
...
class WizardCommand(BaseCommand):
    ...
    def _configure_risk(self) -> Dict[str, Any]:
        ...
        config['risk_per_trade'] = float(
            input("\nRisk per trade (%) [1.0]: ").strip() or '1.0'
        )
        
        config['max_positions'] = int(
            input("Maximum concurrent positions [10]: ").strip() or '10'
        )
        
        config['stop_loss'] = float(
            input("Stop loss (%) [2.0]: ").strip() or '2.0'
        )
