...
alerts generated by GPT-Trader during monitored, readiness-gated runs.

## Pre-Migration And Live-Operation Scope

Use these runbooks under the
[Pre-Migration Decision Framework](PRE_MIGRATION_DECISION_FRAMEWORK.md) and the
...
that touch broker/account data, live product metadata, process state, canary or
production profiles, positions, funds, or order flow are live-operation commands.
Run them only inside a current approved runbook or decision packet that names
the lane, constraints, verification boundary, and rollback or kill-switch
expectations. If the current packet does not explicitly cover a command lane,
stop at local diagnostics and route the gap through `decision-needed`.
...
Steps 1, 2, and 4 are local read-only diagnostics. Step 3 calls a public
Coinbase endpoint and is allowed only when the active runbook or decision packet
includes live broker/API connectivity verification.
...
Restarting a live monitored run is a live-operation action. Use the restart
procedure only under a packet or approved runbook that names the affected lane,
the process boundary, verification checks, and rollback or kill-switch plan.
...
These checks are read-only when they inspect local `status.json` and local
configuration. Do not change collateral, risk budgets, or live profile settings
from this runbook unless the approved packet scopes that action.
...
# 3. Review selected profile risk settings through the canonical loader
uv run python - <<'PY'
...
- Money movement, collateral changes, and risk-budget changes require explicit
  human approval plus an approved runbook or decision packet for that lane.
...
The local order database check is read-only. Account snapshots and product
catalog probes touch live broker/account or product-capability surfaces and are
allowed only inside an approved runbook or decision packet that names the
profile, product lane, account boundary, and rollback or kill-switch
expectations. The Coinbase status check is public service-health evidence, not
account-capability proof.
...
This runbook does not authorize order submission, retrying live orders outside
the active runtime policy, credential reads, or money movement. If remediation
requires any of those actions, record the incident evidence and route through
the current approved runbook or a new `decision-needed` packet.
...
Stopping or starting `gpt-trader` changes live process state. Use this
procedure only when the active incident packet names the process lane,
constraints, verification boundary, and rollback or kill-switch expectations.
If a live trading profile was active, default to stop, observe, and reconcile
before any restart.
...
Steps 1 and 2 are local read-only diagnostics. Step 3 calls a live market-data
endpoint and is allowed only when the current runbook or decision packet scopes
that broker/API verification lane.
...
session. Position recovery is a live-operation lane: the approved packet must
name the profile, product lane, account verification boundary, operator
decision record, and rollback or kill-switch expectation. Restart is an
operator decision, not an automatic step.

1. **Check positions**: `cat status.json | jq '.positions'`
2. **Verify with broker**: `gpt-trader account snapshot --profile <profile>`
   only under an approved account-snapshot lane.
