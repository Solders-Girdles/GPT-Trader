...
def identify_transition_triggers(from_regime: MarketRegime, to_regime: MarketRegime) -> list[str]:
    """
    ...
    if "QUIET" in from_regime.value and "VOLATILE" in to_regime.value:
        triggers.extend(["Volatility spike", "News event impact", "Market uncertainty increase"])

    # From volatile to quiet regimes
    elif "VOLATILE" in from_regime.value and "QUIET" in to_regime.value:
        triggers.extend(
            ["Volatility normalization", "Market stabilization", "Uncertainty resolution"]
        )
    ...
    # To crisis regime
        ...
        triggers.extend(
            ["Market crash", "Systemic risk event", "Liquidity crisis", "Black swan event"]
        )
    ...
    # Sideways transitions
        ...
        triggers.extend(["Trend exhaustion", "Consolidation phase", "Range establishment"])

    return triggers


def calculate_transition_statistics(transitions: list[RegimeTransition]) -> dict[str, any]:
    """
...
def predict_next_regime(
    ...
    adjusted_probs = adjust_probabilities_by_duration(
        adjusted_probs, current_duration, current_regime
    )
...
def adjust_probabilities_by_signals(
    base_probs: dict[MarketRegime, float], signals: dict[str, float]
...
def adjust_probabilities_by_duration(
    probs: dict[MarketRegime, float], current_duration: int, current_regime: MarketRegime
) -> dict[MarketRegime, float]:
    """
...
def find_regime_patterns(regimes: list[tuple[MarketRegime, datetime, datetime]]) -> dict[str, any]:
    """
