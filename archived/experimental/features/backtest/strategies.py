    ...
    def _validate_parameters(self) -> None:
        ...
        if self.fast_period <= 0:
            raise ValidationError(
                "Fast period must be positive", field="fast_period", value=self.fast_period
            )

        if self.slow_period <= 0:
            raise ValidationError(
                "Slow period must be positive", field="slow_period", value=self.slow_period
            )
    ...
    def run(self, data: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            if isinstance(e, StrategyError):
                raise
            raise StrategyError(
                "Failed to generate MA crossover signals",
                strategy_name=self.__class__.__name__,
                context={
                    "fast_period": self.fast_period,
                    "slow_period": self.slow_period,
...
class MomentumStrategy(BacktestStrategy):
    ...
    def __init__(self, lookback: int = 20, threshold: float = 0.02, **kwargs: Any) -> None:
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.lookback <= 0:
            raise ValidationError(
                "Lookback period must be positive", field="lookback", value=self.lookback
            )

        if not 0 < self.threshold <= 1.0:
            raise ValidationError(
                "Threshold must be between 0 and 1", field="threshold", value=self.threshold
            )
    ...
    def run(self, data: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            if isinstance(e, StrategyError):
                raise
            raise StrategyError(
                "Failed to generate momentum signals",
                strategy_name=self.__class__.__name__,
                context={
...
class MeanReversionStrategy(BacktestStrategy):
    ...
    def __init__(self, period: int = 20, num_std: float = 2.0, **kwargs: Any) -> None:
        self.period = period
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.period <= 0:
            raise ValidationError("Period must be positive", field="period", value=self.period)
    ...
    def run(self, data: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            if isinstance(e, StrategyError):
                raise
            raise StrategyError(
                "Failed to generate mean reversion signals",
                strategy_name=self.__class__.__name__,
                context={
                    "period": self.period,
...
class VolatilityStrategy(BacktestStrategy):
    ...
    def __init__(self, period: int = 20, vol_threshold: float = 0.02, **kwargs: Any) -> None:
        self.period = period
    ...
    def get_required_periods(self) -> int:
        """Need period + 1 for volatility calculation."""
    ...
    def run(self, data: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            if isinstance(e, StrategyError):
                raise
            raise StrategyError(
                "Failed to generate breakout signals",
                strategy_name=self.__class__.__name__,
                context={"lookback": self.lookback, "data_shape": data.shape, "error": str(e)},
            ) from e
...
def create_local_strategy(name: str, **params: Any) -> BacktestStrategy:
    """
    ...
    if not name or not isinstance(name, str):
        raise ValidationError(
            "Strategy name must be a non-empty string", field="strategy_name", value=name
        )
    ...
    if name not in STRATEGY_MAP:
        raise StrategyError(
            ...
            strategy_name=name,
            context={"available_strategies": list(STRATEGY_MAP.keys()), "requested_strategy": name},
        )
...
def list_local_strategies() -> list[str]:
...
def get_strategy_info(strategy_name: str) -> dict[str, Any]:
    ...
    if strategy_name not in STRATEGY_MAP:
        return {"name": strategy_name, "exists": False, "error": "Strategy not found"}
