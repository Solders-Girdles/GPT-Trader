...
from bot_v2.features.brokerages.coinbase.models import APIConfig, Product
...
class TestCoinbaseRestContractSuite:
    ...
    def test_list_orders_with_pagination(self, order_service):
        ...
        with patch("bot_v2.features.brokerages.coinbase.rest.orders.to_order") as mock_to_order:
    ...
    def test_order_retry_on_duplicate_client_id(
        ...
        with patch("bot_v2.features.brokerages.coinbase.models.to_order") as mock_to_order:
            mock_to_order.return_value = Mock(spec=Order)
            order = order_service.place_order(
                symbol="BTC-USD",
                side=OrderSide.BUY,
