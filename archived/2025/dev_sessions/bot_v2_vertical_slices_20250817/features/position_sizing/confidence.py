...
def confidence_adjusted_size(base_size: float, confidence: float,
                           adjustment: ConfidenceAdjustment) -> Tuple[float, str]:
    """
...
def multi_model_confidence(confidences: List[float], weights: Optional[List[float]] = None) -> float:
    """
...
def confidence_decay(initial_confidence: float, time_since_prediction: float,
...
def confidence_from_backtest_metrics(sharpe_ratio: float, win_rate: float,
                                   profit_factor: float, max_drawdown: float) -> float:
    """
    ...
    confidence = sum(comp * weight for comp, weight in zip(components, weights))
    return max(0.0, min(1.0, confidence))


def adaptive_confidence_threshold(recent_performance: List[float], 
                                base_threshold: float = 0.6) -> float:
    """
