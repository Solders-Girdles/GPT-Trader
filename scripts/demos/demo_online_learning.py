...
def test_learning_scheduler():
    ...
    config = SchedulerConfig(
        scheduler_type=SchedulerType.EXPONENTIAL, initial_lr=0.01, decay_rate=0.95, decay_steps=10
    )
...
def test_drift_detector():
    ...
    # Create detector
    config = DriftDetectorConfig(warmup_period=30, window_size=50, performance_threshold=0.1)
    ...
    for i in range(50):
        ...
        if result is not None:
            drift_count += 1
            print(
                f"  Drift detected at sample {i}: {result.drift_severity.value}, "
...
def test_online_learning_pipeline():
    ...
    for i in range(10):
        ...
        if i % 3 == 0:
            print(
                f"  Sample {i}: prediction={result['prediction']:.3f}, "
    ...
    performance_before = (
        np.mean(list(pipeline.performance_history)[-10:]) if pipeline.performance_history else 0.5
    )
    ...
    for i in range(30):
        ...
        if result.get("model_updated") and result.get("adaptation_result"):
            adaptations += 1
            print(
                f"    Adaptation {adaptations}: {result['adaptation_result'].get('adaptation_strategy', 'unknown')}"
            )
    ...
    if learning_curve.learning_rates:
        print(
            f"  Learning rate evolution: {learning_curve.learning_rates[0]:.6f} -> {learning_curve.learning_rates[-1]:.6f}"
        )
...
def test_integration_scenario():
    ...
    def generate_market_features(regime, size):
        ...
        sentiment = np.random.normal(
            0.5 if regime == "bull" else -0.5 if regime == "bear" else 0, 0.2, size
        )
    ...
    initial_df = pd.DataFrame(
        features, columns=["returns", "volume", "volatility", "momentum", "sentiment"]
    )
    ...
    for regime, duration, description in regimes:
        ...
        recent_performance = (
            list(pipeline.performance_history)[-20:]
            if len(pipeline.performance_history) >= 20
            else list(pipeline.performance_history)
        )
    ...
    print(
        f"   Final performance: {np.mean(list(pipeline.performance_history)[-10:]) if pipeline.performance_history else 0:.3f}"
    )
    ...
    for regime in ["bull", "bear", "sideways"]:
        ...
        test_df = pd.DataFrame(
            test_features, columns=["returns", "volume", "volatility", "momentum", "sentiment"]
        )
