...
class Greeks:
    ...
    # Additional metrics
    lambda_leverage: float | None = None  # Lambda (elasticity)
    vanna: float | None = None  # dDelta/dVol
    charm: float | None = None  # dDelta/dTime
    vomma: float | None = None  # dVega/dVol
    speed: float | None = None  # dGamma/dSpot
...
class BlackScholesCalculator:
    ...
    def calculate_implied_volatility(
        option: OptionContract, market_price: float, max_iterations: int = 100
    ) -> float:
        """
...
class GreeksCalculator:
    ...
    def _calculate_gamma(
        self, S: float, K: float, T: float, r: float, sigma: float, d1: float
    ) -> float:
    ...
    def _calculate_vanna(
        self, S: float, K: float, T: float, r: float, sigma: float, d1: float, d2: float
    ) -> float:
...
def demonstrate_greeks():
    ...
    print(
        f"    → 1% move in underlying = ${greeks.delta * option.underlying_price * 0.01:.2f} change"
    )
