...
from bot_v2.features.live_trade.strategies.slippage_guard import (
    SlippageGuard, SlippageConfig, MarketDepth, create_test_depth
)
...
class TestSlippageGuard:
    ...
    def test_max_quantity_calculation_buy(self):
        ...
        # For 50 bps impact: max_qty = (50/10000) * 1000 * 0.8 = 4.0 
        # (0.8 factor from 20% safety buffer)
        max_qty = self.guard.calculate_max_quantity_for_impact(depth, "buy", 50.0)
        
        expected = Decimal("1000") * Decimal("0.005") * Decimal("0.8")  # 4.0
        assert max_qty is not None
        assert abs(max_qty - expected) < Decimal("0.01")
    ...
    def test_max_quantity_calculation_sell(self):
        ...
        # For 25 bps impact: max_qty = (25/10000) * 2000 * 0.8 = 4.0
        max_qty = self.guard.calculate_max_quantity_for_impact(depth, "sell", 25.0)
    ...
    def test_order_rejection_excessive_impact(self):
        ...
        large_qty = Decimal("10.0")  # 10 vs 100*0.8*0.005 = 0.4 max safe
        
        should_reject, reason = self.guard.should_reject_order(depth, "buy", large_qty)
        
        assert should_reject == True
        assert "impact" in reason.lower()
        assert "max safe qty" in reason.lower()
    ...
    def test_order_acceptance_within_limits(self):
        ...
        small_qty = Decimal("2.0")  # Well under max safe qty
        
        should_reject, reason = self.guard.should_reject_order(depth, "buy", small_qty)
    ...
    def test_l1_vs_l10_mode(self):
        ...
        max_qty_l1 = l1_guard.calculate_max_quantity_for_impact(depth, "buy", 50.0)
        
        # L10 mode: uses L10 depth (500)  
        max_qty_l10 = self.guard.calculate_max_quantity_for_impact(depth, "buy", 50.0)
        
        assert max_qty_l1 is not None
        assert max_qty_l10 is not None
        assert max_qty_l10 > max_qty_l1  # L10 should allow larger orders
    ...
    def test_safety_buffer_application(self):
        ...
        max_qty_buffered = self.guard.calculate_max_quantity_for_impact(depth, "buy", 100.0)
        
        # Without buffer: 1000 effective depth
        max_qty_no_buffer = no_buffer_guard.calculate_max_quantity_for_impact(depth, "buy", 100.0)
        
        assert max_qty_buffered is not None
        assert max_qty_no_buffer is not None
        assert max_qty_no_buffer > max_qty_buffered  # No buffer allows larger orders
    ...
    def test_get_safe_quantity_reduction(self):
        ...
        desired_qty = Decimal("10.0")  # Too large
        safe_qty, reason = self.guard.get_safe_quantity(depth, "buy", desired_qty)
        
        assert safe_qty < desired_qty  # Should be reduced
        assert safe_qty > Decimal("0")  # But not zero
    ...
    def test_get_safe_quantity_no_reduction_needed(self):
        ...
        desired_qty = Decimal("1.0")  # Small order
        safe_qty, reason = self.guard.get_safe_quantity(depth, "buy", desired_qty)
        
        assert safe_qty == desired_qty  # No reduction needed
