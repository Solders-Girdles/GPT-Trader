...
def evaluate_predictions(predictions: np.ndarray, actuals: np.ndarray) -> ModelPerformance:
    ...
    try:
        ...
        if len(pred_strategies) != len(actual_strategies):
            raise DataError(
                f"Strategy extraction length mismatch: {len(pred_strategies)} vs {len(actual_strategies)}"
            )
        ...
        if len(pred_returns) != len(actual_returns):
            logger.warning(
                f"Return extraction length mismatch: {len(pred_returns)} vs {len(actual_returns)}"
            )
    ...
    try:
        ...
        if "strategy" in predictions and "strategy" in actuals:
            try:
                ...
                metrics["strategy_accuracy"] = _validate_metric_value(
                    accuracy, "strategy_accuracy", 0.0, 1.0
                )
            except Exception as e:
        ...
        if "drawdown" in predictions and "drawdown" in actuals:
            try:
                pred_drawdown = _validate_numeric_value(
                    predictions["drawdown"], "predicted_drawdown"
                )
                actual_drawdown = _validate_numeric_value(actuals["drawdown"], "actual_drawdown")

                drawdown_mae = abs(pred_drawdown - actual_drawdown)
                metrics["drawdown_mae"] = _validate_metric_value(
                    drawdown_mae, "drawdown_mae", 0.0, 100.0
                )
            except Exception as e:
...
def calculate_f1_score(precision: float, recall: float) -> float:
    ...
    try:
        ...
        if precision < 0 or precision > 1 or recall < 0 or recall > 1:
            logger.warning(
                f"Invalid precision ({precision}) or recall ({recall}) for F1 calculation"
            )
            return 0.0
...
def calculate_correlation(pred: np.ndarray, actual: np.ndarray) -> float:
    ...
    try:
        ...
        try:
            ...
            try:
                correlation = np.sum(
                    (valid_pred - np.mean(valid_pred)) * (valid_actual - np.mean(valid_actual))
                ) / (
...
def _validate_evaluation_inputs(predictions: np.ndarray, actuals: np.ndarray) -> None:
    ...
    if len(predictions) != len(actuals):
        raise ValidationError(
            f"Predictions and actuals length mismatch: {len(predictions)} vs {len(actuals)}"
        )
...
def _validate_array_inputs(pred: np.ndarray, actual: np.ndarray, metric_name: str) -> None:
    ...
    if len(pred) != len(actual):
        raise ValidationError(
            f"Array length mismatch for {metric_name}: {len(pred)} vs {len(actual)}"
        )
...
def validate_model_performance(performance: ModelPerformance) -> ModelPerformance:
    ...
    try:
        ...
        total_predictions = (
            max(0, int(performance.total_predictions))
            if performance.total_predictions is not None
            else 0
        )
        successful_predictions = (
            max(0, int(performance.successful_predictions))
            if performance.successful_predictions is not None
            else 0
        )
        ...
        if successful_predictions > total_predictions:
            logger.warning(
                f"Successful predictions ({successful_predictions}) > total ({total_predictions}), fixing"
            )
...
def get_evaluation_summary(performance: ModelPerformance) -> dict[str, any]:
    ...
    try:
        ...
        health_score = (
            ...
            + (1 - min(performance.mean_absolute_error / 100, 1))
            * 20  # 20% weight on MAE (inverted)
            + max(performance.r_squared, 0) * 15  # 15% weight on R² (only positive)
            + (performance.backtest_correlation + 1)
            / 2
            * 10  # 10% weight on correlation (normalized)
        )
        ...
        return {
            ...
            "success_rate": performance.successful_predictions
            / max(performance.total_predictions, 1),
            "key_metrics": {
                "accuracy": performance.accuracy,
                "f1_score": performance.f1_score,
