    ...
    def with_retry(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            backoff_factor: Override default exponential base
            retry_on: Specific exceptions to retry on (others raise immediately)
        ...
        while attempt < current_max_attempts:
            try:
                ...
                # Check if we should retry on this specific exception
                if retry_on is not None and not isinstance(e, retry_on):
                    self._record_error(handle_error(e))
                    raise
                ...
                if attempt < current_max_attempts:
                    # Use local calculation to respect overrides
                    delay = min(
                    ...
                    logger.warning(
                        f"Attempt {attempt}/{current_max_attempts} failed: {e}. "
        ...
        raise TradingError(
            f"All {current_max_attempts} retry attempts failed",
