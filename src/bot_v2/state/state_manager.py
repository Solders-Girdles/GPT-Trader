...
class StateManager:
    ...
    async def get_state(self, key: str, auto_promote: bool = True) -> Optional[Any]:
        """
    ...
    async def _get_from_postgres(self, key: str) -> Optional[Any]:
        ...
        try:
            with self.pg_conn.cursor() as cursor:
                cursor.execute(
                    "SELECT data FROM state_warm WHERE key = %s",
                    (key,)
                )
    ...
    async def _set_in_redis(self, key: str, value: str, ttl_seconds: Optional[int]) -> bool:
        """Set value in Redis"""
        if not self.redis_client:
            return False
    ...
    def _manage_cache_size(self):
        ...
        current_size = sum(
            ...
            for v in self._local_cache.values()
        )
    ...
    async def get_keys_by_pattern(self, pattern: str) -> List[str]:
        ...
        if self.redis_client:
            try:
                ...
                logger.debug("Redis key lookup failed for pattern %s: %s", pattern, exc, exc_info=True)
        ...
        if self.pg_conn:
            try:
                ...
                with self.pg_conn.cursor() as cursor:
                    cursor.execute(
                        "SELECT key FROM state_warm WHERE key LIKE %s",
                        (sql_pattern,)
                    )
        ...
        if self.s3_client:
            try:
                ...
                response = self.s3_client.list_objects_v2(
                    Bucket=self.config.s3_bucket,
                    Prefix=f"cold/{prefix}"
                )
                if 'Contents' in response:
                    s3_keys = [
                        obj['Key'].replace('cold/', '')
