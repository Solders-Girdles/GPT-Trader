...
class PreflightCheck:
    ...
    def check_python_version(self) -> bool:
        ...
        if version.major == 3 and version.minor >= 12:
            self.log_success(f"Python {version.major}.{version.minor}.{version.micro} meets requirements")
            return True
        else:
    ...
    def check_environment_variables(self) -> bool:
        ...
        if not api_key:
            ...
            if api_key.startswith("organizations/") and "/apiKeys/" in api_key:
                ...
                self.log_error(f"Invalid CDP API key format. Expected: organizations/.../apiKeys/...")
                all_good = False
            ...
            if val:
                try:
                    ...
                    if min_val <= num <= max_val:
                        ...
                        self.log_warning(f"{var}={val} outside recommended range [{min_val}, {max_val}]")
                except ValueError:
    ...
    def check_risk_configuration(self) -> bool:
        ...
        try:
            ...
            if config.daily_loss_limit > Decimal("1000"):
                self.log_warning(f"Daily loss limit ${config.daily_loss_limit} seems high for testing")
            ...
            if api_key and private_key:
                ...
                try:
                    ...
                    if server_time_resp and "iso" in server_time_resp:
                        server_time = datetime.fromisoformat(server_time_resp["iso"].replace("Z", "+00:00"))
    ...
    def check_disk_space(self) -> bool:
        ...
        try:
            ...
            if free_gb > 1.0:
                self.log_success(f"Disk space: {free_gb:.1f}GB free of {total_gb:.1f}GB ({used_pct:.0f}% used)")
                return True
            elif free_gb > 0.5:
    ...
    def simulate_dry_run(self) -> bool:
        ...
        try:
            from bot_v2.orchestration.perps_bot import BotConfig, PerpetualBot
            ...
            # Check if we can create the bot
    ...
    def generate_report(self) -> Tuple[bool, str]:
        ...
        if status == "READY":
            ...
            print(f"2. Review .env.production for configuration guidance")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Production preflight check for GPT-Trader"
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
