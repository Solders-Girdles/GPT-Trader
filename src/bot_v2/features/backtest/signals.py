...
from ...errors import StrategyError, DataError, ValidationError
from ...validation import (
    validate_inputs, StrategyNameValidator, DataFrameValidator,
...
def generate_signals(
    ...
    **strategy_params
    ...
    try:
        ...
        raise DataError(
            f"Data quality validation failed for signal generation",
            context={
                'strategy': strategy_name,
...
def _validate_data_quality(data: pd.DataFrame) -> None:
    ...
    if not (data['high'] >= data['low']).all():
        ...
        raise DataError(
            f"Invalid High < Low relationships found",
...
def get_signal_statistics(signals: pd.Series) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            'activity_rate': (buy_signals + sell_signals) / total_signals if total_signals > 0 else 0
        }
    except Exception as e:
