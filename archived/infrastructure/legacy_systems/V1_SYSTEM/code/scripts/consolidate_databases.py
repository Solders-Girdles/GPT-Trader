...
class DatabaseConsolidator:
    ...
    def __init__(self, project_root: Path = None):
        ...
        self.backup_dir = (
            self.project_root / "data" / "backup" / datetime.now().strftime("%Y%m%d_%H%M%S")
        )
    ...
    def audit_databases(self) -> dict[str, Any]:
        ...
        for db_name, rel_path in self.LEGACY_DATABASES.items():
            ...
            try:
                ...
                logger.info(
                    f"      Tables: {len(tables)}, Records: {total_records:,}, Size: {file_size_mb:.2f} MB"
                )
