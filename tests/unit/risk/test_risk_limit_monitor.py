...
from src.bot.risk.risk_limit_monitor import LimitBreach, LimitType, RiskLimit, RiskLimitMonitor
...
class TestRiskLimitMonitor:
    ...
    def test_dynamic_limit_adjustment(self, monitor):
        ...
        adjusted_threshold = monitor._adjust_dynamic_threshold(dynamic_limit, high_vol_metrics)
    ...
    def test_limit_breach_cooldown(self, monitor):
        ...
        breach_metrics = {"var_95": -0.08, "max_position_size": 0.20, "current_drawdown": -0.05}
    ...
    def test_batch_limit_update(self, monitor):
        ...
        updates = {"max_var": -0.07, "position_concentration": 0.30, "drawdown_warning": -0.15}
    ...
    def test_circuit_breaker_integration(self, monitor):
        ...
        with patch.object(monitor, "_halt_system") as mock_halt:
            ...
            circuit_breach = next(
                (b for b in breaches if b.action == BreachAction.HALT_SYSTEM), None
            )
    ...
    def test_limit_performance_monitoring(self, monitor):
        ...
        large_metrics.update(
            {"var_95": -0.03, "max_position_size": 0.20, "current_drawdown": -0.05}
        )
    ...
    def test_limit_hierarchy(self, monitor):
        ...
        critical_breach = next((b for b in var_breaches if b.limit_name == "critical_var"), None)
    ...
    def test_risk_budget_allocation(self, monitor):
        ...
        equity_breach = next((b for b in budget_breaches if "equity" in b["category"]), None)
...
class TestRiskLimitMonitorIntegration:
    ...
    def mock_risk_engine(self):
        ...
        mock.calculate_exposure_metrics.return_value = Mock(
            gross_exposure=0.85, net_exposure=0.75, concentration_ratio=0.25
        )
        return mock
    ...
    def test_integration_with_risk_engine(self, mock_risk_engine, sample_limits):
        ...
        monitor = RiskLimitMonitor(limits=sample_limits, risk_engine=mock_risk_engine)
    ...
    def test_integration_with_alerting(self, mock_alerting_system, sample_limits):
        ...
        monitor = RiskLimitMonitor(limits=sample_limits, alerting_system=mock_alerting_system)
