...
class StatisticalAnalyzer:
    ...
    def _t_test(self,
                sample_a: np.ndarray,
                sample_b: np.ndarray,
                paired: bool) -> StatisticalTestResult:
        """Perform t-test"""
        if paired:
    ...
    def _mann_whitney_test(self,
                          sample_a: np.ndarray,
                          sample_b: np.ndarray) -> StatisticalTestResult:
    ...
    def _chi_square_test(self,
                        sample_a: Union[np.ndarray, List],
                        sample_b: Union[np.ndarray, List],
                        categorical: bool = False) -> StatisticalTestResult:
        """
        ...
        if not categorical and contingency_table.shape == (2, 2):
            ...
            se = np.sqrt(prop_a * (1 - prop_a) / len(sample_a) + 
                        prop_b * (1 - prop_b) / len(sample_b))
    ...
    def _make_recommendation(self, result: StatisticalTestResult) -> str:
        ...
        if result.is_significant:
            if abs(result.effect_size) >= 0.5:  # Medium or large effect
                return "Strong evidence for difference. Consider adopting the better performing model."
            else:  # Small effect
                return "Statistically significant but small effect. Consider practical significance."
        else:
    ...
    def compare_multiple_samples(self,
        ...
        if correction_method in [CorrectionMethod.BENJAMINI_HOCHBERG, 
    ...
    def _apply_correction(self,
                         p_values: List[float],
                         method: CorrectionMethod) -> List[float]:
        """Apply multiple comparison correction"""
    ...
    def test_variance_equality(self, *samples) -> StatisticalTestResult:
        ...
        return StatisticalTestResult(
            ...
            recommendation="Use Welch's t-test" if p_value < self.alpha else "Standard t-test appropriate"
        )
...
if __name__ == "__main__":
    ...
    print(f"   CI: [{bootstrap_result.confidence_interval[0]:.4f}, "
