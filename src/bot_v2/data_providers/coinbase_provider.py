...
class CoinbaseDataProvider(DataProvider):
    ...
    def __init__(
        ...
        adapter: Optional[CoinbaseBrokerage] = None,
        ...
        # Configure streaming
        self.enable_streaming = enable_streaming or os.environ.get('COINBASE_ENABLE_STREAMING', '0') == '1'
        self.cache_ttl = cache_ttl
        
        # Initialize ticker service for streaming if enabled
        self.ticker_service: Optional[CoinbaseTickerService] = None
        ...
        logger.info(f"CoinbaseDataProvider initialized (streaming={'enabled' if self.enable_streaming else 'disabled'})")
    ...
    def _on_ticker_update(self, symbol: str, ticker: Any):
        ...
        logger.debug(f"Ticker update for {symbol}: bid={ticker.bid}, ask={ticker.ask}, last={ticker.last}")
    ...
    def get_multiple_symbols(self, symbols: List[str], period: str = "60d") -> Dict[str, pd.DataFrame]:
        ...
        if self.enable_streaming and self.ticker_service:
            ...
            try:
                ...
                logger.info(f"Subscribed to WebSocket updates for {len(normalized_symbols)} symbols")
            except Exception as e:
