...
class ReduceOnlyStubBroker(IBrokerage):
    ...
    def get_quote(self, symbol: str) -> Quote:
        return Quote(symbol=symbol, bid=Decimal('99'), ask=Decimal('101'), last=Decimal('100'), ts=datetime.utcnow())
        ...
        if filled > 0:
            if side == OrderSide.SELL:
                if pos and pos.side == 'long':
                    remaining = pos.qty - min(filled, pos.qty)
                    self.positions[symbol] = Position(symbol, remaining, pos.entry_price, avg, Decimal('0'), Decimal('0'), None, 'long' if remaining > 0 else 'long')
                else:
                ...
                if pos and pos.side == 'short':
                    remaining = pos.qty - min(filled, pos.qty)
                    self.positions[symbol] = Position(symbol, remaining, pos.entry_price, avg, Decimal('0'), Decimal('0'), None, 'short' if remaining > 0 else 'short')
                else:
                    if not reduce_only:
    ...
    def list_orders(self, status: Optional[OrderStatus] = None, symbol: Optional[str] = None):  # pragma: no cover
        return []
...
def test_reduce_only_rejects_opening_trades():
    ...
    risk = LiveRiskManager(config=RiskConfig(reduce_only_mode=True, max_position_pct_per_symbol=0.9, max_exposure_pct=0.9))
...
def test_reduce_only_sell_clamped_to_position_size():
    ...
    broker.positions['BTC-PERP'] = Position('BTC-PERP', Decimal('1'), Decimal('100'), Decimal('100'), Decimal('0'), Decimal('0'), None, 'long')
    risk = LiveRiskManager(config=RiskConfig(reduce_only_mode=True, max_position_pct_per_symbol=0.9, max_exposure_pct=0.9))
    engine = LiveExecutionEngine(broker, risk_manager=risk)
    ...
    oid = engine.place_order(
        ...
        side=OrderSide.SELL,
        order_type=OrderType.MARKET,
        qty=Decimal('2'),  # greater than position size
        reduce_only=True,
    )
