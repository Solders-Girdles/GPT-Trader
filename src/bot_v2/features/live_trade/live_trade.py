...
from .types import (
    BrokerConnection, AccountInfo, OrderStatus, MarketHours
)
from .brokers import AlpacaBroker, IBKRBroker, SimulatedBroker
from .risk import LiveRiskManager
from .execution import ExecutionEngine
from .strategies.perps_baseline import BaselinePerpsStrategy, Decision, Action
from ...errors import NetworkError, ValidationError, ExecutionError, handle_error, log_error
from ...errors.handler import get_error_handler, RecoveryStrategy
from ...validation import SymbolValidator, PositiveNumberValidator, ChoiceValidator
...
def get_positions() -> List[Position]:
    ...
    try:
        ...
        network_error = NetworkError(
            "Unexpected error retrieving positions",
...
def get_account() -> Optional[AccountInfo]:
    ...
    try:
        ...
        network_error = NetworkError(
            "Unexpected error retrieving account",
            context={'original_error': str(e)}
        )
        log_error(network_error)
...
def close_all_positions() -> bool:
    ...
    try:
        ...
        execution_error = ExecutionError(
            "Unexpected error closing all positions",
            context={'original_error': str(e)}
        )
        log_error(execution_error)
...
def run_strategy(
    ...
    symbols: List[str],
    mark_cache: Optional[Dict[str, Decimal]] = None,
    mark_windows: Optional[Dict[str, List[Decimal]]] = None
) -> Dict[str, Decision]:
    """
        ...
        for symbol in symbols:
            try:
                ...
                if decision.action in [Action.BUY, Action.SELL]:
                    ...
                    if decision.target_notional:
                        qty = decision.target_notional / current_mark
                        
                        # Apply quantization rules
                        from ..brokerages.coinbase.utils import enforce_perp_rules
                        ...
                        # Place order (ExecutionEngine would handle this in real impl)
                        logger.info(f"Would place {side} order: {quantized_qty} {symbol} @ market")
                        
                elif decision.action == Action.CLOSE:
                    # Exit order
                    if position_state:
                        close_side = "sell" if position_state['side'] == "long" else "buy"
                        qty = decision.qty or position_state['qty']
                        ...
                        logger.info(f"Would place reduce-only {close_side} order: {quantized_qty} {symbol}")
