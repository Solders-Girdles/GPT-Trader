...
from .advanced_degradation_detector import DegradationType
...
class DegradationDashboard:
    ...
    def __init__(self, 
                 integrator: Optional[DegradationIntegrator] = None,
    ...
    def create_overview_dashboard(self) -> go.Figure:
        ...
        fig = make_subplots(
            rows=3, cols=3,
            subplot_titles=(
                'Overall Status', 'Accuracy Trend', 'Degradation Score',
                'Feature Drift', 'Confidence Trend', 'Error Distribution',
                'CUSUM Chart', 'Alerts Timeline', 'Retraining History'
            ),
            specs=[
        ...
        fig.add_trace(
            go.Indicator(
                ...
                domain={'x': [0, 1], 'y': [0, 1]},
                delta={'reference': latest.legacy_accuracy * 100 if len(self.integrator.reports) > 1 
        ...
        if len(self.integrator.reports) > 1:
            ...
            fig.add_hline(y=0.6, line_dash="dash", line_color="orange", 
                         annotation_text="Warning", row=1, col=3)
            fig.add_hline(y=0.8, line_dash="dash", line_color="red", 
                         annotation_text="Critical", row=1, col=3)
        ...
        if self.integrator.advanced_detector and latest.feature_drift_count > 0:
            ...
            if drift_scores:
                ...
                drift_values = [1.0 - drift_scores.get(f, {}).get('mean', 1.0) 
                               for f in features]
        ...
        if len(self.integrator.reports) > 1:
            ...
            if self.integrator.advanced_detector:
                ...
                fig.add_hline(y=h_value, line_dash="dash", line_color="red",
                             annotation_text="Control Limit", row=3, col=1)
        ...
        if latest.alerts:
            ...
            colors = ['green' if s == 'low' else 'orange' if s == 'medium' else 'red' 
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    text=[f"{t}: {s}" for t, s in zip(alert_types, alert_severities)],
    ...
    def create_feature_drift_dashboard(self) -> go.Figure:
        ...
        fig = make_subplots(
            ...
            subplot_titles=(
                ...
                'Drift Evolution', 'Feature Correlations'
            ),
            specs=[
    ...
    def create_performance_dashboard(self) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2, cols=3,
            subplot_titles=(
                'Accuracy vs Baseline', 'Performance Volatility', 'Trend Analysis',
                'Error Rate', 'Confidence Distribution', 'Performance by Hour'
            ),
            specs=[
    ...
    def create_alert_dashboard(self) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=(
                'Alert Frequency', 'Alert Types Distribution',
                'Severity Timeline', 'Alert Patterns'
            ),
            specs=[
        ...
        if pattern_data:
            ...
            fig.add_trace(
                go.Sunburst(
                    labels=['All'] + df_patterns['severity'].tolist() + df_patterns['type'].tolist(),
    ...
    def _get_retraining_history(self) -> Dict[str, List]:
        ...
        return {
            'dates': [a['timestamp'].strftime('%Y-%m-%d') if isinstance(a['timestamp'], datetime)
                     else a['timestamp'] for a in retrain_alerts[-5:]],
    ...
    def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        if len(self.integrator.reports) > 1:
            ...
            summary['score_trend'] = 'increasing' if recent_scores[-1] > recent_scores[0] else 'decreasing'
