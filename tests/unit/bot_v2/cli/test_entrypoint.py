...
def reload_cli(monkeypatch, runtime_settings_factory):
    """Reload bot_v2.cli with deterministic settings and logging stubs."""
    ...
    def _loader(env_overrides: dict[str, str] | None = None):
        ...
        configure_mock = Mock()
        monkeypatch.setattr("bot_v2.logging.configure_logging", configure_mock)
        ...
        if "bot_v2.cli" in sys.modules:
            module = sys.modules["bot_v2.cli"]
        else:
            module = importlib.import_module("bot_v2.cli")
...
def test_main_dispatches_to_handler(monkeypatch, reload_cli):
    ...
    cli_services = importlib.import_module("bot_v2.cli.services")
...
def test_maybe_enable_debug_logging(monkeypatch, reload_cli):
    ...
    perps_logger = logging.getLogger("bot_v2.features.brokerages.coinbase")
    orchestrator_logger = logging.getLogger("bot_v2.orchestration")
