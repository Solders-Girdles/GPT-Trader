...
class ModelComparisonReport:
    ...
    def compare_models(self,
                      model_a_id: str,
                      model_b_id: str,
                      metrics: Optional[List[ComparisonMetric]] = None) -> Dict[str, ComparisonResult]:
        """
    ...
    def _compare_metric(self,
                       ...
                       metric: ComparisonMetric) -> Optional[ComparisonResult]:
        ...
        if metric in [ComparisonMetric.MAX_DRAWDOWN, ComparisonMetric.LATENCY, 
                     ComparisonMetric.ERROR_RATE, ComparisonMetric.MEMORY_USAGE]:
            # Lower is better
    ...
    def _get_available_metrics(self,
                              model_a: ModelMetrics,
                              model_b: ModelMetrics) -> List[ComparisonMetric]:
    ...
    def generate_report(self,
                       ...
                       include_visuals: bool = False) -> str:
        """
    ...
    def _generate_text_report(self) -> str:
        ...
        for model_id, metrics in self.models.items():
            ...
            report.write(f"  Sharpe Ratio: {metrics.sharpe_ratio:.2f}\n" if metrics.sharpe_ratio else "")
            report.write(f"  Max Drawdown: {metrics.max_drawdown:.2%}\n" if metrics.max_drawdown else "")
            report.write(f"  Win Rate: {metrics.win_rate:.2%}\n" if metrics.win_rate else "")
            report.write(f"  Avg Latency: {metrics.avg_latency_ms:.2f}ms\n" if metrics.avg_latency_ms else "")
            report.write(f"  Predictions: {metrics.n_predictions:,}\n" if metrics.n_predictions else "")
    ...
    def _generate_markdown_report(self) -> str:
        ...
        if len(model_ids) >= 2:
            ...
            report.write("| Metric | ")
            report.write(" | ".join(f"{m1} vs {m2}" for m1 in model_ids for m2 in model_ids if m1 < m2))
            report.write(" |\n")
    ...
    def _generate_html_report(self, include_visuals: bool) -> str:
        ...
        html.write("<table>")
        html.write("<tr><th>Model</th><th>Accuracy</th><th>Sharpe</th><th>Drawdown</th><th>Latency</th></tr>")
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if rankings:
            ...
            recommendations.append(
                f"Best overall model: {best_model} (score: {best_score:.2f})"
            )
            ...
            if len(rankings) > 1:
                ...
                if margin > 0.1:  # 10% better
                    ...
                    recommendations.append(
                        "Models are closely matched - consider A/B testing"
                    )
            ...
            if best_metrics.max_drawdown and best_metrics.max_drawdown > 0.20:
                recommendations.append(
                    "Warning: High drawdown detected - implement risk controls"
                )
            
            if best_metrics.error_rate and best_metrics.error_rate > 0.01:
                recommendations.append(
                    "Consider improving model reliability (error rate > 1%)"
                )
    ...
    def save_report(self,
                   filepath: str,
                   format: ReportFormat = ReportFormat.TEXT) -> None:
        """
        Save report to file.
