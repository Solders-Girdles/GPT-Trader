...
class FeeTierResolver:
    ...
    def __init__(self, client=None, refresh_interval_hours: int = 6):
        self.client = client
        self.refresh_interval = timedelta(hours=refresh_interval_hours)
        
        # Current tier info
        self._current_tier: Optional[FeeTier] = None
        self._last_refresh: Optional[datetime] = None
        self._volume_30d: Optional[Decimal] = None
    ...
    async def get_30day_volume(self) -> Optional[Decimal]:
...
class FeesEngine:
    ...
    async def calculate_order_fee(
        ...
        logger.debug(f"Fee calculated: {fee_amount:.4f} for {symbol} ({fee_type.value}, {current_tier.tier_name})")
    ...
    async def get_fee_summary(self, hours_back: int = 24) -> Dict:
        """Get fee summary for specified period."""
        cutoff_time = datetime.now() - timedelta(hours=hours_back)
        
        recent_fees = [
            f for f in self._fee_history 
    ...
    async def estimate_trade_cost(
        ...
        fee_calc = await self.calculate_order_fee(symbol, notional, is_post_only)
        
        if side.lower() == 'buy':
            total_cost = notional + fee_calc.fee_amount
        else:  # sell
            total_cost = notional - fee_calc.fee_amount
        ...
        return {
            ...
            'fee_rate': float(fee_calc.fee_rate),
            'fee_type': fee_calc.fee_type.value,
            'total_cost': float(total_cost),
            'tier': fee_calc.tier_name
        }
...
async def create_fees_engine(client=None) -> FeesEngine:
