...
def test_market_calendar():
    ...
    for test_time, expected_status in test_cases:
        ...
        logger.info(
            f"  {result} {test_time.strftime('%Y-%m-%d %H:%M')} -> {status.value} (expected {expected_status.value})"
        )
...
def test_data_validator():
    ...
    test_cases = [
        ...
        (
            MarketData(
                "AAPL", datetime.now(), Decimal("100"), bid=Decimal("101"), ask=Decimal("99")
            ),
            "Inverted bid/ask",
        ),
        (
            MarketData("AAPL", datetime.now() + timedelta(hours=1), Decimal("100")),
            "Future timestamp",
        ),
    ]
    ...
    logger.info(
        f"  Anomaly detection (price={anomaly_price}): {'✓ Detected' if is_anomaly else '✗ Not detected'}"
    )
...
async def test_realtime_feed():
    ...
    config = DataFeedConfig(validate_data=True, buffer_size=1000)
