...
class MarketRegimeDetector(MLModel):
    ...
    def __init__(
        ...
        """
        if config is None:
            config = ComponentConfig(component_id="regime_detector", component_type="ml_model")
    ...
    def get_transition_probabilities(self) -> pd.DataFrame:
        ...
        trans_df = pd.DataFrame(self.transition_matrix, index=regime_names, columns=regime_names)
        ...
        for state in range(self.n_components):
            ...
            if np.sum(mask) > 0:
                ...
                self.regime_stats[regime_name] = {
                    ...
                    "sharpe_ratio": float(
                        np.nanmean(regime_returns) / (np.nanstd(regime_returns) + 1e-10)
                    ),
                }
            else:
                self.regime_stats[regime_name] = {
    ...
    def _store_predictions(self, timestamps, regimes: np.ndarray, states: np.ndarray):
        ...
        for ts, regime, state in zip(timestamps, regimes, states, strict=False):
            try:
