...
def _get_next_regime(current: MarketRegime) -> MarketRegime:
    ...
    regimes, probs = zip(*possible_transitions)
...
def _generate_trigger_events(from_regime: MarketRegime, to_regime: MarketRegime) -> List[str]:
    ...
    economic_events = [
        "Fed policy change", "Economic data release", "Earnings season",
        "GDP report", "Inflation data", "Employment report"
    ]
    
    # Market events  
    market_events = [
        "Technical breakout", "Support/resistance break", "Volume spike",
        "Sector rotation", "Options expiration", "Rebalancing flows"
    ]
    
    # Crisis events
    crisis_events = [
        "Geopolitical tension", "Financial institution failure", 
        "Currency crisis", "Credit crunch", "Black swan event"
    ]
    
    # News events
    news_events = [
        "Corporate announcement", "Regulatory change", "Trade news",
        "Political development", "Natural disaster", "Technology disruption"
    ]
    ...
    if to_regime == MarketRegime.CRISIS:
        ...
        return np.random.choice(economic_events + ["Positive sentiment shift"], size=2, replace=False).tolist()
    elif 'BEAR' in to_regime.value and 'BULL' in from_regime.value:
        return np.random.choice(economic_events + ["Negative sentiment shift"], size=2, replace=False).tolist()
    else:
        # General transition
