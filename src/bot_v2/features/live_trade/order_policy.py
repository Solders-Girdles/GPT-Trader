...
class OrderCapability:
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            'quantity_increment': float(self.quantity_increment) if self.quantity_increment else None,
...
class SymbolPolicy:
    ...
    spread_threshold_bps: Optional[Decimal] = None  # Require post-only if spread > threshold
    
    def get_capability(self, order_type: str, tif: str) -> Optional[OrderCapability]:
    ...
    def is_order_allowed(
        ...
        if capability.max_quantity and quantity > capability.max_quantity:
            return False, f"Quantity {quantity} exceeds capability maximum {capability.max_quantity}"
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            'spread_threshold_bps': float(self.spread_threshold_bps) if self.spread_threshold_bps else None
        }
...
class OrderPolicyMatrix:
    ...
    def add_symbol(
        self,
        symbol: str,
        capabilities: Optional[List[OrderCapability]] = None,
        **policy_kwargs
    ) -> SymbolPolicy:
    ...
    def get_symbol_policy(self, symbol: str) -> Optional[SymbolPolicy]:
    ...
    def get_supported_order_types(self, symbol: str) -> List[Dict]:
    ...
    def recommend_order_config(
        ...
        policy = self.get_symbol_policy(symbol)
        if not policy:
            return {'error': f'No policy for {symbol}'}
        ...
        if market_conditions:
            ...
            if market_conditions.get('volatility_percentile', 0) > 90:
                config['tif'] = 'IOC'
    ...
    def set_reduce_only_mode(self, symbol: str, enabled: bool = True):
    ...
    def get_policy_summary(self) -> Dict:
        ...
        for symbol, policy in self._symbol_policies.items():
            supported_types = [
                cap.order_type for cap in policy.capabilities 
    ...
    async def get_capabilities(self, symbol: str) -> Dict:
    ...
    def _check_rate_limit(self, symbol: str, limit_per_minute: int) -> bool:
        ...
        # Clean old entries
        self._rate_limits[symbol] = [
            ts for ts in self._rate_limits[symbol] if ts >= cutoff
        ]
