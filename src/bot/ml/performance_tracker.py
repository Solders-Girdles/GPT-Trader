...
class PerformanceTracker:
    ...
    def track_prediction(
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """,
            (datetime.now(), model_id, json.dumps(features), prediction, actual, confidence),
        )
    ...
    def update_performance(self, model_id: str, metrics: PerformanceMetrics):
        """
    ...
    def detect_drift(
        self, model_id: str, current_features: pd.DataFrame, reference_features: pd.DataFrame
        ...
        for column in current_features.columns:
            if column in reference_features.columns:
                statistic, p_value = stats.ks_2samp(
                    reference_features[column].dropna(), current_features[column].dropna()
                )
        ...
        if drift_detected:
            logger.warning(
                f"Drift detected for {model_id}: {drift_type.value} (score={drift_score:.3f})"
            )
    ...
    def get_model_health(self, model_id: str) -> ModelHealth | None:
        """
    ...
    def get_performance_history(self, model_id: str, days: int = 30) -> pd.DataFrame:
        """
                    ...
                    comparison_data.append(
                        {
                            ...
                            "Avg Sharpe (7d)": (
                                recent_perf["sharpe_ratio"].mean()
                                if "sharpe_ratio" in recent_perf
                                else None
                            ),
    ...
    def generate_report(self, model_id: str) -> dict[str, Any]:
        ...
        report = {
            ...
            "history_summary": {
                ...
                "avg_f1_30d": (
                    history["f1_score"].mean() if not history.empty and "f1_score" in history else 0
                ),
                "total_predictions_30d": (
                    history["n_predictions"].sum()
                    if not history.empty and "n_predictions" in history
                    else 0
                ),
            },
        }
