...
class CheckpointHandler:
        ...
        with self._checkpoint_lock:
            ...
            try:
                ...
                if await self._store_checkpoint_atomic(checkpoint, checkpoint_data):
                    ...
                    logger.info(
                        f"Checkpoint {checkpoint_id} created successfully in {duration_ms:.2f}ms"
                    )
                    return checkpoint
                else:
                ...
                logger.error(
                    f"Checkpoint creation timed out after {self.config.checkpoint_timeout_seconds}s"
                )
                return None
        ...
        try:
            ...
            if "portfolio" in state_snapshot:
                await self.state_manager.set_state(
                    "portfolio_current", state_snapshot["portfolio"], StateCategory.HOT
                )
            ...
            if "performance_metrics" in state_snapshot:
                await self.state_manager.set_state(
                    "performance_metrics", state_snapshot["performance_metrics"], StateCategory.WARM
                )

            logger.info(
                f"Restored {len(state_snapshot.get('positions', {}))} positions, "
    ...
    async def _verify_restoration(self, checkpoint: Checkpoint) -> bool:
        ...
        try:
            ...
            if original_positions != current_positions:
                logger.warning(
                    f"Position count mismatch: {original_positions} vs {current_positions}"
                )
                return False
    ...
    def _load_checkpoint_from_disk(self, checkpoint_id: str) -> Checkpoint | None:
    ...
    def get_checkpoint_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "invalid_checkpoints": len(
                [cp for cp in self._checkpoint_history if cp.status == CheckpointStatus.INVALID]
            ),
            "total_size_bytes": sum(cp.size_bytes for cp in self._checkpoint_history),
            "oldest_checkpoint": (
                min(self._checkpoint_history, key=lambda cp: cp.timestamp).timestamp
                if self._checkpoint_history
                else None
            ),
            "newest_checkpoint": (
                max(self._checkpoint_history, key=lambda cp: cp.timestamp).timestamp
                if self._checkpoint_history
                else None
            ),
            "average_size_bytes": (
                sum(cp.size_bytes for cp in valid_checkpoints) / len(valid_checkpoints)
                if valid_checkpoints
                else 0
            ),
        }
