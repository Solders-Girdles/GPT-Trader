...
_quality_checker: DataQualityChecker | None = None
_coinbase_manager: HistoricalDataManager | None = None
...
def initialize_data_layer(
    ...
    Example:
        ...
        initialize_data_layer(coinbase_client=client)
        ```
    """
    global _cache, _quality_checker, _coinbase_manager

    _cache = DataCache(max_size_mb=cache_max_size_mb)
    ...
    if coinbase_client is not None:
        ...
        _coinbase_manager = create_coinbase_data_provider(
...
def fetch_data(query: DataQuery) -> pd.DataFrame | None:
    ...
    if query.source == DataSource.COINBASE and _coinbase_manager is not None:
        result = download_from_coinbase(
            query.symbols, query.start_date, query.end_date, query.interval
        )
    else:
        # Legacy fallback
        result = download_from_yahoo(
            query.symbols, query.start_date, query.end_date, query.interval
        )
    ...
    if isinstance(result, dict):
        ...
        for symbol, df in result.items():
            if _cache and not df.empty:
                ...
                _cache.put(symbol_cache_key, df)
...
def download_from_coinbase(
    ...
    async def fetch_all() -> None:
        assert _coinbase_manager is not None
        for symbol in symbols:
            candles = await _coinbase_manager.get_candles(
                symbol=symbol,
...
def download_from_yahoo(*args: Any, **kwargs: Any) -> Any:
    ...
    if len(args) >= 4:
        ...
        return download_from_coinbase(symbols, start_date, end_date, interval)
    return None
