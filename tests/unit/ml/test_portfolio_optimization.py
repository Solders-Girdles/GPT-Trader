...
class TestPortfolioOptimizer:
    ...
    def sample_returns(self):
        ...
        return pd.DataFrame(returns, columns=["Asset1", "Asset2", "Asset3", "Asset4", "Asset5"])
    ...
    def test_max_sharpe_optimization(self, sample_returns):
        ...
        constraints = OptimizationConstraints(min_weight=0.0, max_weight=0.4, long_only=True)

        weights, metrics = optimizer.optimize(
            sample_returns, constraints=constraints, objective="max_sharpe"
        )
    ...
    def test_min_risk_optimization(self, sample_returns):
        ...
        weights, metrics = optimizer.optimize(
            sample_returns, constraints=constraints, objective="min_risk"
        )
    ...
    def test_efficient_frontier(self, sample_returns):
        ...
        weights, metrics = optimizer.optimize(sample_returns, objective="efficient_frontier")
    ...
    def test_portfolio_constraints(self, sample_returns):
        ...
        constraints = OptimizationConstraints(max_weight=0.2, long_only=True)
        ...
        constraints = OptimizationConstraints(
            ...
            long_only=True,  # Daily return target
        )
...
class TestRebalancingEngine:
    ...
    def mock_positions(self):
        ...
        return {"AAPL": 30000, "MSFT": 25000, "GOOGL": 20000, "AMZN": 15000, "META": 10000}
    ...
    def test_rebalancing_triggers(self, mock_positions):
        ...
        triggered, urgency, details = threshold_trigger.check(mock_positions, portfolio_value)
        ...
        triggered, urgency, details = time_trigger.check(mock_positions, portfolio_value, old_date)
    ...
    def test_transaction_costs(self, mock_positions):
        ...
        cost = cost_model.estimate_trade_cost(
            symbol="AAPL", quantity=100, price=150.0, volatility=0.20
        )
    ...
    def test_rebalancing_decision(self, mock_positions, mock_market_data):
        ...
        prices = {symbol: data["close"].iloc[-1] for symbol, data in mock_market_data.items()}
...
class TestMLEnhancedAllocator:
    ...
    def test_signal_integration(self):
        ...
        # Create signals
        signals = {"AAPL": 0.8, "MSFT": -0.5, "GOOGL": 0.3}  # Strong buy  # Sell  # Weak buy
        ...
        positions = allocator.allocate(universe, market_data, signals=signals, capital=capital)
...
def test_integration_flow():
    ...
    capital = 100000
    target_positions = {symbol: weight * capital for symbol, weight in weights.items()}

    # Check rebalancing need
    current_positions = {"AAPL": 40000, "MSFT": 35000, "GOOGL": 25000}
    ...
    costs = cost_model.estimate_rebalancing_cost(current_positions, target_positions, prices)
