"""
Enhanced baseline perpetuals strategy with Week 2 features.

Includes:
- Market condition filters (spread, depth, volume)
- RSI confirmation for MA crossover signals
- Liquidation distance and slippage impact guards
...
class Decision:
    ...
    rejection_type: Optional[str] = None  # 'spread', 'depth', 'volume', 'rsi', 'liq', 'slippage'
...
class StrategyConfig:
    ...
    # Filters and guards
    filters_config: Optional[StrategyFiltersConfig] = None
...
class EnhancedPerpsStrategy:
    """
    Enhanced MA crossover strategy with Week 2 features:
    - Market condition filtering
    - RSI confirmation
    - Risk guards for liquidation and slippage
    ...
    def __init__(
        ...
        logger.info(f"EnhancedPerpsStrategy initialized with filters: {self.config.filters_config}")
    ...
    def decide(
        ...
        return Decision(
            action=Action.HOLD,
            reason=f"No signal (MA diff: {ma_diff:.2f}, eps: {eps:.2f})"
        )
    ...
    def _check_trailing_stop(
        ...
        if symbol not in self.trailing_stops:
            ...
            self.trailing_stops[symbol] = (current_price, current_price * (1 - self.config.trailing_stop_pct))
            return False
    ...
    def _record_rejection(self, rejection_type: str, symbol: str):
        """Record rejection metrics."""
        if rejection_type in self.rejection_counts:
            self.rejection_counts[rejection_type] += 1
        
        if self.event_store:
            self.event_store.append_metric(self.bot_id, {
    ...
    def _record_acceptance(self, symbol: str):
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        total_rejections = sum(
            v for k, v in self.rejection_counts.items()
            if k != 'entries_accepted'
        )
