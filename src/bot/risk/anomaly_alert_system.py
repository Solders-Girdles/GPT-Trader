...
class AlertConfig:
    ...
    enabled_channels: list[AlertChannel] = field(
        default_factory=lambda: [AlertChannel.LOG, AlertChannel.DASHBOARD]
    )
    ...
    severity_thresholds: dict[str, float] = field(
        default_factory=lambda: {"info": 0.3, "warning": 0.5, "error": 0.7, "critical": 0.9}
    )
...
class AlertGenerator:
    ...
    def generate_anomaly_alert(self, anomaly: Any) -> Alert | None:
        """
...
class AnomalyVisualizer:
    ...
    def plot_anomaly_timeline(
        ...
        if anomaly_times:
            ax1.scatter(
                anomaly_times, anomaly_values, color="red", s=50, zorder=5, label="Anomalies"
            )
    ...
    def plot_alert_heatmap(self, alerts: list[Alert], save_path: str | None = None):
        """
        ...
        pivot = df.pivot_table(
            index="hour", columns="severity", values="day", aggfunc="count", fill_value=0
        )
    ...
    def plot_anomaly_distribution(self, anomalies: list[Any], save_path: str | None = None):
        """
    ...
    def create_dashboard_html(
        self, alerts: list[Alert], anomalies: list[Any], save_path: str = "anomaly_dashboard.html"
    ):
        """
        ...
        <div class="summary">
            ...
            <div class="card">
                ...
                <div class="metric">{len(active_alerts)}</div>
            </div>
            <div class="card">
                <div class="label">Critical Alerts</div>
                <div class="metric" style="color: #e74c3c;">{len(critical_alerts)}</div>
            </div>
            <div class="card">
                <div class="label">Avg Severity</div>
                <div class="metric">{np.mean([a.severity for a in anomalies]) if anomalies else 0:.2f}</div>
            </div>
        </div>
        """
...
class AnomalyInvestigator:
    ...
    def investigate(
        self, anomaly: Any, historical_data: pd.DataFrame, context_window: int = 100
    ) -> dict[str, Any]:
        """
        ...
        if anomaly.metric_name in historical_data.columns:
            ...
            results["context"]["percentile"] = (metric_data <= anomaly.observed_value).mean() * 100
