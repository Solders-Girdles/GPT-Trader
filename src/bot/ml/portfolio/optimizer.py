...
class MarkowitzOptimizer(BaseComponent):
    ...
    def __init__(
        ...
        if config is None:
            config = ComponentConfig(
                component_id="markowitz_optimizer", component_type="portfolio_optimizer"
            )
    ...
    def optimize(
        ...
        self.logger.info(
            f"Optimizing portfolio with {len(returns.columns)} assets, objective: {objective}"
        )
        ...
        if objective == "max_sharpe":
            ...
            weights_opt, metrics = self._optimize_max_sharpe(mean_returns, cov_matrix, constraints)
        ...
        weights_dict = {
            asset: float(weight)
            for asset, weight in zip(returns.columns, weights_opt, strict=False)
    ...
    def _solve_optimization(
        ...
        if constraints.max_positions is not None:
            ...
            constraint_list.append(
                cp.norm(weights, 1) <= constraints.max_positions * constraints.max_weight
            )
    ...
    def _optimize_max_sharpe(
        ...
        # Constraints
        constraint_list = [excess_returns @ y == 1, kappa >= 0]
    ...
    def _generate_efficient_frontier(
        ...
        sharpe_ratios = [
            ...
            for ret, risk in zip(frontier_returns, frontier_risks, strict=False)
            if risk > 0
        ]
    ...
    def backtest_allocation(
        self, returns: pd.DataFrame, weights: dict[str, float], rebalance_freq: str = "M"
    ) -> dict[str, Any]:
    ...
    def calculate_risk_metrics(
        self, returns: pd.DataFrame, weights: dict[str, float], confidence_level: float = 0.95
    ) -> dict[str, float]:
        ...
        downside_dev = (
            float(np.sqrt(np.mean(negative_returns**2))) if len(negative_returns) > 0 else 0
        )

        # Sortino ratio (using downside deviation)
        annual_return = portfolio_returns.mean() * 252
        sortino = (
            (annual_return - self.risk_free_rate) / (downside_dev * np.sqrt(252))
            if downside_dev > 0
            else 0
        )
    ...
    def _record_optimization(
        self, weights: dict[str, float], metrics: dict[str, Any], objective: str
    ):
