...
class MarketDepth:
    ...
    l1_bid_depth: Optional[Decimal] = None    # Size at best bid
    l1_ask_depth: Optional[Decimal] = None    # Size at best ask
    l10_bid_depth: Optional[Decimal] = None   # Total size top 10 bid levels
    l10_ask_depth: Optional[Decimal] = None   # Total size top 10 ask levels
...
class SlippageGuard:
    ...
    def calculate_max_quantity_for_impact(
        ...
        Simplified model: impact_bps = (order_qty / available_depth) * 10000
        So: max_qty = (target_impact_bps / 10000) * available_depth
        ...
        max_qty = effective_depth * Decimal(str(impact_ratio))
    ...
    def should_reject_order(
        ...
        max_qty = self.calculate_max_quantity_for_impact(
        ...
        if quantity > max_qty:
            ...
            if effective_depth > 0:
                ...
                return True, (
                    f"Order qty {quantity} would cause {impact_bps:.1f}bps impact "
                    f"(max {self.config.max_impact_bps}bps). Max safe qty: {max_qty}"
                )
            else:
    ...
    def get_safe_quantity(
        ...
        max_safe_qty = self.calculate_max_quantity_for_impact(
            depth, side, self.config.max_impact_bps
        )
        
        if max_safe_qty is None or max_safe_qty <= 0:
            return Decimal("0"), f"No safe quantity available: {reason}"
        
        return max_safe_qty, f"Reduced from {desired_quantity} to {max_safe_qty} for slippage protection"
    ...
    def log_depth_summary(self, depth: MarketDepth) -> None:
        ...
        for side in ["buy", "sell"]:
            ...
            logger.info(f"  Max safe {side} qty ({depth_type}): {max_qty}")
