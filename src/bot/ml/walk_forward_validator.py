...
from backtest.realistic_backtester import BacktestConfig, BacktestResults, RealisticBacktester
...
class FoldResult:
    ...
    performance_change: float | None = None  # vs previous fold
...
class WalkForwardResults:
    ...
    # Feature stability
    feature_importance_stability: dict[str, float] | None = None
...
class WalkForwardValidator:
    ...
    def __init__(self, config: WalkForwardConfig | None = None):
        ...
        logger.info(
            f"WalkForwardValidator initialized with {self.config.train_window} day train window"
        )
    ...
    def _validate_fold(
        ...
        self.fold_predictions[fold_number] = {"y_test": y_test, "y_pred": y_test_pred}
    ...
    def _backtest_fold(
        ...
        try:
            ...
            logger.info(
                f"Fold {fold_result.fold_number} backtest: "
    ...
    def _aggregate_results(
        self, fold_results: list[FoldResult], model_name: str
    ) -> WalkForwardResults:
        ...
        if test_accuracies:
            cv = (
                np.std(test_accuracies) / np.mean(test_accuracies)
                if np.mean(test_accuracies) > 0
                else 1
            )
    ...
    def plot_results(self, results: WalkForwardResults):
        ...
        plt.suptitle(
            f"Walk-Forward Validation Results: {results.model_name}\n"
