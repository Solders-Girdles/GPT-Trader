...
class AdaptiveRiskManager:
    ...
    def calculate_risk_metrics(
        self, portfolio_snapshot: PortfolioSnapshot, tier_config: TierConfig
    ) -> dict[str, float]:
        """
        ...
        # Tier compliance
        metrics["tier_compliant"] = self._check_tier_compliance(portfolio_snapshot, tier_config)
    ...
    def check_position_size_limits(
        self, position_value: float, total_portfolio_value: float, tier_config: TierConfig
    ) -> tuple[bool, str]:
        """
    ...
    def calculate_position_size(
        ...
        base_position_size = float(total_portfolio_value) / max(target_positions, 1)

        # Adjust for confidence
        confidence_adjusted_size = base_position_size * float(confidence)
        ...
        raw_max_pct = self.config.validation.get("max_position_size_pct", 25.0)
        max_position_pct = float(raw_max_pct)
        max_size = float(total_portfolio_value) * max_position_pct / 100
    ...
    def check_trading_frequency_limits(
        self, trades_this_week: int, tier_config: TierConfig
    ) -> tuple[bool, str]:
        """
    ...
    def calculate_stop_loss_price(
        self, entry_price: float, tier_config: TierConfig, position_direction: str = "LONG"
    ) -> float:
        """
    ...
    def assess_portfolio_risk_level(
        ...
        daily_risk_utilization = (
            abs(portfolio_snapshot.daily_pnl_pct) / tier_config.risk.daily_limit_pct
        )
    ...
    def _calculate_avg_position_size(
        self, positions: list[PositionInfo], total_value: float
    ) -> float:
