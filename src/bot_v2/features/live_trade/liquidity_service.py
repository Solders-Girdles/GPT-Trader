...
class ImpactEstimate:
    ...
    max_slice_size: Optional[Decimal]
    use_post_only: bool
...
class LiquidityMetrics:
    ...
    def add_trade(self, price: Decimal, size: Decimal, timestamp: Optional[datetime] = None):
    ...
    def add_spread(self, spread_bps: Decimal, timestamp: Optional[datetime] = None):
        """Add spread data."""
        if timestamp is None:
            timestamp = datetime.now()
    ...
    def get_volume_metrics(self) -> Dict[str, Decimal]:
        ...
        volumes = {'1m': Decimal('0'), '5m': Decimal('0'), '15m': Decimal('0')}
        windows = {'1m': 1, '5m': 5, '15m': 15}
        
        for window_name, minutes in windows.items():
            cutoff = now - timedelta(minutes=minutes)
            window_volume = sum(
                volume for timestamp, volume in self._volume_data
                if timestamp >= cutoff
            )
        ...
        # Trade metrics
        trades_15m = [
            (ts, size) for ts, price, size in self._trade_data
        ...
        avg_trade_size = Decimal('0')
        if trade_count > 0:
            total_size = sum(size for _, size in trades_15m)
            avg_trade_size = total_size / trade_count
    ...
    def get_spread_metrics(self) -> Dict[str, Decimal]:
        ...
        recent_spreads = [
            spread for timestamp, spread in self._spread_data
            if timestamp >= cutoff
        ]
        ...
        return {
            'avg_spread_bps': sum(recent_spreads) / len(recent_spreads),
    ...
    def _clean_old_data(self):
        ...
        self._volume_data = [
            (ts, vol) for ts, vol in self._volume_data if ts >= cutoff
        ]
        self._spread_data = [
            (ts, spread) for ts, spread in self._spread_data if ts >= cutoff
        ]
        self._trade_data = [
...
class LiquidityService:
    ...
    def analyze_order_book(
        ...
        # Imbalance metrics
        total_l1_size = bid_size + ask_size
        bid_ask_ratio = bid_size / ask_size if ask_size > 0 else Decimal('999')
        
        total_depth_5 = bid_depth_5 + ask_depth_5
        depth_imbalance = ((bid_depth_5 - ask_depth_5) / total_depth_5) if total_depth_5 > 0 else Decimal('0')
        ...
        liquidity_score = sum(score_components.values()) / len(score_components)
    ...
    def estimate_market_impact(
        ...
        side: str,
        quantity: Decimal,
        book_data: Optional[Tuple[List[Tuple[Decimal, Decimal]], List[Tuple[Decimal, Decimal]]]] = None
    ) -> ImpactEstimate:
        """
        ...
        use_post_only = (
            analysis.condition in [LiquidityCondition.FAIR, LiquidityCondition.POOR, LiquidityCondition.CRITICAL] or
            final_impact_bps > self.max_impact_bps / 2
        )
    ...
    def get_liquidity_snapshot(self, symbol: str) -> Optional[Dict]:
    ...
    def _score_depth(self, depth_usd: Decimal, mid_price: Decimal) -> Decimal:
        ...
        depth_score = min(depth_usd / 10000, Decimal('1')) * 100  # $10k = 100 score
        return depth_score
    
    def _score_imbalance(self, imbalance: Decimal) -> Decimal:
        """Score imbalance component (0-100, lower imbalance = better)."""
        return max(Decimal('0'), 100 - imbalance * 200)  # 50% imbalance = 0 score
...
async def create_liquidity_service(**kwargs) -> LiquidityService:
