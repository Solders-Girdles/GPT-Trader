...
import re
from typing import Any, Optional, Union, List, Callable, Type, Dict
...
class TypeValidator(Validator):
    ...
    def validate(self, value: Any, field_name: str = "value") -> Any:
        if not isinstance(value, self.expected_type):
            msg = self.error_message or f"{field_name} must be of type {self.expected_type.__name__}"
...
class DateValidator(Validator):
    ...
    def validate(self, value: Any, field_name: str = "date") -> Union[date, datetime]:
        ...
        if isinstance(value, str):
            try:
                ...
                raise ValidationError(
                    f"{field_name} must be a valid date/datetime",
                    field=field_name,
                    value=value
                )
        ...
        if self.min_date and value < self.min_date:
            raise ValidationError(
                f"{field_name} must be after {self.min_date}",
                field=field_name,
                value=value
            )
        ...
        if self.max_date and value > self.max_date:
            raise ValidationError(
                f"{field_name} must be before {self.max_date}",
                field=field_name,
                value=value
            )
...
class PositiveNumberValidator(Validator):
    ...
    def __init__(self, allow_zero: bool = False, error_message: Optional[str] = None):
        super().__init__(error_message)
...
class PercentageValidator(Validator):
    ...
    def __init__(self, as_decimal: bool = True, error_message: Optional[str] = None):
        super().__init__(error_message)
...
class OHLCDataValidator(DataFrameValidator):
    ...
    def validate(self, value: Any, field_name: str = "ohlc_data") -> pd.DataFrame:
        ...
        if (df[['Open', 'High', 'Low', 'Close', 'Volume']] < 0).any().any():
            raise ValidationError(
                f"{field_name} contains negative values",
                field=field_name
            )
