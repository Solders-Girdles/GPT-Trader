...
if TORCH_AVAILABLE:
    ...
    class PolicyNetwork(nn.Module):
        ...
        def get_action(self, state: torch.Tensor) -> Tuple[int, torch.Tensor, torch.Tensor, Optional[torch.Tensor]]:
    ...
    class ContinuousPolicyNetwork(nn.Module):
        ...
        def __init__(self, config: PolicyGradientConfig):
            ...
            self.log_std = nn.Parameter(torch.ones(1, config.action_dim) * np.log(config.action_std_init))
        ...
        def get_action(self, state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Optional[torch.Tensor]]:
    ...
    class REINFORCE:
        ...
        def compute_advantages(self) -> torch.Tensor:
            ...
            for t in reversed(range(len(rewards))):
                ...
                advantage = td_error + self.config.discount_factor * self.config.gae_lambda * advantage
    ...
    class NaturalPolicyGradient(REINFORCE):
        ...
        def compute_fisher_vector_product(self, vector: torch.Tensor) -> torch.Tensor:
            ...
            fisher_vector_product = torch.autograd.grad(grad_vector_product, self.policy.parameters())
        ...
        def update(self) -> Dict[str, float]:
            ...
            # Compute step size
            shs = 0.5 * torch.dot(natural_gradient, self.compute_fisher_vector_product(natural_gradient))
            ...
            for param in self.policy.parameters():
                ...
                param_update = natural_gradient[param_idx:param_idx + param_length].view(param.shape)
...
def create_policy_gradient_agent(state_dim: int, action_dim: int,
                                policy_type: PolicyType = PolicyType.DISCRETE,
                                use_natural_gradient: bool = False) -> REINFORCE:
