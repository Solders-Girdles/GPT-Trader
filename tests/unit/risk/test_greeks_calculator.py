...
from src.bot.risk.greeks_calculator import (
    GreeksCalculator,
    OptionType,
    Greeks,
    OptionContract
...
class TestGreeksCalculator:
    ...
    def test_delta_calculation(self, calculator, option_params):
        ...
        call_delta = calculator.calculate_delta(
            option_type=OptionType.CALL,
            **option_params
        )
        put_delta = calculator.calculate_delta(
            option_type=OptionType.PUT,
            **option_params
        )
        ...
        assert abs((call_delta - put_delta) - np.exp(-option_params['dividend_yield'] * option_params['time_to_expiry'])) < 0.01
    ...
    def test_theta_calculation(self, calculator, option_params):
        ...
        call_theta = calculator.calculate_theta(
            option_type=OptionType.CALL,
            **option_params
        )
        put_theta = calculator.calculate_theta(
            option_type=OptionType.PUT,
            **option_params
        )
    ...
    def test_rho_calculation(self, calculator, option_params):
        ...
        call_rho = calculator.calculate_rho(
            option_type=OptionType.CALL,
            **option_params
        )
        put_rho = calculator.calculate_rho(
            option_type=OptionType.PUT,
            **option_params
        )
    ...
    def test_second_order_greeks(self, calculator, option_params):
        ...
        # Charm (dDelta/dTime)
        charm = calculator.calculate_charm(
            option_type=OptionType.CALL,
            **option_params
        )
    ...
    def test_implied_volatility_calculation(self, calculator, option_params):
        ...
        market_price = calculator.black_scholes_price(
            option_type=OptionType.CALL,
            **option_params
        )
    ...
    def test_greeks_deep_itm_otm(self, calculator, option_params):
        ...
        itm_greeks = calculator.calculate_all_greeks(
            option_type=OptionType.CALL,
            **itm_params
        )
        ...
        otm_greeks = calculator.calculate_all_greeks(
            option_type=OptionType.CALL,
            **otm_params
        )
    ...
    def test_discrete_dividend_adjustment(self, calculator, option_params):
        ...
        dividend_params['discrete_dividends'] = [
            {'amount': 2.0, 'ex_date_days': 30}
        ]
...
class TestGreeksCalculatorIntegration:
    ...
    def test_alert_on_extreme_greeks(self, mock_alert):
        ...
        greeks = calculator.calculate_and_monitor(
            option_type=OptionType.CALL,
            **params
        )
