...
if TORCH_AVAILABLE:
    ...
    class Critic(nn.Module):
        ...
        def forward(self, state: torch.Tensor, action: Optional[torch.Tensor] = None) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
            """Forward pass"""
    ...
    class A2C:
        ...
        def compute_gae(self, next_value: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
            ...
            for t in reversed(range(len(rewards))):
                ...
                delta = rewards[t] + self.config.discount_factor * next_val * (1 - dones[t]) - values[t]
                gae = delta + self.config.discount_factor * self.config.gae_lambda * gae * (1 - dones[t])
...
def create_actor_critic_agent(state_dim: int, action_dim: int,
                             algorithm: ActorCriticType = ActorCriticType.A2C,
                             continuous: bool = False) -> Union[A2C, SAC, TD3]:
