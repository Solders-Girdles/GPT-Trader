...
Usage:
- Prefer this in unit tests over MockBroker.
...
class DeterministicBroker(IBrokerage):
    ...
    def place_order(
        ...
        if status == OrderStatus.FILLED:
            self._apply_fill(symbol, side, qty, avg_fill or Decimal("0"))
        return order
    ...
    def list_orders(self, status: OrderStatus | None = None, symbol: str | None = None) -> list[Order]:
        res = self._orders
    ...
    # ---- Test helpers ----
    def seed_position(self, symbol: str, side: str, qty: Decimal, price: Decimal) -> None:
    ...
    def _apply_fill(self, symbol: str, side: OrderSide, qty: Decimal, price: Decimal) -> None:
        ...
        if existing is None:
            ...
            return
        # Simplified netting: if same side, increase qty with avg price; else reduce
        if (existing.side == "long" and side == OrderSide.BUY) or (existing.side == "short" and side == OrderSide.SELL):
            new_qty = existing.qty + qty
            new_entry = ((existing.entry_price * existing.qty) + (price * qty)) / new_qty
            self._positions[symbol] = Position(
                symbol=symbol,
                qty=new_qty,
            ...
            remaining = existing.qty - reduce_qty
            if remaining > 0:
                self._positions[symbol] = Position(
                    symbol=symbol,
                    qty=remaining,
                ...
                # Flip to the other side with leftover qty
