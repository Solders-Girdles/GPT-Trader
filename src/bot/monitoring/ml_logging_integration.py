...
from .structured_logger import SpanType, get_logger
...
class MLLoggingMixin:
    ...
    def log_metric(self, metric_name: str, value: float | int, **kwargs):
        """Log an ML metric."""
        self.logger.metric(
            ...
            with self.logger.start_span(op_name, span_type):
                ...
                try:
                    ...
                    if log_performance:
                        ...
                        if duration_ms > 1000:
                            self.logger.warning(
                                f"Slow operation {op_name} took {duration_ms:.2f}ms", **log_kwargs
                            )
                        else:
    ...
    def engineer_features(self, data: pd.DataFrame, feature_config: dict[str, Any]) -> pd.DataFrame:
        ...
        for feature_type, config in feature_config.items():
            if feature_type == "technical_indicators":
                features[f"sma_{config.get('window', 20)}"] = (
                    data["close"].rolling(window=config.get("window", 20)).mean()
                )
            elif feature_type == "volatility":
                features[f"volatility_{config.get('window', 20)}"] = (
                    data["close"].pct_change().rolling(window=config.get("window", 20)).std()
                )
...
class LoggedRiskCalculator:
    ...
    def calculate_var(
        self, portfolio_returns: np.ndarray, confidence: float = 0.95, method: str = "historical"
    ) -> dict[str, float]:
        ...
        results = {"var": float(var_value), "cvar": float(cvar_value), "confidence": confidence}
...
class LoggedBacktestEngine:
    ...
    def run_backtest(
        self, start_date: str, end_date: str, symbols: list, initial_capital: float = 100000
    ) -> dict[str, Any]:
...
def log_data_quality_check(data: pd.DataFrame, component_name: str = "data_quality"):
    ...
    if quality_metrics["missing_values"] > 0:
        logger.warning(
            ...
            operation="data_quality_check",
            attributes={
                "missing_percentage": quality_metrics["missing_values"]
    ...
    if quality_metrics["duplicate_rows"] > 0:
        logger.warning(
            ...
            operation="data_quality_check",
            attributes={
                "duplicate_percentage": quality_metrics["duplicate_rows"] / len(data) * 100
            },
        )
...
def log_model_deployment(
    model_id: str, model_metadata: dict[str, Any], component_name: str = "model_deployment"
):
