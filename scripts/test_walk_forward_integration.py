...
from src.bot.ml.walk_forward_validator import (
    create_walk_forward_validator, WalkForwardConfig
)
from src.bot.ml.model_degradation_monitor import (
    create_degradation_monitor, RetrainingTrigger
)
...
from src.bot.ml import FeatureSelectionConfig, SelectionMethod
...
def test_walk_forward_validation():
    ...
    for model_name, model in models.items():
        ...
        logger.info(f"  Mean Test Accuracy: {results.mean_test_accuracy:.3f} ± {results.std_test_accuracy:.3f}")
...
def test_model_degradation_monitoring():
    ...
    for day in range(60):
        ...
        if day % 10 == 0 and day > 0:
            ...
            logger.info(f"  Day {day}: Status={report['status']}, "
...
def test_integrated_pipeline_with_walk_forward():
    ...
    logger.info(f"  Mean Accuracy: {results.mean_test_accuracy:.3f} ± {results.std_test_accuracy:.3f}")
