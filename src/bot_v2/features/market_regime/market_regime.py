...
def detect_regime(
    symbol: str, lookback_days: int = 60, use_ensemble: bool = True
) -> RegimeAnalysis:
    """
...
def monitor_regime_changes(
    symbols: list[str],
    callback: Callable[[RegimeAlert], None] | None = None,
...
def get_regime_history(
    symbol: str, start_date: datetime | None = None, end_date: datetime | None = None
) -> RegimeHistory:
    """
...
def predict_regime_change(symbol: str, horizon_days: int = 5) -> RegimeChangePrediction:
    """
    ...
    change_prob = _calculate_change_probability(current_analysis, history, horizon_days)
...
def analyze_regime_stability(symbol: str, regime: MarketRegime | None = None) -> float:
    """
...
def _combine_regimes(
    volatility: VolatilityRegime, trend: TrendRegime, risk: RiskSentiment
) -> MarketRegime:
...
def _calculate_transition_probabilities(
    current: MarketRegime, features: RegimeFeatures
...
def _run_monitoring_check(callback: Callable | None) -> None:
...
def _calculate_returns_by_regime(
    symbol: str, regimes: list[tuple[MarketRegime, datetime, datetime]]
) -> dict[MarketRegime, float]:
...
def _calculate_sharpe_by_regime(
    returns: dict[MarketRegime, float], volatility: dict[MarketRegime, float]
) -> dict[MarketRegime, float]:
...
def _calculate_change_probability(
    analysis: RegimeAnalysis, history: RegimeHistory, horizon_days: int
) -> float:
...
def _identify_confirming_indicators(current: MarketRegime, next_regime: MarketRegime) -> list[str]:
