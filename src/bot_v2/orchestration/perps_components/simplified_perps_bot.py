"""Simplified PerpsBot using focused components.
...
logger = get_logger(__name__, component="simplified_perps_bot")


class SimplifiedPerpsBot:
    """Simplified PerpsBot using focused component services.
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize simplified PerpsBot.
        ...
        """
        # Core attributes
        self.bot_id = "simplified_perps_bot"
        ...
        self.bot_state = PerpsBotRuntimeState()
    ...
    async def start(self) -> None:
        """Start the simplified PerpsBot."""
        if self.running:
            logger.warning(
                "Simplified PerpsBot already running",
                operation="bot_start",
        ...
        self.running = True
        logger.info(
            "Starting simplified PerpsBot",
            operation="bot_start",
            bot_id=self.bot_id,
            symbol_count=len(self.symbols),
        )

        with correlation_context("simplified_perps_bot"):
            ...
            logger.info(
                "Simplified PerpsBot started successfully",
                operation="bot_ready",
                bot_id=self.bot_id,
            )

    async def stop(self) -> None:
        """Stop the simplified PerpsBot."""
        if not self.running:
            logger.warning(
                "Simplified PerpsBot not running",
                operation="bot_stop",
                status="not_running",
            )
            return
    ...
    def get_status(self) -> dict[str, Any]:
        """Get current status of the simplified PerpsBot."""
        return {
            "bot_id": self.bot_id,
