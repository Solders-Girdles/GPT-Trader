...
if TYPE_CHECKING:  # pragma: no cover - typing only
    ...
    from pandas import Timestamp as _PandasTimestamp

    DataFrame: TypeAlias = _PandasDataFrame
    Timestamp: TypeAlias = _PandasTimestamp
    HAS_PANDAS = True
else:
...
class AdaptivePortfolioManager:
    ...
    def analyze_portfolio(
        ...
        risk_metrics = self.risk_manager.calculate_risk_metrics(portfolio_snapshot, tier_config)
        ...
        warnings = self._generate_warnings(portfolio_snapshot, tier_config, risk_metrics)
    ...
    def run_adaptive_backtest(
        self, symbols: list[str], start_date: str, end_date: str, initial_capital: float = 1000
    ) -> BacktestMetrics:
        """
        ...
        for symbol in symbols:
            try:
                df = self.data_provider.get_historical_data(
                    symbol, period=f"{period_days}d", interval="1d"
                )
        ...
        for current_date in dates:
            ...
            current_capital, positions = self._update_positions(positions, data, current_date)
                ...
                tier_transitions.append(
                    {
                        ...
                        "to_tier": (
                            result.tier_transition_target.value
                            if result.tier_transition_target
                            else None
                        ),
            ...
            # Execute signals
            new_trades = self._execute_signals(result.signals, data, current_date, current_capital)
    ...
    def _create_portfolio_snapshot(
        self, total_value: float, positions: list[PositionInfo], current_tier: PortfolioTier
    ) -> PortfolioSnapshot:
        ...
        sector_exposures: dict[str, float] = {}  # Would need sector mapping in real implementation
    ...
    def _generate_tier_appropriate_signals(
        ...
        return self.strategy_selector.generate_signals(tier_config, portfolio_snapshot, market_data)
    ...
    def _generate_recommendations(
        ...
        # Cash allocation
        cash_pct = (
            portfolio_snapshot.cash / portfolio_snapshot.total_value * 100
        ...
        if risk_metrics.get("daily_risk_pct", 0) > tier_config.risk.daily_limit_pct:
            recommendations.append(
                "Daily risk exceeds tier limits - consider reducing position sizes"
            )
    ...
    def _update_positions(
        self, positions: list[PositionInfo], data: dict[str, DataFrame], current_date: Timestamp
    ) -> tuple[float, list[PositionInfo]]:
        """Update position values with current market prices."""

        updated_positions: list[PositionInfo] = []
        total_value = 0.0

        for pos in positions:
            if pos.symbol in data and current_date in data[pos.symbol].index:
                price_raw = data[pos.symbol].loc[current_date, "Close"]
                try:
                    current_price = float(price_raw)  # type: ignore[arg-type]
                except (TypeError, ValueError):
                    continue

                position_value = float(pos.shares) * current_price
                entry_value = float(pos.shares) * pos.entry_price
                unrealized_pnl = position_value - entry_value
                unrealized_pnl_pct = (unrealized_pnl / entry_value * 100) if entry_value else 0.0
        ...
        for signal in signals:
            if signal.action == "BUY" and signal.symbol in data:
                if current_date in data[signal.symbol].index:
                    price_raw = data[signal.symbol].loc[current_date, "Close"]
                    try:
    ...
    def _update_positions_with_trades(
        self, positions: list[PositionInfo], new_trades: list[dict[str, Any]]
    ) -> list[PositionInfo]:
    ...
    def _calculate_backtest_metrics(
        ...
        if not daily_values:
            return BacktestMetrics(
                ...
                total_trades=0,
                tier_transitions=0,
                final_tier=PortfolioTier.MICRO,
                tier_performance={},
            )
        ...
        final_tier = (
            PortfolioTier(daily_values[-1]["tier"]) if daily_values else PortfolioTier.MICRO
        )
