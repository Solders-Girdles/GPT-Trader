...
import pandas as pd
import pytest
from bot.portfolio.allocator import PortfolioRules, _to_float, allocate_signals, position_size
...
class TestPositionSize:
    ...
    def test_position_size_with_cost_adjustment(self):
        ...
        rules = PortfolioRules(
            per_trade_risk_pct=0.01, cost_adjusted_sizing=True, cost_bps=20.0, slippage_bps=10.0
        )
...
class TestAllocateSignals:
    ...
    def test_allocate_signals_max_positions(self):
        ...
        for i in range(5):
            ...
            signals[symbol] = pd.DataFrame(
                {"Close": [100 + i], "donchian_upper": [99 + i], "atr": [2.0], "signal": [1.0]}
                * 10,
                index=dates,
            )
    ...
    def test_allocate_signals_gross_exposure_limit(self):
        ...
        total_exposure = sum(qty * 100 for qty in allocations.values())  # qty * price
    ...
    def test_allocate_signals_missing_columns(self, default_rules):
        ...
        signals = {
            "AAPL": pd.DataFrame(
                {"Close": [100] * 10, "donchian_upper": [99] * 10, "signal": [1.0] * 10},
                index=dates,
            )
        }
...
class TestIntegrationScenarios:
    ...
    def test_full_portfolio_allocation(self, market_data):
        ...
        rules = PortfolioRules(
            per_trade_risk_pct=0.01, max_positions=3, max_gross_exposure_pct=0.80
        )
    ...
    def test_risk_parity_allocation(self):
        ...
        rules = PortfolioRules(per_trade_risk_pct=0.01, atr_k=1.0)  # Use 1 ATR for equal risk
    ...
    def test_turnover_constraint(self):
        ...
        rules = PortfolioRules(max_turnover_per_rebalance=0.20)  # 20% max turnover
