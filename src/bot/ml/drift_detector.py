...
class DriftDetectorConfig:
    ...
    severity_thresholds: dict[str, float] = field(
        default_factory=lambda: {"low": 0.01, "medium": 0.05, "high": 0.1, "critical": 0.2}
    )
...
class ConceptDriftDetector:
    ...
    def __init__(self, config: DriftDetectorConfig):
        ...
        self.ph_detector = PageHinkleyDetector(config.min_instances, config.threshold, config.alpha)
    ...
    def _detect_drift(self) -> DriftDetection | None:
        ...
        if len(self.performance_history) > 1:
            ...
            if adwin_drift:
                drift_detections.append({"method": "ADWIN", "confidence": 0.95, "score": 0.8})
        ...
        if len(self.performance_history) > self.config.min_instances:
            ...
            if ph_drift:
                drift_detections.append({"method": "Page-Hinkley", "confidence": 0.9, "score": 0.7})
    ...
    def _detect_performance_drift(self) -> dict | None:
    ...
    def _combine_drift_detections(self, detections: list[dict]) -> DriftDetection:
        ...
        logger.warning(
            f"Concept drift detected: {severity.value} severity, "
    ...
    def _update_feature_statistics(self, features: pd.Series):
        ...
        for feature, value in features.items():
            if pd.notna(value) and isinstance(value, (int, float)):
                ...
                if stats["count"] > 1:
                    variance = (stats["sum_sq"] - stats["sum"] * stats["mean"]) / (
                        stats["count"] - 1
                    )
    ...
    def reset(self):
        ...
        self.ph_detector = PageHinkleyDetector(
            self.config.min_instances, self.config.threshold, self.config.alpha
        )
...
SENSITIVE_DETECTOR = DriftDetectorConfig(
    delta=0.001, threshold=30.0, performance_threshold=0.02, p_value_threshold=0.01
)

ROBUST_DETECTOR = DriftDetectorConfig(
    delta=0.01, threshold=100.0, performance_threshold=0.1, p_value_threshold=0.001
)
