...
class DashboardData:
    ...
    def get_performance_history(self, days: int = 30) -> pd.DataFrame:
        ...
        cumulative_return = pd.Series(range(days)).apply(
            lambda x: 100000 * (1 + 0.001 * x + 0.002 * pd.Series(range(days)).sample(1).values[0])
        )
...
def render_portfolio_overview(data_provider: DashboardData):
    ...
    with col2:
        st.metric(
            "Daily P&L", f"${metrics['daily_pnl']:+,.2f}", f"{metrics['daily_pnl_pct']:+.2f}%"
        )
...
def render_risk_metrics(data_provider: DashboardData):
    ...
    alerts = [
        {
            "Level": "⚠️ Warning",
            "Message": "TSLA position exceeds 15% of portfolio",
            "Time": "10 min ago",
        },
