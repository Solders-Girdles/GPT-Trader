...
class SimpleOnlineLearningPipeline:
    ...
    def __init__(self, config: OnlineLearningConfig):
        ...
        logger.info(
            f"Initialized simple online learning pipeline with {config.learning_mode.value} mode"
        )
    ...
    def _internal_update(self, features, target, force_update):
        ...
        if self._is_classification_task([encoded_target]):
            try:
                loss = (
                    log_loss([encoded_target], [prediction])
                    if 0 < prediction < 1
                    else abs(prediction - encoded_target)
                )
            except:
    ...
    def _is_classification_task(self, targets) -> bool:
        ...
        return len(unique_targets) <= 10 and all(
            isinstance(t, (int, np.integer)) or t in [0, 1] for t in unique_targets
        )
...
# Factory function
def create_simple_online_learning_pipeline(
