...
class MLEnhancedStrategy(Strategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        if self.use_strategy_selection:
            ...
            ensemble_info = self.strategy_selector.create_ensemble_strategy(
                current_features, threshold=self.ensemble_threshold
            )
            ...
            if ensemble_info["type"] == "ensemble":
                ...
                if strategy_name in self.strategies:
                    ...
                    self.logger.info(
                        f"Using strategy: {strategy_name} (confidence: {strategy_confidence:.1%})"
                    )
                else:
    ...
    def analyze_performance(self) -> dict[str, Any]:
        ...
        analysis = {
            ...
            "ensemble_usage_rate": sum(
                1 for r in self.strategy_history if r["strategy"] == "ensemble"
            )
            / len(self.strategy_history),
        }
