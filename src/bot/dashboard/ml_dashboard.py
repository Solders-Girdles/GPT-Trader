...
def render_overview_page():
    ...
    with col1:
        ...
        for model, status in models.items():
            ...
            st.markdown(f'<div class="metric-card">📈 {model}: <span class="{status_class}">{status}</span></div>', 
                       unsafe_allow_html=True)
    ...
    with col2:
        ...
        for item, status in trading_status.items():
            ...
            st.markdown(f'<div class="metric-card">⚡ {item}: <span style="color: {color}">{status}</span></div>', 
                       unsafe_allow_html=True)
    ...
    with col3:
        ...
        for item, value in market_info.items():
            st.markdown(f'<div class="metric-card">📊 {item}: <b>{value}</b></div>', 
                       unsafe_allow_html=True)
    ...
    activity_data = pd.DataFrame({
        ...
        'Event': ['Regime Change', 'Strategy Switch', 'Trade Executed', 'Rebalancing', 
                 'Risk Alert', 'Trade Executed', 'Model Update', 'Trade Executed',
                 'Strategy Switch', 'Market Close'],
        'Details': ['Bull Quiet → Bull Volatile', 'Momentum → Trend Following', 'Buy AAPL 100 shares',
                   '5 positions adjusted', 'Volatility spike detected', 'Sell MSFT 50 shares',
                   'Regime detector retrained', 'Buy GOOGL 25 shares', 'Trend Following → Mean Reversion',
                   'Trading halted'],
        'Impact': ['Medium', 'High', 'Low', 'Medium', 'High', 'Low', 'Low', 'Low', 'Medium', 'N/A']
    })
...
def render_live_trading_page():
    ...
    orders_df = pd.DataFrame({
        ...
        'Status': ['FILLED', 'FILLED', 'FILLED', 'PENDING', 'FILLED', 'CANCELLED', 'FILLED', 'PENDING']
    })
    ...
    st.dataframe(
        orders_df.style.applymap(color_status, subset=['Status']).format({
            'Price': '${:.2f}'
        }),
        use_container_width=True,
        height=300
