...
class BollingerBandsModel(BaselineModel):
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        upper, middle, lower = talib.BBANDS(
            prices, timeperiod=self.period, nbdevup=self.num_std, nbdevdn=self.num_std, matype=0
        )
...
class MACDModel(BaselineModel):
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        macd, macd_signal, macd_hist = talib.MACD(
            prices, fastperiod=self.fast, slowperiod=self.slow, signalperiod=self.signal
        )
...
class EnsembleBaselineModel(BaselineModel):
    ...
    def __init__(self, models: list[BaselineModel] | None = None):
        super().__init__("EnsembleBaseline")
        if models is None:
            self.models = [SMAcrossoverModel(), RSIModel(), MomentumModel(), BollingerBandsModel()]
        else:
            self.models = models
...
class AdaptiveBaselineModel(BaselineModel):
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        if high_vol_mask.any():
            predictions[high_vol_mask] = self.range_model.predict(X[high_vol_mask])

        # Low volatility: use momentum
        if low_vol_mask.any():
            predictions[low_vol_mask] = self.trend_model.predict(X[low_vol_mask])
