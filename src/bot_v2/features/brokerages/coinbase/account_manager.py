...
class CoinbaseAccountManager:
    ...
    def snapshot(self, emit_metric: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            logger.log_event(LogLevel.WARNING, 'account_snapshot', 'key permissions unavailable', error=str(exc))
            out['key_permissions'] = {}
        try:
    ...
    def convert(
        ...
        if self.event_store is not None:
            try:
                self.event_store.append_metric(
                    bot_id='coinbase_account',
                    metrics={'event_type': 'convert_commit', 'trade_id': trade_id, 'quote': quote, 'result': commit_result}
                )
            except Exception as exc:  # pragma: no cover - defensive telemetry guard
                logger.log_event(
                    LogLevel.DEBUG,
    ...
    def move_funds(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.event_store is not None:
            try:
                self.event_store.append_metric(
                    bot_id='coinbase_account',
                    metrics={'event_type': 'portfolio_move', **payload}
                )
            except Exception as exc:  # pragma: no cover - defensive telemetry guard
                logger.log_event(
                    LogLevel.DEBUG,
