...
class UnifiedMonitor:
    ...
    def _collect_metrics(self) -> None:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            """,
                ("monitor", "portfolio_metrics", metrics.get("portfolio_value", 0), str(metrics)),
            )
    ...
    def _create_alert(self, event_type: str, severity: str, message: str) -> None:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            cursor = conn.execute(
                ...
                AND created_at > datetime('now', '-{self.config.alert_cooldown} seconds')
            """,
                (event_type, message),
            )
    ...
    def get_health_status(self) -> dict[str, Any]:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            components = {}
            for row in cursor:
                components[row[0]] = {"status": row[1], "health": row[2], "last_update": row[3]}
