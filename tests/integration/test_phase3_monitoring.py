...
class TestAdvancedDegradationDetector:
    ...
    def test_update_baseline(self, detector, sample_data):
        ...
        detector.update_baseline(
            features[:100], predictions[:100], actuals[:100], confidences[:100]
        )
    ...
    def test_comprehensive_degradation_check(self, detector, sample_data):
        ...
        metrics = detector.check_degradation(
            features[100:150], predictions[100:150], actuals[100:150], confidences[100:150]
        )
    ...
    def test_degradation_type_determination(self, detector, sample_data):
        ...
        metrics = detector.check_degradation(
            drifted_features, predictions[100:150], actuals[100:150], confidences[100:150]
        )

        # Should detect feature drift
        assert metrics.status in [DegradationType.FEATURE_DRIFT, DegradationType.CONCEPT_DRIFT]
    ...
    def test_alert_generation(self, detector, sample_data):
        ...
        # Check degradation
        metrics = detector.check_degradation(
            features[100:150], bad_predictions, bad_actuals, low_confidences
        )
...
class TestIntegrationScenarios:
    ...
    def test_gradual_degradation_scenario(self, detector):
        ...
        baseline_features = pd.DataFrame(
            {"f1": np.random.normal(0, 1, batch_size), "f2": np.random.normal(0, 1, batch_size)}
        )
        ...
        detector.update_baseline(baseline_features, baseline_pred, baseline_actual, baseline_conf)
    ...
    def test_sudden_drift_scenario(self, detector):
        ...
        for i in range(5):
            ...
            predictions = np.random.choice([0, 1], batch_size)
            actuals = predictions.copy()
            actuals[np.random.choice(batch_size, 10, replace=False)] = (
                1 - actuals[np.random.choice(batch_size, 10, replace=False)]
            )
