...
class TestTrendBreakoutStrategy:
    ...
    def test_donchian_channels(self, strategy, sample_data):
        ...
        if valid_idx.any():
            ...
            assert (result.loc[valid_idx, 'donchian_upper'] >= 
                   sample_data.loc[valid_idx, 'Low']).all()
            
            # Lower channel should be at or below recent highs
            assert (result.loc[valid_idx, 'donchian_lower'] <= 
                   sample_data.loc[valid_idx, 'High']).all()
    ...
    @pytest.mark.parametrize("lookback,atr_period,atr_k", [
        (20, 14, 1.0),
        (55, 20, 2.0),
        (100, 30, 3.0),
        (10, 5, 0.5)
