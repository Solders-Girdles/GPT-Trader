...
def _make_logger(monkeypatch: pytest.MonkeyPatch, *, enable_console: bool = False) -> tuple[ProductionLogger, list[dict[str, Any]]]:
    ...
    logger._py_json_logger = DummyLogger()  # type: ignore[attr-defined]
...
def test_log_event_respects_min_level(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert emitted == []
    assert logger._recent_logs == []  # noqa: SLF001
...
def test_log_trade_and_correlation_id(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    logger.log_trade("buy", "BTC-USD", quantity=1.0, price=50000.0, strategy="default", success=False)
...
def test_recent_logs_buffer_trims(monkeypatch: pytest.MonkeyPatch) -> None:
    logger, emitted = _make_logger(monkeypatch)
    logger._max_recent_logs = 3  # noqa: SLF001

    for idx in range(5):
        logger.log_event(LogLevel.INFO, "heartbeat", f"event-{idx}")

    assert len(logger._recent_logs) == 3  # noqa: SLF001
    assert logger._recent_logs[0]["message"] == "event-2"  # noqa: SLF001
    assert len(emitted) == 5


def test_log_performance_console_output(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_auth_pnl_funding_and_market_logs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    logger.log_pnl("BTC-USD", realized_pnl=10.5, unrealized_pnl=-2.0, fees=0.1, funding=0.2, position_size=0.3)
    logger.log_funding("BTC-USD", funding_rate=0.0001, payment=-0.5, period_start="t0", period_end="t1")
    logger.log_market_heartbeat("coinbase", last_update_ts="2025-01-15T12:00:00Z", latency_ms=12.3, staleness_ms=5.6, threshold_ms=50)
...
def test_correlation_generation_and_counts(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert logger._log_count >= 1  # noqa: SLF001
    assert logger._total_log_time > 0  # noqa: SLF001
