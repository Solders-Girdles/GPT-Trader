...
class MomentumStrategy(OptimizableStrategy):
    ...
    def __init__(self, lookback: int = 20, threshold: float = 0.02, hold_period: int = 5, **kwargs):
...
class MeanReversionStrategy(OptimizableStrategy):
    ...
    def __init__(self, period: int = 20, entry_std: float = 2.0, exit_std: float = 0.5, **kwargs):
...
class VolatilityStrategy(OptimizableStrategy):
    ...
    def __init__(self, vol_period: int = 20, vol_threshold: float = 0.02, 
                 trend_period: int = 50, **kwargs):
        super().__init__(vol_period=vol_period, vol_threshold=vol_threshold, 
                        trend_period=trend_period, **kwargs)
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        # Entry signals
        signals[low_vol & uptrend & (uptrend.shift(1) == False)] = 1
        signals[low_vol & downtrend & (downtrend.shift(1) == False)] = -1
...
class BreakoutStrategy(OptimizableStrategy):
    ...
    def __init__(self, lookback: int = 20, confirm_bars: int = 2, 
                 stop_loss: float = 0.02, **kwargs):
        super().__init__(lookback=lookback, confirm_bars=confirm_bars, 
                        stop_loss=stop_loss, **kwargs)
        self.lookback = lookback
