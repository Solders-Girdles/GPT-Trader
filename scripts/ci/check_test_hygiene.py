...
ALLOWLIST = {
    ...
    "tests/integration/test_canary_profile.py",
    ...
    "tests/unit/bot_v2/orchestration/test_ml_integration.py",
    "tests/unit/backtest/test_spot_backtester.py",
    ...
    "tests/unit/bot_v2/test_data_provider.py",
}
...
def scan(paths: Sequence[str]) -> int:
    ...
    for path in normalized:
        ...
        if line_count > THRESHOLD and str(rel) not in ALLOWLIST:
            problems.append(
                f"{rel} exceeds {THRESHOLD} lines ({line_count}). Split into smaller modules or add to allowlist with justification."
            )
