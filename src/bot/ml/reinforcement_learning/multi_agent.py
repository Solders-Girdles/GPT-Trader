    ...
    class CommunicationNetwork(nn.Module):
        ...
        def __init__(self, config: MultiAgentConfig):
            ...
            if config.communication_type == CommunicationType.BROADCAST:
                self.aggregator = nn.GRU(config.message_dim, config.message_dim, batch_first=True)
            elif config.communication_type == CommunicationType.GRAPH:
                self.graph_attention = nn.MultiheadAttention(
                    config.message_dim, num_heads=4, batch_first=True
                )
        ...
        def forward(
            self, states: torch.Tensor, adjacency: torch.Tensor | None = None
        ) -> torch.Tensor:
            """
            ...
            if self.config.communication_type == CommunicationType.BROADCAST:
                ...
                self.config.communication_type == CommunicationType.GRAPH and adjacency is not None
            ):
                # Graph-based communication
                aggregated, _ = self.graph_attention(
                    messages, messages, messages, attn_mask=adjacency
                )
            else:
                # No aggregation
                aggregated = messages
    ...
    class MADDPG:
        ...
        def __init__(self, config: MultiAgentConfig):
            ...
            for i in range(self.n_agents):
                ...
                critic_input_dim = (
                    config.state_dim * self.n_agents + config.action_dim * self.n_agents
                )
            ...
            self.actor_optimizers = [
                optim.Adam(actor.parameters(), lr=config.learning_rate) for actor in self.actors
            ]
            self.critic_optimizers = [
                optim.Adam(critic.parameters(), lr=config.learning_rate) for critic in self.critics
            ]
        ...
        def update(self) -> dict[str, float]:
            ...
            rewards_batch = [
                torch.FloatTensor(r).unsqueeze(1).to(self.device) for r in rewards_batch
            ]
            ...
            for i in range(self.n_agents):
                ...
                with torch.no_grad():
                    ...
                    target_q = (
                        rewards_batch[i]
                        + self.config.discount_factor * (1 - dones_batch) * target_q
                    )
                ...
                actor_loss = -self.critics[i](
                    torch.cat([all_states, new_all_actions], dim=1)
                ).mean()
                ...
                for target_param, param in zip(
                    self.target_actors[i].parameters(), self.actors[i].parameters(), strict=False
                ):
                ...
                for target_param, param in zip(
                    self.target_critics[i].parameters(), self.critics[i].parameters(), strict=False
                ):
                    target_param.data.copy_(
                        self.config.tau * param.data + (1 - self.config.tau) * target_param.data
                    )
            ...
            return {"critic_loss": np.mean(critic_losses), "actor_loss": np.mean(actor_losses)}
            ...
            for i in range(self.n_agents):
                ...
                agent_features = np.array(
                    [
                        ...
                        (self.agent_cash[i] + self.agent_positions[i] * self.price)
...
def create_multi_agent_system(n_agents: int = 4, algorithm: str = "maddpg") -> MADDPG | None:
    """Factory function to create multi-agent system"""

    config = MultiAgentConfig(n_agents=n_agents, algorithm=algorithm)
...
if __name__ == "__main__":
    ...
    if TORCH_AVAILABLE:
        ...
        config = MultiAgentConfig(n_agents=4, state_dim=50, action_dim=3)
        ...
        comm_config = MultiAgentConfig(n_agents=4, communication_type=CommunicationType.BROADCAST)
