...
class WorkflowTracker:
    ...
    def register_workflow(self, workflow_name: str, expected_steps: int, description: str = ""):
    ...
    def start_workflow(self, workflow_name: str, context: Optional[Dict] = None) -> str:
    ...
    def track_step(self, execution_id: str, step_name: str, status: str, 
        ...
        total_duration = sum(
            step['duration'] for step in self.step_timings[execution_id].values()
        )
    ...
    def update_context(self, execution_id: str, context_updates: Dict[str, Any]):
        """Update the execution context with new data"""
        if execution_id in self.executions:
            self.executions[execution_id].context.update(context_updates)
    
    def complete_workflow(self, execution_id: str, status: WorkflowStatus, 
    ...
    def get_execution_report(self, execution_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'duration': execution.metrics.get('total_duration', execution.metrics.get('current_duration', 0)),
            'steps': {
    ...
    def get_workflow_stats(self, workflow_name: str) -> Dict[str, Any]:
        ...
        avg_duration = sum(
            e.metrics.get('total_duration', 0) for e in relevant_executions
        ) / total_executions
    ...
    def get_active_workflows(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            for execution_id in self.executions.keys()
        ]
    ...
    def cleanup_old_data(self, max_age_days: int = 7):
        ...
        self.execution_history = [
            e for e in self.execution_history 
            if e.started_at.timestamp() > cutoff_date
        ]