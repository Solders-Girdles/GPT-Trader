...
class TradingEngine(BaseEngine):
    ...
    async def execute_decision(
        ...
        execution_engine: ExecutionEngine | None = self.context.execution_coordinator
        if execution_engine is None:
            raise RuntimeError("Execution engine not available for decision execution")

        # Map to new API: execute_decision(action, **kwargs)
        await execution_engine.execute_decision(
    ...
    def ensure_order_lock(self) -> asyncio.Lock:
        execution_engine: ExecutionEngine | None = self.context.execution_coordinator
        if execution_engine is None:
            raise RuntimeError("Execution engine not available to ensure order lock")
        return execution_engine.ensure_order_lock()
    ...
    async def place_order(self, **kwargs: Any) -> Order | None:
        ...
        if execution_engine is None or exec_engine is None:
            raise RuntimeError("Execution engine not initialized")
        return await execution_engine.place_order(exec_engine, **kwargs)

    async def place_order_inner(self, **kwargs: Any) -> Order | None:
        # execution_engine: ExecutionEngine | None = self.context.execution_coordinator
        # if execution_engine is None:
        #     raise RuntimeError("Execution engine not available")
        # return await execution_engine.place_order_inner(**kwargs)
