...
class PaperTradingDashboard:
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        ...
        winning_trades = sum(1 for t in self.engine.trades if hasattr(t, 'pnl') and t.pnl and t.pnl > 0)
        losing_trades = sum(1 for t in self.engine.trades if hasattr(t, 'pnl') and t.pnl and t.pnl < 0)
    ...
    def print_portfolio_summary(self, metrics: Dict) -> None:
        ...
        # Two column layout
        print(f"{'Equity:':<20} {self.format_currency(metrics['equity']):<20} "
              f"{'Returns:':<15} {self.format_pct(metrics['returns_pct'])}")
        
        print(f"{'Cash:':<20} {self.format_currency(metrics['cash']):<20} "
              f"{'Drawdown:':<15} {self.format_pct(metrics['drawdown_pct'])}")
        
        print(f"{'Positions Value:':<20} {self.format_currency(metrics['positions_value']):<20} "
    ...
    def print_positions(self) -> None:
        ...
        if not self.engine.positions:
            ...
            for symbol, pos in self.engine.positions.items():
                ...
                print(f"{symbol:<15} {pos.quantity:<12.6f} "
    ...
    def print_recent_trades(self, limit: int = 5) -> None:
        ...
        recent = self.engine.trades[-limit:] if len(self.engine.trades) > limit else self.engine.trades
        ...
        if not recent:
            ...
            for trade in reversed(recent):  # Show newest first
                ...
                print(f"{time_str:<20} {trade.symbol:<10} {trade.side:<6} "
    ...
    def display_continuous(self, duration: Optional[int] = None) -> None:
        """
    ...
    def generate_html_summary(self, output_path: Optional[str] = None) -> str:
        """
        ...
        if not output_path:
            ...
            output_path = f"results/paper_trading_summary_{self.engine.bot_id}_{timestamp}.html"
