...
class StrategyMetaSelector(MLModel):
    ...
    def __init__(
        ...
        """
        if config is None:
            config = ComponentConfig(component_id="strategy_selector", component_type="ml_model")
    ...
    def train(
        self, X: pd.DataFrame, y: pd.Series, optimize_hyperparams: bool = False
    ) -> dict[str, float]:
    ...
    def predict_proba(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        prob_df = pd.DataFrame(probs, index=X.index, columns=self.strategies)

        return prob_df

    def select_strategy_with_confidence(
        self, X: pd.DataFrame
    ...
    def analyze_strategy_performance(
        self, X: pd.DataFrame, y: pd.Series, returns: pd.Series
    ...
    def get_feature_importance_analysis(self) -> dict[str, Any]:
        ...
        for feature, importance in sorted_features:
            ...
            feature_groups[group].append({"name": feature, "importance": float(importance)})
    ...
    def create_ensemble_strategy(self, X: pd.DataFrame, threshold: float = 0.6) -> dict[str, Any]:
    ...
    def _store_predictions(self, timestamps, strategies: np.ndarray):
        ...
        for ts, strategy in zip(timestamps, strategies, strict=False):
            try:
