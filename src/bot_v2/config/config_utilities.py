...
def parse_mapping_env(
    ...
    try:
        ...
        raise ValidationError(
            f"Failed to parse {var_name}: {exc}", field=var_name, value=env_value
        ) from exc
...
def parse_decimal_env(var_name: str, default: Decimal | None = None) -> Decimal | None:
    ...
    try:
        ...
        raise ValidationError(
            f"Failed to parse {var_name} as decimal: {exc}", field=var_name, value=env_value
        ) from exc
...
def parse_int_env(var_name: str, default: int | None = None) -> int | None:
    ...
    try:
        ...
        raise ValidationError(
            f"Failed to parse {var_name} as integer: {exc}", field=var_name, value=env_value
        ) from exc
...
def parse_float_env(var_name: str, default: float | None = None) -> float | None:
    ...
    try:
        ...
        raise ValidationError(
            f"Failed to parse {var_name} as float: {exc}", field=var_name, value=env_value
        ) from exc
