...
class RealisticBacktester:
    ...
    def run(self,
            data: pd.DataFrame,
            signals: pd.Series,
            prices: Optional[pd.Series] = None) -> BacktestResults:
        """
    ...
    def _close_position(self,
        ...
        costs = self.config.costs.calculate_total_cost(
            trade_value, trade.quantity,
            is_short=(trade.side == PositionSide.SHORT)
        )
    ...
    def _calculate_equity(self, current_price: float) -> float:
        ...
        for symbol, trade in self.positions.items():
            if trade.side == PositionSide.LONG:
                ...
                # Short position
                position_value = 2 * trade.entry_price * trade.quantity - current_price * trade.quantity
...
def create_backtester(config: Optional[BacktestConfig] = None) -> RealisticBacktester:
