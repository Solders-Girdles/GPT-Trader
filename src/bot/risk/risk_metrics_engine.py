...
class RiskMetrics:
    ...
    var_995: float | None = None

    # Conditional Value at Risk (Expected Shortfall)
    cvar_95: float | None = None
    cvar_99: float | None = None
    cvar_995: float | None = None
...
class VaRCalculator:
    ...
    def calculate_marginal_var(
        self, portfolio_returns: np.ndarray, position_returns: np.ndarray, position_weight: float
    ) -> float:
        """
    ...
    def calculate_component_var(
        ...
        for i, (pos_returns, weight) in enumerate(zip(position_returns, weights, strict=False)):
            marginal_var = self.calculate_marginal_var(portfolio_returns, pos_returns, weight)
...
class ExposureAggregator:
    ...
    def calculate_exposures(self) -> dict[str, float]:
        ...
        short_exposure = abs(
            sum(p.market_value for p in self.positions if p.position_type == "short")
        )
...
class RiskMetricsEngine:
    ...
    def get_risk_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "var": {"95%": latest.var_95, "99%": latest.var_99, "99.5%": latest.var_995},
            "cvar": {"95%": latest.cvar_95, "99%": latest.cvar_99, "99.5%": latest.cvar_995},
            "risk_adjusted_returns": {
                "sharpe_ratio": latest.sharpe_ratio,
                "sortino_ratio": latest.sortino_ratio,
            },
            "drawdown": {"current": latest.current_drawdown, "maximum": latest.max_drawdown},
            "exposure": {
...
def demonstrate_risk_metrics():
    ...
    print("\nExposure Metrics:")
    print(
        f"  Gross Exposure: ${metrics.gross_exposure:,.2f} ({metrics.gross_exposure/capital:.1%})"
    )
