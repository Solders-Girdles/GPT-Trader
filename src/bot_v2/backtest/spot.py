...
class BollingerMeanReversionStrategy:
    ...
    def __init__(self, window: int = 20, num_std: Decimal | float = Decimal("2")):
        if window <= 1:
...
class SpotBacktester:
    ...
    def run(self) -> BacktestResult:
        ...
        for bar in self.bars:
            signal = self.strategy.on_bar(bar, has_position=(position_qty > 0))
            price = bar.close

            if signal == StrategySignal.BUY and position_qty == 0:
                qty = (cash / price).quantize(Decimal("0.00000001"), rounding=ROUND_HALF_UP)
                if qty > 0:
                    cost = qty * price
            ...
            elif signal == StrategySignal.SELL and position_qty > 0:
        ...
        if position_qty > 0:
            ...
            equity_records[-1]["position_qty"] = 0.0

        equity_df = pd.DataFrame(equity_records)
        metrics = _compute_metrics(equity_df, float(initial_cash), self.config.risk_free_rate)
        return BacktestResult(metrics=metrics, equity_curve=equity_df, trades=trades)
...
def _true_range(bar: Bar, prev_close: Optional[Decimal]) -> Decimal:
...
def _compute_metrics(equity_df: pd.DataFrame, initial_equity: float, risk_free_rate: float) -> BacktestMetrics:
    ...
    annualized_return = (1 + total_return) ** (365.0 / total_days) - 1 if total_return > -1 else -1.0
