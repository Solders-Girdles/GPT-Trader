...
class ProductSpec:
    ...
    def __init__(self, product_id: str, spec_data: Dict[str, Any]):
...
class SpecsService:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path is None:
            config_path = os.getenv('PERPS_SPECS_PATH', 
    ...
    def build_spec(self, product_id: str, api_data: Optional[Dict] = None) -> ProductSpec:
    ...
    def quantize_price_side_aware(self, product_id: str, side: str, 
                                 price: float) -> Decimal:
        """
    ...
    def calculate_safe_position_size(self, product_id: str, 
                                    target_notional: float,
                                    mark_price: float) -> Tuple[Decimal, str]:
        """
        ...
        if notional < spec.min_notional:
            ...
            min_size = (spec.min_notional * (Decimal('1') + spec.safe_buffer)) / Decimal(str(mark_price))
    ...
    def validate_order(self, product_id: str, side: str, order_type: str,
...
# Global instance
_specs_service: Optional[SpecsService] = None
...
def validate_order(
    *,
    ...
    # Quantize qty
    adj_qty = quantize_size(qty, step)
    if adj_qty < min_size:
        # Do not auto-bump in validator; surface failure to caller
        return ValidationResult(ok=False, reason="min_size")

    # Price handling
    adj_price: Optional[Decimal] = None
    ...
    if min_notional is not None and adj_price is not None:
        ...
        if notional < min_notional:
            # Suggest qty to clear notional threshold (no buffer in validator)
            needed = quantize_size(min_notional / adj_price, step)
            if needed < min_size:
                needed = min_size
            return ValidationResult(ok=False, reason="min_notional", adjusted_qty=needed, adjusted_price=adj_price)

    return ValidationResult(ok=True, adjusted_qty=adj_qty, adjusted_price=adj_price)
...
def calculate_safe_position_size(
    *,
    ...
    safe_qty = quantize_size(max(intended_qty, min_size * Decimal('1.10')), step)
    if min_notional:
        notional = safe_qty * ref_price
