...
Handles tier detection, transitions, and tier-specific behavior.
"""

from typing import Dict, Optional, Tuple, List
import logging

from .types import PortfolioTier, TierConfig, PortfolioConfig, PortfolioSnapshot
...
class TierManager:
    ...
    def get_tier_transitions_needed(
        self, 
        ...
        should_transition, target_tier = self.should_transition(
            current_tier, current_capital
        )
    ...
    def recommend_tier_transition_steps(
        ...
        small_positions = [
            pos for pos in portfolio_snapshot.positions 
        ...
        if target_config.risk.daily_limit_pct < current_config.risk.daily_limit_pct:
            steps.append(f"4. Reduce position sizes to meet lower risk limits")
