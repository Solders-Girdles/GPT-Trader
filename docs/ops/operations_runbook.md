...
- [ ] Confirm streaming telemetry is updating (latest timestamps in `var/logs/coinbase_trader.log`)
- [ ] Review overnight PnL and positions via `poetry run coinbase-trader account snapshot`
- [ ] Scan `var/logs/coinbase_trader.log` for new ERROR/CRITICAL entries
...
- **Dry-run smoke** – `poetry run coinbase-trader run --profile dev --dev-fast` should complete without errors on staging infrastructure.
...
# Check recent errors
tail -n 100 var/logs/coinbase_trader.log | grep ERROR
...
poetry run coinbase-trader orders apply-edit --order-id ORDER_ID --preview-id PREVIEW_ID
...
tail -f var/logs/coinbase_trader.log | grep -E "ERROR|CRITICAL"

# PnL tracking
grep "PnL" var/logs/coinbase_trader.log | tail -20

# Order execution analysis
grep "order.*filled" var/logs/coinbase_trader.log | tail -10
```
...
2. Capture logs (`journalctl -u coinbase-trader` or container logs).
...
1. Guard will raise `market_data_staleness`. Switch to reduce-only mode with `poetry run coinbase-trader run --profile spot --reduce-only`.
...
- `poetry run coinbase-trader run --profile canary --dry-run` – Protective canary run before deployments.
- `poetry run coinbase-trader account snapshot` – On-demand permissions audit.
...
# Backup results
cp -r var/data/coinbase_trader/ /backup/perps_data_$(date +%Y%m%d)/
```
...
- System Logs: `var/logs/coinbase_trader.log`
- Configuration: `.env`
