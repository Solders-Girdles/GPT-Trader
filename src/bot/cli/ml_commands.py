...
class MLTrainCommand(BaseCommand):
    ...
    def add_parser(cls, subparsers):
        ...
        parser.add_argument(
            "--validation-split", type=float, default=0.2, help="Validation data split ratio"
        )
        ...
        parser.add_argument(
            "--evaluate", action="store_true", help="Evaluate model performance after training"
        )
    ...
    def _train_strategy_selector(
        self, data, save_path, optimize_hyperparams, n_trials, validation_split
    ):
...
class AutoTradeCommand(BaseCommand):
    ...
    def add_parser(cls, subparsers):
        ...
        # Model paths
        parser.add_argument(
            "--regime-model", type=Path, help="Path to trained regime detector model"
        )

        parser.add_argument(
            "--strategy-model", type=Path, help="Path to trained strategy selector model"
        )

        # Rebalancing parameters
        parser.add_argument(
            "--rebalance-frequency", type=int, default=30, help="Rebalancing frequency in days"
        )
        ...
        parser.add_argument("--stop-loss", type=float, default=0.1, help="Stop loss threshold")

        parser.add_argument(
            "--max-drawdown", type=float, default=0.2, help="Maximum drawdown before halting"
        )

        # Execution parameters
        parser.add_argument(
            "--check-interval", type=int, default=60, help="Market check interval in seconds"
        )

        parser.add_argument(
            "--trading-hours-only", action="store_true", help="Trade only during market hours"
        )

        # Output parameters
        parser.add_argument("--log-trades", action="store_true", help="Log all trades to file")
    ...
    def _initialize_components(self, args):
        ...
        try:
            ...
            from ..rebalancing import RebalancingConfig, RebalancingEngine
            ...
            components["rebalancer"] = RebalancingEngine(
                config=config, allocator=components["allocator"]
            )
        ...
        while True:
            ...
            if needs_rebalancing:
                ...
                result = components["rebalancer"].execute_rebalancing(
                    current_positions, details.get("target_positions", {}), current_prices
                )
