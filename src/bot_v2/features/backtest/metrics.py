...
from ...validation import (
    validate_inputs, SeriesValidator, PositiveNumberValidator
)
...
def calculate_metrics(
    ...
    try:
        ...
        if isinstance(e, (BacktestError, ValidationError)):
            raise
        raise BacktestError(
            f"Failed to calculate performance metrics",
            context={
                'trades_count': len(trades),
                'equity_points': len(equity_curve),
                'returns_points': len(returns),
...
def _validate_metrics_inputs(
    ...
    if equity_curve.isna().all():
        raise ValidationError(
            "Equity curve contains only NaN values",
            field="equity_curve"
        )
...
def _calculate_trade_statistics(
    ...
    try:
        ...
        if metrics.total_trades > 0:
            ...
            # Calculate win rate
            metrics.win_rate = (wins / metrics.total_trades) * 100 if metrics.total_trades > 0 else 0.0
...
def validate_metric_value(value: float, metric_name: str, bounds: Optional[tuple] = None) -> float:
    """
