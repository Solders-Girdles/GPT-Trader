...
# ============================================================
# Test: Transient failure with client_order_id reuse (Phase 5.4)
# ============================================================
...
class TestTransientFailureWithClientOrderIdReuse:
    ...
    def test_transient_failure_then_success_reuses_client_order_id(
        ...
        This verifies that:
        1. The same client_order_id is used across retry attempts
        2. Only one order is recorded as open after success
        """
        from datetime import datetime
        from decimal import Decimal
        ...
        # Track client_ids passed to broker
        captured_client_ids: list[str] = []
        call_count = [0]
        ...
        # Order should NOT be in open_orders after failure
        assert fixed_client_id not in open_orders

        # Second submission (retry) with same client_order_id succeeds
        ...
        assert result2 is not None  # Succeeded
        assert result2 == "order-success-123"  # Returns order_id string

        # Verify same client_id was used for both attempts
        assert len(captured_client_ids) == 2
        assert captured_client_ids[0] == fixed_client_id
        assert captured_client_ids[1] == fixed_client_id

        # Verify only one open order after success (broker order ID is stored)
        # The open_orders list stores broker order IDs, not client_order_ids
