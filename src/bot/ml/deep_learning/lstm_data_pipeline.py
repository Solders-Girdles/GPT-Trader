...
class LSTMDataPipeline:
    ...
    def fit_transform(
        self, X_sequences: np.ndarray, y_sequences: np.ndarray
    ...
    def _magnitude_warping(self, X_seq: np.ndarray) -> np.ndarray:
        ...
        warp_curve = np.interp(
            np.linspace(0, 1, seq_len), np.linspace(0, 1, warp_steps), warp_points
        )

        return X_seq * warp_curve[:, np.newaxis]

    def time_series_split(
        self, X_sequences: np.ndarray, y_sequences: np.ndarray, timestamps: np.ndarray
        ...
        if train_end <= 0:
            raise ValueError(
                "Not enough data for train/val/test split with specified sizes and gaps"
            )
        ...
        val_data = (X_sequences[val_indices], y_sequences[val_indices], timestamps[val_indices])

        test_data = (X_sequences[test_indices], y_sequences[test_indices], timestamps[test_indices])

        logger.info(
            f"Split data: Train={len(train_data[0])}, Val={len(val_data[0])}, Test={len(test_data[0])}"
        )
