...
from .triggers import RebalancingTrigger, RegimeTrigger, ThresholdTrigger, TimeTrigger
...
class RebalancingEngine:
    ...
    def __init__(
        ...
        allocator: MLEnhancedAllocator | None = None,
    ...
    def _initialize_triggers(self) -> list[RebalancingTrigger]:
        ...
        # Time-based trigger
        triggers.append(TimeTrigger(interval_days=self.config.time_interval_days))

        # Regime change trigger (if available)
        if self.config.regime_aware_rebalancing and self.regime_detector:
            triggers.append(RegimeTrigger(regime_detector=self.regime_detector))
    ...
    def check_rebalancing_needed(
        ...
        for trigger in self.triggers:
            ...
            if triggered:
                trigger_results.append(
                    {"trigger": trigger.__class__.__name__, "urgency": urgency, "details": details}
                )
        ...
        target_positions = self.allocator.allocate(
            universe=universe, market_data=market_data, capital=portfolio_value
        )
        ...
        cost_benefit_ratio = (
            expected_benefit / cost_estimate["total_cost"]
    ...
    def execute_rebalancing(
        ...
        if turnover_ratio > self.config.max_turnover:
            ...
            self.logger.warning(
                f"Scaled down trades to meet turnover constraint ({turnover_ratio:.1%} -> {self.config.max_turnover:.1%})"
            )
    ...
    def get_rebalancing_schedule(self) -> dict[str, Any]:
        ...
        if self.last_rebalance_date:
            ...
            schedule["next_scheduled"] = self.last_rebalance_date + timedelta(
                days=self.config.time_interval_days
            )
    ...
    def analyze_rebalancing_history(self) -> dict[str, Any]:
        ...
        analysis = {
            ...
            "cost_per_dollar_traded": (
                (total_cost / self.total_turnover) if self.total_turnover > 0 else 0
            ),
        }
