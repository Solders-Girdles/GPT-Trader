...
class RiskLimitExceeded(TradingError):
    ...
    def __init__(self, message: str, limit_type: str, limit_value: float, current_value: float, **kwargs):
...
class SliceIsolationError(TradingError):
    ...
    def __init__(self, message: str, slice_name: str, violation: str, **kwargs):
...
class AggregateError(TradingError):
    ...
    def __init__(self, message: str, errors: List[TradingError], **kwargs):
...
# Helper functions
def handle_error(error: Exception, context: Optional[Dict[str, Any]] = None) -> TradingError:
...
def log_error(error: TradingError, level: int = logging.ERROR):
