...
class TestDegradationIntegration:
    ...
    def test_degradation_check(self):
        ...
        integrator.set_baseline(
            baseline_features, baseline_predictions, baseline_actuals, baseline_confidences
        )
        ...
        report_degraded = integrator.check_degradation(
            degraded_features, degraded_predictions, degraded_actuals, degraded_confidences
        )

        assert report_degraded.overall_status in ["degraded", "critical"]
        assert report_degraded.advanced_score > 0.3
        print(
            f"✅ Degraded check: {report_degraded.overall_status} (score={report_degraded.advanced_score:.2f})"
        )
    ...
    def test_retraining_decision(self):
        ...
        report = integrator.check_degradation(
            features, bad_predictions, bad_actuals, low_confidences
        )
    ...
    def test_status_summary(self):
        ...
        for i in range(3):
            ...
            integrator.check_degradation(shifted_features, predictions, actuals, confidences)
    ...
    def test_confidence_calculation(self):
        ...
        report_ambiguous = integrator.check_degradation(
            features, mixed_predictions, actuals, confidences * 0.7
        )
