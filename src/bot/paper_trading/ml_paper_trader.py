...
class MLPaperTrader(BaseComponent):
    ...
    def __init__(self, config: MLPaperTradingConfig | None = None):
        super().__init__()
    ...
    async def _train_models(self):
        ...
        for symbol in symbols:
            data = await self.data_source.fetch_historical(symbol, start_date, end_date)
            if data is not None:
    ...
    def _generate_predictions(self, market_data: dict[str, pd.DataFrame]) -> dict:
        ...
        for symbol, data in market_data.items():
            try:
                ...
                regime_confidence = self.regime_detector.get_regime_confidence(latest_features)[1][
                    0
                ]

                # Select strategy
                strategy, confidence, probs = (
                    self.strategy_selector.select_strategy_with_confidence(latest_features)
                )
    ...
    def _open_position(self, symbol: str, size: float, price: float, side: str):
        ...
        if order and order.status == "filled":
            ...
            logger.info(
                f"Opened position: {symbol} - {shares:.2f} shares @ ${order.filled_price:.2f}"
            )
    ...
    def _close_position(self, symbol: str, reason: str):
        ...
        # Execute sell order
        order = self._execute_order(symbol, position.quantity, position.current_price, "sell")
        ...
        if order and order.status == "filled":
            ...
            logger.info(
                f"Closed position: {symbol} - P&L: ${position.pnl:.2f} ({position.pnl_percent:.2f}%)"
            )
    ...
    def _update_metrics(self):
        ...
        if len(self.daily_returns) > 20:  # Need sufficient data
            ...
            self.metrics_history.append({"timestamp": current_time, **metrics})
    ...
    def _calculate_performance_metrics(self) -> dict[str, float]:
        ...
        # Basic metrics
        total_return = (
            self._calculate_portfolio_value() - self.config.initial_capital
