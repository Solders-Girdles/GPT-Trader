...
def test_request_retries_on_429_and_5xx_then_succeeds(
...
def test_request_fast_fails_on_auth_errors(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(base_module.time, "sleep", lambda _s: None)
    client = _make_client()
...
def test_non_json_response_yields_brokerage_error(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(base_module.time, "sleep", lambda _s: None)
    client = _make_client()
