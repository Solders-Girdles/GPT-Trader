...
try:
    ...
    HAS_PANDAS = False
    # Use Any as placeholder when pandas is not available
...
class StrategySelector:
    ...
    def __init__(self, config: PortfolioConfig, data_provider: DataProvider):
        """Initialize with portfolio configuration and data provider."""
        self.config = config
        self.data_provider = data_provider
        self.logger = logging.getLogger(__name__)
    
    def _safe_get_price(self, data, column: str, index: int):
        ...
        if hasattr(data, 'data'):
            ...
            if column in data.data and abs(index) < len(data.data[column]):
                return data.data[column][index]
            return None
        else:
            # pandas DataFrame
            if column in data.columns and abs(index) < len(data):
    ...
    def _get_symbol_universe(
        ...
        base_universe = [
            ...
            "SPY", "QQQ", "IWM", "VTI", "BRK-B", "JNJ", "V", "JPM", "UNH",
            "HD", "PG", "DIS", "MA", "BAC", "ADBE", "CRM", "PYPL"
        ]
