...
class RecoveryOperation:
    ...
    data_loss_estimate: Optional[str] = None
...
class RecoveryHandler:
    ...
    def __init__(self, state_manager: Any, checkpoint_handler: Any, 
                 backup_manager: Any = None, config: Optional[RecoveryConfig] = None):
        ...
        self._current_operation: Optional[RecoveryOperation] = None
        ...
        logger.info(f"RecoveryHandler initialized with RTO={self.config.rto_minutes}min, "
    ...
    async def initiate_recovery(self, failure_event: FailureEvent, 
                               mode: RecoveryMode = RecoveryMode.AUTOMATIC) -> RecoveryOperation:
        """
        ...
        with self._recovery_lock:
            ...
            try:
                logger.info(f"Starting recovery operation {operation.operation_id} "
                          f"for {failure_event.failure_type.value} in {mode.value} mode")
                
                # Send initial alert
                await self._send_alert(
                    f"Recovery started for {failure_event.failure_type.value}",
                    operation
                )
                ...
                if success:
                    ...
                    if await self._validate_recovery(operation):
                        ...
                        logger.info(f"Recovery {operation.operation_id} completed successfully "
                                  f"in {operation.recovery_time_seconds:.2f} seconds")
                        
                        # Check RTO compliance
                        if operation.recovery_time_seconds > self.config.rto_minutes * 60:
                            logger.warning(f"Recovery exceeded RTO: {operation.recovery_time_seconds}s "
                                         f"> {self.config.rto_minutes * 60}s")
                    else:
                ...
                await self._send_alert(
                    f"Recovery {operation.status.value} for {failure_event.failure_type.value}",
                    operation
                )
                ...
                operation.status = RecoveryStatus.FAILED
                operation.actions_taken.append(f"Error: {str(e)}")
    ...
    async def _recover_postgres(self, operation: RecoveryOperation) -> bool:
        ...
        try:
            ...
            operation.data_loss_estimate = f"Up to {time_since_checkpoint.total_seconds():.0f} seconds"
    ...
    async def _test_s3_health(self) -> bool:
        ...
        try:
            if self.state_manager.s3_client:
                self.state_manager.s3_client.head_bucket(
                    Bucket=self.state_manager.config.s3_bucket
                )
                return True
        except Exception as exc:
    ...
    async def _escalate_recovery(self, operation: RecoveryOperation):
    ...
    def _generate_manual_recovery_checklist(self, operation: RecoveryOperation) -> str:
        ...
        return checklists.get(failure_type, "1. Assess situation\n2. Contact support\n3. Follow runbook")
    
    async def _send_alert(self, message: str, operation: RecoveryOperation, 
                         priority: str = "normal"):
        """Send recovery alert"""
        try:
            alert_data = {
            ...
            # Store alert
            await self.state_manager.set_state(
                f"alert:{operation.operation_id}:{datetime.utcnow().timestamp()}",
                alert_data
            )
    ...
    def get_recovery_stats(self) -> Dict[str, Any]:
        ...
        successful = [op for op in self._recovery_history 
                     if op.status == RecoveryStatus.COMPLETED]
        
        total_time = sum(op.recovery_time_seconds for op in successful 
        ...
        return {
            ...
            'rto_compliance': all(
                ...
                for op in successful if op.recovery_time_seconds
            ),
            'last_recovery': self._recovery_history[-1].started_at if self._recovery_history else None
        }


# Convenience functions
async def detect_and_recover(state_manager: Any, checkpoint_handler: Any, 
                            backup_manager: Any = None) -> Optional[RecoveryOperation]:
