...
def render_regime_detection():
    ...
    with st.expander("Transition Matrix"):
        transition_matrix = pd.DataFrame(
            np.random.rand(5, 5) * 0.2 + np.eye(5) * 0.8,
            index=regimes,
            columns=regimes
        )
...
def render_confidence_metrics():
    ...
    for model, (status, updated) in models.items():
        ...
        st.markdown(f"• {model}: <span style='color: {status_color}'>{status}</span>", 
                   unsafe_allow_html=True)
...
def render_portfolio_optimization_page():
    ...
    with st.sidebar:
        ...
        objective = st.selectbox(
            "Objective",
            ["Max Sharpe", "Min Risk", "Max Return", "Risk Parity"]
        )
...
def render_rebalancing_analysis():
    ...
    with col1:
        ...
        for trigger, details in triggers.items():
            status_color = "#00cc88" if details["status"] == "Active" else "#ff9900" if details["status"] == "Pending" else "#999"
    ...
    with col2:
        ...
        for key, value in analysis.items():
            if key == "Decision":
                ...
                st.markdown(f"**{key}:** <span style='color: {color}'>{value}</span>", 
                          unsafe_allow_html=True)
            else:
        ...
        history_df = pd.DataFrame({
            'Date': pd.date_range(end=datetime.now(), periods=5, freq='30D').strftime('%Y-%m-%d'),
            'Trades': [6, 8, 5, 7, 9],
...
def render_ml_dashboard():
    ...
    page = st.sidebar.selectbox(
        "Select Page",
        ["ML Predictions", "Portfolio Optimization"]
    )
