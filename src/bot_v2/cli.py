...
def main():
    ...
    parser.add_argument("--profile", type=str, default="dev", choices=["dev", "demo", "prod", "canary", "spot"], help="Configuration profile")
    parser.add_argument("--dry-run", action="store_true", help="Run without placing real orders")
    parser.add_argument("--symbols", type=str, nargs="+", help="Symbols to trade (e.g., BTC-PERP ETH-PERP)")
    ...
    parser.add_argument("--reduce-only", dest="reduce_only_mode", action="store_true", help="Enable reduce-only mode")
    # Optional TIF override maps to BotConfig.time_in_force (validated later)
    parser.add_argument("--tif", dest="time_in_force", type=str, choices=["GTC", "IOC", "FOK"], help="Time in force policy (GTC/IOC/FOK)")
    parser.add_argument("--enable-preview", dest="enable_order_preview", action="store_true", help="Enable order preview before placement")
    parser.add_argument("--account-interval", dest="account_telemetry_interval", type=int, help="Account telemetry interval in seconds")
    parser.add_argument("--account-snapshot", action="store_true", help="Print account telemetry snapshot and exit")
    parser.add_argument("--convert", metavar="FROM:TO:AMOUNT", help="Perform a convert trade and exit")
    parser.add_argument("--move-funds", metavar="FROM:TO:AMOUNT", help="Move funds between portfolios and exit")
    ...
    parser.add_argument("--edit-order-preview", metavar="ORDER_ID", help="Preview edits for ORDER_ID and exit")
    parser.add_argument("--apply-order-edit", metavar="ORDER_ID:PREVIEW_ID", help="Apply order edit using preview id and exit")
    parser.add_argument("--order-symbol", help="Symbol for order preview/edit commands")
    parser.add_argument("--order-side", choices=["buy", "sell"], help="Order side for preview/edit commands")
    parser.add_argument("--order-type", choices=["market", "limit", "stop_limit"], help="Order type for preview/edit commands")
    parser.add_argument("--order-qty", type=Decimal, help="Order quantity for preview/edit commands")
    ...
    parser.add_argument("--order-tif", choices=["GTC", "IOC", "FOK"], help="Time in force for preview/edit commands")
    parser.add_argument("--order-client-id", help="Client order id for preview/edit commands")
    parser.add_argument("--order-reduce-only", action="store_true", help="Set reduce_only flag for preview/edit commands")
    parser.add_argument("--order-leverage", type=int, help="Leverage override for preview/edit commands")
    ...
    skip_keys = {
        'profile', 'account_snapshot', 'convert', 'move_funds', 'preview_order',
        'edit_order_preview', 'apply_order_edit', 'order_side', 'order_type',
        'order_qty', 'order_price', 'order_stop', 'order_tif', 'order_client_id',
        'order_reduce_only', 'order_leverage', 'order_symbol'
    }
        ...
        try:
            if args.preview_order:
                if not args.order_side or not args.order_type or args.order_qty is None:
                    parser.error("--preview-order requires --order-side, --order-type, and --order-qty")
            ...
            if args.edit_order_preview:
                if not args.order_side or not args.order_type or args.order_qty is None:
                    parser.error("--edit-order-preview requires --order-side, --order-type, and --order-qty")
                side = OrderSide[args.order_side.upper()]
                order_type = OrderType[args.order_type.upper()]
                qty = Decimal(str(args.order_qty))
                tif = TimeInForce[args.order_tif.upper()] if args.order_tif else TimeInForce.GTC
