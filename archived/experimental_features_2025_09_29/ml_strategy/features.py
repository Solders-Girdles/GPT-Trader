...
from bot_v2.features.ml_strategy.types import MarketConditions
...
def engineer_features(conditions: MarketConditions) -> list[float]:
    ...
    try:
        ...
        try:
            stress = (validated_conditions.vix_level / 100) * (
                validated_conditions.volatility / 100
            )
...
def _validate_market_conditions(conditions: MarketConditions) -> MarketConditions:
    ...
    try:
        ...
        validated = MarketConditions(
            volatility=_validate_numeric_field(
                conditions.volatility, "volatility", 0.0, 200.0, 20.0
            ),
            trend_strength=_validate_numeric_field(
                conditions.trend_strength, "trend_strength", -100.0, 100.0, 0.0
            ),
            volume_ratio=_validate_numeric_field(
                conditions.volume_ratio, "volume_ratio", 0.1, 10.0, 1.0
            ),
            price_momentum=_validate_numeric_field(
                conditions.price_momentum, "price_momentum", -100.0, 100.0, 0.0
            ),
            ...
            correlation_spy=_validate_numeric_field(
                conditions.correlation_spy, "correlation_spy", -1.0, 1.0, 0.7
            ),
        )
...
def _validate_numeric_field(
    value: float, field_name: str, min_val: float, max_val: float, default_val: float
) -> float:
    ...
    try:
        ...
        if value < min_val:
            logger.warning(
                f"{field_name} below minimum ({value} < {min_val}), clipping to {min_val}"
            )
            return min_val

        if value > max_val:
            logger.warning(
                f"{field_name} above maximum ({value} > {max_val}), clipping to {max_val}"
            )
            return max_val
...
def _validate_feature_array(features: np.ndarray) -> None:
    ...
    if len(features) != expected_length:
        raise DataError(
            f"Feature array length mismatch: expected {expected_length}, got {len(features)}"
        )
