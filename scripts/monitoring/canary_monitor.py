    ...
    async def monitor_positions(self):
        ...
        while not self.kill_switch_triggered:
            try:
                ...
                # Calculate total PnL
                total_pnl = sum(p.get("unrealized_pnl", 0) for p in self.metrics["positions"].values())
        ...
        while not self.kill_switch_triggered:
            try:
                ...
                for order in orders.get("orders", []):
                    ...
                    if created_at:
                        order_timestamps.append(datetime.fromisoformat(created_at.replace("Z", "+00:00")))
    ...
    def trigger_violation(self, message: str):
        ...
        if self.alert_dispatcher:
            try:
                ...
                severity = (
                    AlertSeverity.CRITICAL if len(self.metrics["violations"]) >= 2
