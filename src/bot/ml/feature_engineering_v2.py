...
class OptimizedFeatureEngineer:
    ...
    def __init__(self, config: Optional[FeatureConfig] = None):
        ...
        self.essential_features = {
            ...
            FeatureCategory.TREND: [
                ...
                'ema_trend_strength', 'sma_trend_alignment',
                'ichimoku_signal', 'supertrend_signal'
            ],
            FeatureCategory.MARKET_MICROSTRUCTURE: [
                'bid_ask_spread', 'order_flow_imbalance',
                'tick_rule', 'effective_spread',
                'price_impact'
            ]
        }
        
        logger.info(f"OptimizedFeatureEngineer initialized with max {self.config.max_features} features")
    ...
    def calculate_price_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features['close_to_high'] = (df['high'] - df['close']) / (df['high'] - df['low']).replace(0, np.nan)
        features['close_to_low'] = (df['close'] - df['low']) / (df['high'] - df['low']).replace(0, np.nan)
    ...
    def calculate_trend_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features['adx_trend_strength'] = pd.cut(pd.Series(adx), 
                                               bins=[0, 25, 50, 75, 100], 
                                               labels=[0.25, 0.5, 0.75, 1.0]).astype(float)
        ...
        features['supertrend_signal'] = np.where(df['close'] > upper_band, 1, 
    ...
    def calculate_microstructure_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features['order_flow_imbalance'] = (2 * df['close'] - df['high'] - df['low']) / (df['high'] - df['low']).replace(0, np.nan)
        ...
        features['effective_spread'] = 2 * np.sqrt(np.abs(price_changes.rolling(20).cov(price_changes.shift(1))))
        
        # Price impact (Kyle's lambda approximation)
        returns = df['close'].pct_change()
        volume_sqrt = np.sqrt(df['volume'])
        features['price_impact'] = returns.rolling(20).std() / volume_sqrt.rolling(20).mean().replace(0, np.nan)
    ...
    def remove_low_variance_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.debug(f"Removed {len(features.columns) - len(selected_features)} low variance features")
        
        return pd.DataFrame(features_array, 
...
def create_feature_pipeline(config: Optional[FeatureConfig] = None) -> OptimizedFeatureEngineer:
