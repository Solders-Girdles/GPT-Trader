...
class RebalancingTrigger(ABC):
    ...
    def check(self,
             ...
             market_data: Optional[Dict[str, pd.DataFrame]] = None) -> Tuple[bool, float, Dict[str, Any]]:
...
class ThresholdTrigger(RebalancingTrigger):
    ...
    def check(self,
        ...
        if max_deviation >= self.emergency_threshold:
            ...
            urgency = 0.5 + 0.5 * ((max_deviation - self.threshold) / 
                                  (self.emergency_threshold - self.threshold))
        ...
        if triggered:
            ...
            self.logger.info(f"Threshold trigger fired: max deviation {max_deviation:.1%}, urgency {urgency:.1f}")
...
class TimeTrigger(RebalancingTrigger):
    ...
    def check(self,
        ...
        if days_since >= self.max_interval_days:
            ...
            urgency = 0.3 + 0.7 * ((days_since - self.interval_days) / 
                                  (self.max_interval_days - self.interval_days))
        ...
        if triggered:
            self.last_triggered = datetime.now()
            self.logger.info(f"Time trigger fired: {days_since} days since last rebalance, urgency {urgency:.1f}")
...
class VolatilityTrigger(RebalancingTrigger):
    ...
    def check(self,
        ...
        if triggered:
            ...
            self.last_triggered = datetime.now()
            self.logger.info(f"Volatility trigger fired: regime change from {self.last_vol_regime} to {current_regime}")
...
class RegimeTrigger(RebalancingTrigger):
    ...
    def check(self,
        ...
        details = {
            ...
            'transition': f"{self.last_regime} -> {current_regime}" if self.last_regime else "Initial"
        }
        
        if triggered:
            self.logger.info(f"Regime trigger fired: {details['transition']}, urgency {urgency:.1f}")
            self.last_triggered = datetime.now()
...
class DrawdownTrigger(RebalancingTrigger):
    ...
    def check(self,
        ...
        if drawdown >= self.emergency_drawdown:
            ...
            triggered = True
            # Scale urgency
            urgency = 0.5 + 0.5 * ((drawdown - self.drawdown_threshold) / 
        ...
        if triggered:
            self.last_triggered = datetime.now()
            self.logger.info(f"Drawdown trigger fired: {drawdown:.1%} drawdown, urgency {urgency:.1f}")
