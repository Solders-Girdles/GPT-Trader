...
class PositionState:
    ...
    def update_position(
        ...
        elif is_reduce or (self.side == 'long' and side == 'sell') or (self.side == 'short' and side == 'buy'):
...
class PnLTracker:
    ...
    def accrue_funding(
        ...
        position = self.get_or_create_position(symbol)
        return self.funding_calc.accrue_if_due(
    ...
    def generate_daily_metrics(self, current_equity: Decimal) -> Dict[str, any]:
        """
        ...
        max_drawdown = max(
            (p.max_drawdown for p in self.positions.values()),
            default=Decimal('0')
        )
    ...
    def get_position_metrics(self) -> List[Dict[str, any]]:
