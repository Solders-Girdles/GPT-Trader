...
class ExecutionEngine:
    ...
    def place_order(
        ...
        def _place_order_with_broker() -> Order:
            return cast(
                Order,
        ...
        try:
            order = cast(
                ...
                error_handler.with_retry(
                    _place_order_with_broker, recovery_strategy=RecoveryStrategy.RETRY
                ),
            )
    ...
    def update_order_status(self) -> None:
        ...
        def _get_orders_from_broker() -> list[Order]:
            return cast(list[Order], self.broker.get_orders("all"))

        try:
            # Get current orders from broker with error handling
            current_orders = error_handler.with_retry(
                _get_orders_from_broker, recovery_strategy=RecoveryStrategy.RETRY
            )
            ...
            for current_order in current_orders:
                if current_order.id in self.pending_orders:
                    ...
                    if pending_order.status != current_order.status:
                        ...
                        pending_order.filled_quantity = current_order.filled_quantity
    ...
    def _log_execution(self, order: Order, event_type: str) -> None:
        """
        ...
        """
        # Convert Decimal quantity to int for ExecutionReport
        quantity_int = int(order.quantity)
        price_source: Decimal | float | None = (
            order.avg_fill_price if order.avg_fill_price is not None else order.price
        )
        price_float = float(price_source) if price_source is not None else 0.0
        ...
        # Emit execution message via standard logging
        if event_type == "ORDER_PLACED":
            ...
            filled_int = int(order.filled_quantity)
            logger.warning(
            ...
            logger.error("🚫 Order rejected: %s", order.id)
