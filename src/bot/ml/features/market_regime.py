...
class MarketRegimeFeatures(FeatureEngineer):
    ...
    def _generate_return_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Cumulative returns
        features['cum_returns_20d'] = (1 + features['returns_1d']).rolling(20).apply(
            lambda x: x.prod() - 1, raw=False
        )
        features['cum_returns_60d'] = (1 + features['returns_1d']).rolling(60).apply(
            lambda x: x.prod() - 1, raw=False
        )
