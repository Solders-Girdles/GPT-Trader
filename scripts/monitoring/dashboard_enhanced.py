...
class EnhancedDashboard:
    ...
    def __init__(self):
        ...
        self.log_file = log_dir / f"dashboard_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def calculate_avg_latency(self, window_minutes: int = 5) -> float:
        ...
        recent_latencies = [l['latency_ms'] for l in self.metrics['latencies'] 
    ...
    def render_dashboard(self):
        ...
        print(f"Uptime: {str(uptime).split('.')[0]} | Environment: {os.getenv('COINBASE_API_MODE', 'unknown')}")
        ...
        acceptance_color = "🟢" if acceptance_rate >= 90 else "🟡" if acceptance_rate >= 75 else "🔴"
        ...
        health_checks = {
            ...
            'No Recent Errors': len([e for e in self.metrics['errors'] 
                                   if (datetime.now(timezone.utc) - e['timestamp']).seconds < 300]) == 0
        }
    ...
    async def simulate_data(self):
        ...
        while True:
            ...
            if random.random() < 0.3:  # 30% chance
                ...
                rejection_reason = random.choice(['POST_ONLY_WOULD_CROSS', 'INSUFFICIENT_FUNDS', 'SIZE_TOO_SMALL']) if status == 'rejected' else None
...
async def main():
    ...
    parser.add_argument('--simulate', action='store_true',
                       help='Simulate trading data for testing')
