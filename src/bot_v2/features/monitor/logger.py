...
class ProductionLogger:
    ...
    def __init__(self, service_name: str = "bot_v2", enable_console: bool = True):
        """
    ...
    def get_correlation_id(self) -> str:
        ...
        return self.correlation_ids.value
    ...
    def _emit_log(self, entry: Dict[str, Any]):
        ...
        try:
            if _LEVEL_MAP.get(entry.get("level", "info"), logging.INFO) < _LEVEL_MAP.get(self._min_level, logging.INFO):
                return
        except Exception:
            pass
    ...
    def log_pnl(
        ...
        transition: Optional[str] = None,  # e.g., "unrealized->realized"
        **kwargs,
    ):
    ...
    def log_order_submission(
        ...
        **kwargs,
        ...
        entry = self._create_log_entry(
            ...
            message=f"submit {side} {qty} {symbol} @{price if price is not None else 'mkt'}",
    ...
    def log_order_status_change(
        ...
        **kwargs,
        ...
        level = LogLevel.WARNING if to_status in ("REJECTED", "CANCELLED", "EXPIRED") else LogLevel.INFO
        entry = self._create_log_entry(
            level=level,
    ...
    def log_ws_latency(self, stream: str, latency_ms: float, **kwargs):
        entry = self._create_log_entry(
            level=LogLevel.INFO,
    ...
    def log_rest_response(self, endpoint: str, method: str, status_code: int, duration_ms: float, **kwargs):
    ...
    def log_strategy_duration(self, strategy: str, duration_ms: float, **kwargs):
        entry = self._create_log_entry(
            level=LogLevel.INFO,
    ...
    def get_recent_logs(self, count: int = 100) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            return self._recent_logs[-count:] if count < len(self._recent_logs) else self._recent_logs.copy()
...
def log_event(event_type: str, message: str, level: LogLevel = LogLevel.INFO, **kwargs):
    """Log a system event."""
    logger = get_logger()
    logger.log_event(level, event_type, message, **kwargs)


def log_trade(action: str, symbol: str, quantity: float, price: float, strategy: str, **kwargs):
    """Log a trade execution."""
    logger = get_logger()
    logger.log_trade(action, symbol, quantity, price, strategy, **kwargs)


def log_ml_prediction(model_name: str, prediction: Any, **kwargs):
