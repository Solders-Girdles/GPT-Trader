...
def create_realistic_btc_scenario(
    scenario_type: str = "profit", position_size: Decimal = Decimal("0.1"), hold_days: int = 1
) -> TradeScenario:
    """
...
def create_risk_limit_test_scenario(limit_type: str, should_pass: bool = False) -> dict[str, Any]:
    """
...
def run_behavioral_validation(
    ...
    tolerances: dict[str, Decimal] | None = None,
) -> tuple[bool, list[str]]:
    """
    ...
    pnl_passed, pnl_msg = validate_pnl_calculation(
        trades_for_validation, scenario.expected_pnl, tolerances["pnl"]
    )
...
def create_market_stress_scenario() -> list[TradeScenario]:
    ...
    scenarios.append(
        create_realistic_btc_scenario(scenario_type="loss", position_size=Decimal("0.05"))
    )

    # High volatility ETH
    scenarios.append(create_realistic_eth_scenario(scenario_type="short_loss", volatility="high"))
