    ...
    class RolloutBuffer:
        ...
        def compute_returns_and_advantages(
            ...
            for step in reversed(range(self.buffer_size)):
                ...
                delta = (
                    self.rewards[step] + gamma * next_value * next_non_terminal - self.values[step]
                )
    ...
    class PPO:
        ...
        def collect_rollouts(self, env) -> bool:
            ...
            self.rollout_buffer.compute_returns_and_advantages(
                last_value, self.config.discount_factor, self.config.gae_lambda
            )
        ...
        def update(self) -> dict[str, float]:
            ...
            for epoch in range(self.config.n_epochs):
                ...
                for (
                    ...
                    advantages,
                    returns,
                ) in self.rollout_buffer.get_samples(self.config.minibatch_size):
                    # Convert to tensors
                    ...
                    with torch.no_grad():
                        ...
                        # Clip fraction
                        clip_fraction = (
                            (torch.abs(ratio - 1) > self.config.clip_ratio).float().mean()
                        )
...
def create_ppo_agent(
    state_dim: int, action_dim: int, continuous: bool = False, ppo_type: PPOType = PPOType.CLIP
) -> PPO:
    """Factory function to create PPO agent"""

    config = PPOConfig(
        state_dim=state_dim, action_dim=action_dim, continuous_actions=continuous, ppo_type=ppo_type
    )
...
if __name__ == "__main__":
    ...
    if TORCH_AVAILABLE:
        ...
        config = PPOConfig(
            state_dim=50, action_dim=3, continuous_actions=False, ppo_type=PPOType.CLIP
        )
        ...
        continuous_config = PPOConfig(
            state_dim=50, action_dim=3, continuous_actions=True, ppo_type=PPOType.HYBRID
        )
