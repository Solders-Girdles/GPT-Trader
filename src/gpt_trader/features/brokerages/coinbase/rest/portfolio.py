...
from gpt_trader.features.brokerages.core.interfaces import Balance, Position, InvalidRequestError
...
class PortfolioRestMixin:
        ...
        try:
            ...
            for acc in accounts:
                try:
                    ...
                    avail_val = acc.get("available_balance", {}).get("value") or acc.get("available") or "0"
    ...
    def get_intx_balances(self, portfolio_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self._event_store.append_metric(
                metrics={"event_type": "intx_balances", "data": {"portfolio_id": portfolio_id, "count": len(balances)}}
            )
            return balances
        except Exception:
            return []
