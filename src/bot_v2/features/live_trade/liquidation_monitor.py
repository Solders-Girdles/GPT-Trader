...
class LiquidationRisk:
    ...
    distance_pct: Optional[float] = None       # Distance as percentage
    distance_bps: Optional[float] = None       # Distance in basis points  
...
class LiquidationMonitor:
    ...
    def calculate_liquidation_price(self, margin_info: MarginInfo) -> Optional[Decimal]:
        """
    ...
    def assess_liquidation_risk(self, margin_info: MarginInfo) -> LiquidationRisk:
        ...
        if distance_pct <= 0:
            ...
            result.should_reject_entry = self.enable_entry_rejection
            result.reason = f"Liquidation warning (distance: {distance_pct:.1f}% <= {self.warning_buffer_pct}%)"
    ...
    def should_block_new_position(self, symbol: str, existing_positions: Dict[str, MarginInfo]) -> Tuple[bool, str]:
        """
        ...
        if critical_positions and self.enable_entry_rejection:
            return True, f"Portfolio liquidation risk from positions: {', '.join(critical_positions)}"
    ...
    def log_risk_summary(self, positions: Dict[str, MarginInfo]) -> None:
        ...
        for symbol, margin_info in positions.items():
            ...
            logger.info(f"  Position: {margin_info.position_side} {margin_info.position_size} @ {margin_info.entry_price}")
            logger.info(f"  Current: {margin_info.current_price}, Leverage: {margin_info.leverage}x")
