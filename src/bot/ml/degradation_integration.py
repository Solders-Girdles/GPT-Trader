...
from .model_degradation_monitor import (
    ...
    DegradationStatus,
    DegradationMetrics as LegacyMetrics,
    RetrainingTrigger
)
...
from .advanced_degradation_detector import (
    ...
    DegradationAlert,
    DegradationMetrics as AdvancedMetrics
)
...
class DegradationIntegrator:
    ...
    def __init__(self,
                 legacy_monitor: Optional[ModelDegradationMonitor] = None,
        ...
        logger.info(f"Initialized DegradationIntegrator (legacy={use_legacy}, advanced={use_advanced})")
    ...
    def set_baseline(self,
        ...
        if self.advanced_detector:
            self.advanced_detector.update_baseline(
        ...
        logger.info(f"Baseline set for model {model_id} with accuracy {np.mean(predictions == actuals):.3f}")
    ...
    def check_degradation(self,
                         ...
                         actuals: np.ndarray,
                         confidences: np.ndarray,
                         model_id: str = "default") -> IntegratedDegradationReport:
        """
        ...
        if self.advanced_detector:
            ...
            feature_drift_count = sum(
                1 for p in advanced_metrics.feature_drift_scores.values() 
        ...
        overall_status, primary_issue, recommended_action = self._combine_assessments(
            advanced_status, advanced_score,
            legacy_status, legacy_accuracy,
            feature_drift_count, confidence_decay
        )
        
        # Calculate confidence in assessment
        confidence_level = self._calculate_confidence(
            advanced_score, legacy_status, len(alerts)
        )
    ...
    def _combine_assessments(self,
                           ...
                           legacy_accuracy: float,
                           feature_drift_count: int,
                           confidence_decay: bool) -> Tuple[str, str, str]:
        """
        ...
        if self.use_legacy and self.use_advanced:
            if advanced_score > 0.7 and severity_map.get(legacy_status, 0) >= 2:
                overall_status = "critical"
                recommended_action = "Both systems indicate critical degradation - retrain immediately"
    ...
    def _calculate_confidence(self,
        ...
        if self.use_legacy and self.use_advanced:
            if (advanced_score > 0.6 and legacy_status in [DegradationStatus.DEGRADED, DegradationStatus.CRITICAL]) or \
               (advanced_score < 0.3 and legacy_status == DegradationStatus.HEALTHY):
                confidence += 0.2  # Systems agree
        
        return max(0.0, min(1.0, confidence))
    
    def should_retrain(self, 
                       report: Optional[IntegratedDegradationReport] = None,
        ...
        if not force_check and self.last_retraining:
            ...
            if time_since_retraining < timedelta(hours=6):
                return False, f"Recently retrained {time_since_retraining.total_seconds()/3600:.1f} hours ago"
        ...
        if len(self.reports) >= 5:
            ...
            warning_count = sum(1 for r in recent_reports if r.overall_status in ["warning", "degraded"])
            if warning_count >= 4:
    ...
    def get_status_summary(self) -> Dict[str, Any]:
        ...
        if len(self.reports) >= 3:
            ...
            trend_info['score_trend'] = 'increasing' if recent_scores[-1] > recent_scores[0] else 'decreasing'
...
def create_integrated_monitor(use_legacy: bool = True,
                            use_advanced: bool = True) -> DegradationIntegrator:
    """
